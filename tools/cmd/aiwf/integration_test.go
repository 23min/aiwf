...
func aiwfBinary(t *testing.T) string {
	...
	buildOnce.Do(func() {
		...
		// dir to where go.mod lives. tools/cmd/aiwf -> .. -> .. -> ..
		// The test binary's working dir is the package dir, so
		// `./...` is wrong; pass an absolute module path.
		cmd := exec.Command("go", "build", "-o", bin, "github.com/23min/ai-workflow-v2/tools/cmd/aiwf")
...
func TestIntegration_FreshRepoLifecycle(t *testing.T) {
	...
	// aiwf.yaml exists with the derived actor.
	cfg, err := os.ReadFile(filepath.Join(root, "aiwf.yaml"))
	if err != nil {
		t.Fatal(err)
	}
	if !strings.Contains(string(cfg), "actor: human/peter") {
		t.Errorf("aiwf.yaml missing expected actor: %s", cfg)
	}
