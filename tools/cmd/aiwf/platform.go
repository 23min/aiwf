...
// docs/poc-design-decisions.md): the pre-push hook is a /bin/sh
...
func assertSupportedOS(goos string) error {
	if goos == "windows" {
		return fmt.Errorf("aiwf is not supported on Windows in the PoC (POSIX-only assumptions: /bin/sh hook, exec.LookPath validators, flock(2) repo lock). See docs/poc-design-decisions.md and the README's Known Limitations section")
	}
	return nil
}
