...
// matching entry in tools/internal/check/hint.go's hintTable. A
	...
	for _, sc := range seen {
		...
		out = append(out, Violation{
			...
			Detail: "Finding{Code: " + strconv.Quote(sc.Code) +
				...
				"} has no entry in tools/internal/check/hint.go hintTable",
		})
	}
	return out, nil
}

// loadHintTableKeys reads tools/internal/check/hint.go and returns
...
func loadHintTableKeys(files []FileEntry) map[string]struct{} {
	...
	for _, f := range files {
		if f.Path != "tools/internal/check/hint.go" {
			continue
		}
