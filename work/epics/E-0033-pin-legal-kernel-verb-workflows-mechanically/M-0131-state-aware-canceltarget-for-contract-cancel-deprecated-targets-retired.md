...
- **Trunk-collision resolution landed on the milestone branch, not the epic branch.** Per CLAUDE.md's chokepoint discipline (resolve at the branch whose pre-push fires), the two `aiwf reallocate` commits (G-0149 → G-0151; archived G-0150 → G-0152) landed on `milestone/M-0131-cancel-target-state-aware` rather than on `epic/E-0033`. The reallocated gaps belong to M-0137 work (epic-branch state inherited at milestone-branch creation); the milestone branch was the chokepoint. The reallocate commits will propagate to epic via the milestone-branch merge and to main via the epic-branch merge.
