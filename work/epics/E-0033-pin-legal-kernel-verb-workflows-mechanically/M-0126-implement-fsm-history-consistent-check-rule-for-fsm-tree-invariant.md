...
Implement the `fsm-history-consistent` check rule that makes the per-entity status FSM a **tree-invariant** rather than just a verb-precondition. Closes gap **G-0128** (filed during M-0121's audit). Addresses the catalog's R-RULE-149 placeholder.
...
6. **Update the audit catalog** (`docs/pocv3/design/legal-workflows-audit.md`) to remove R-RULE-149's "currently unimplemented" qualifier and remove the "pending G-0128" note from §10.1's enforcement-status legend.
...
Promote G-0128 to `addressed`:

```
aiwf promote G-0128 addressed
```

(Per the gap FSM: `open → addressed | wontfix`.) Add `addressed_by: [M-0126]` to G-0128's frontmatter in the same wrap commit.

## Related

- **G-0128** — the gap this milestone closes
