...
# E-0021 — Open-work synthesis: aiwfx-whiteboard skill replaces critical-path.md
...
`aiwf status` and `aiwf render roadmap` render *state* (what entities exist, what their statuses are). Neither renders *direction* (what to do next, in what order, with which dependencies foregrounded). That synthesis lived in conversation context during E-0020 planning and would have scrolled away if not captured into `critical-path.md` as a temporary holding doc.
...
The proximate trigger: during E-0020's planning session, the operator named the gap explicitly — *"It's very difficult to keep sequence in my head when there are so many ADRs, epics, milestones, gaps."* The synthesis happened once, manually, with significant LLM judgement. Without a feature, every future session re-derives it from scratch, often inconsistently. With a feature, the synthesis is template-driven, reproducible, and Q&A-gated.

## Scope

### In scope

- A synthesis skill named `aiwfx-whiteboard` (locked during milestone planning; rationale recorded in M-0078's design ADR) that loads tree state via existing read verbs (`aiwf status`, `aiwf list` once E-0020 ships, `aiwf show`, `aiwf history`) and produces:
...
- **Skill-coverage policy compliance** (per E-0020/M-0074). The new skill goes through the policy that lands in M-0074 — proper frontmatter, name matches dir, all referenced verbs resolve.
...
- [ ] The skill's frontmatter description includes synthesis-shaped query phrasings (per the M-0074 description-quality conventions).
...
- [M-0078](M-078-planning-conversation-skills-design-adr-placement-tiering-name-rationale.md) — Design ADR: rituals-plugin placement; pure-skill-first tiering; `aiwfx-whiteboard` name with rejected alternatives · `tdd: none` · depends on: — · sizing ~0.5–1d
- [M-0079](M-079-aiwfx-whiteboard-skill-classification-rubric-output-template-q-a-gate.md) — Skill scaffold + body: tier-classification rubric, output template (landscape, sequence, first decision, pending), Q&A gate, anti-patterns · `tdd: advisory` · depends on: M-0078 · sizing ~2–3d
- [M-0080](M-080-whiteboard-skill-fixture-validation-retire-critical-path-md-close-e-21.md) — Fixture validation against `critical-path.md`; deletion of holding doc; close E-0021 · `tdd: required` · depends on: M-0079 · sizing ~1d
...
- **Depends on E-0020.** The synthesis skill calls `aiwf list` directly (not yet shipped); also expects the skills-coverage policy from E-0020/M-0074 to police the new skill's frontmatter. Starting E-0021 before E-0020 ships would force re-work of both pieces.
- **G-0071 / G-0072 / G-0065 fixes already landed via E-0022** (closed 2026-05-08). The skill now operates on the cleaner baseline `critical-path.md` anticipated. One downstream effect: critical-path.md's Tier 1 row (the bundling-into-M-0072 fork) is now historical — those gaps are closed. The skill's live output will surface a different Tier 1 (or empty Tier 1) than the fixture. M-0080's structural-agreement check tolerates this drift.
- **Compatible with the future agent-orchestration substrate** that unfreezes E-0019. If the substrate generalises into a "landscape over arbitrary work axes" pattern, this skill is an instance of that pattern.
...
- E-0020 — Add list verb. Provides one of the read verbs this skill consumes; ships skills-coverage policy that governs this skill's frontmatter.
