...
Bootstrap dependency: G-0051, G-0052, and G-0053 cannot be promoted to `addressed` until **M-0059** ships, because closing a gap as `addressed` requires `addressed_by` to be set, and M-0059 is the milestone that adds the `--by` flag to do that without hand-editing frontmatter. So:

- **Recommended:** ship **M-0059 first**. Cheapest path to unlock the gap-close chain. The other three milestones can then ship in any order.
- **Alternative:** ship **M-0056 first** for the largest day-to-day ergonomic win (`--body-file` is the highest-leverage flag), and only do M-0059 once you're ready to close the gaps. Acceptable if you don't mind the gaps staying `open` in the meantime.

After M-0059 ships, close the three gaps:
...
M-0057 (batched `--ac-title`) and M-0058 (`aiwf edit-body` + skill update) have no ordering constraints with the other milestones — pick by whichever is most useful next.

## Note on the meta-irony

This epic plans the fix for friction observed during E-0014's planning. By design, creating the epic itself triggers the same friction one more time — accepted as the cost of capturing the design rationale durably before the fix lands. After M-0056 ships, this epic's body and ACs become the first thing to use the new `--body-file` flag.
