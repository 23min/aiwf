---
id: M-0074
title: skill-coverage policy, judgment ADR, CLAUDE.md skills section, G-0061 closure
status: done
parent: E-0020
tdd: required
acs:
...
### AC-10 — G-0061 promoted to terminal status with closing commit citing this epic

### AC-11 — G-0085 doc-sweep and gap closure

Five sites (CLAUDE.md, three under `docs/pocv3/`, one gap body) advertise the non-existent `aiwf status --kind gap` command. M-0072 shipped the canonical replacement (`aiwf list --kind gap`); this AC sweeps the prose and closes the gap. Mechanical search-and-replace across:
...
After the doc edits land in the same commit (or commit pair) as the AC-9 CLAUDE.md edit, `aiwf promote G-085 <terminal>` closes the gap with a body citing E-0020 — same shape as AC-10's G-0061 closure.
...
- AC-10's closing commit is produced by `aiwf promote G-061 <terminal>`; the commit's `aiwf-entity:` trailer references G-0061 and the body cites this epic. Do not hand-craft the closure commit.
- AC-11's G-0085 closure follows the same pattern as AC-10: `aiwf promote G-085 <terminal>` after the five doc-sweep edits land. The doc sweep is mechanical (a single `aiwf status --kind gap` → `aiwf list --kind gap` substitution at each of the five sites); confirm each site renders correctly afterwards (especially `docs/pocv3/archive/gaps-pre-migration.md`, which is otherwise frozen content).
...
- G-0061 closure: `aiwf promote G-061 addressed` (or whichever terminal status `gap` uses; `entity.IsTerminal(KindGap, ...)` from M-0072's helper resolves it). The closing reason cites E-0020.
...
- G-0061 (status promotion only)
- G-0085 (status promotion only — AC-11)
...
- Closure of G-0068 (discoverability policy misses dynamic finding subcodes). Different policy, different fix shape.
- Any change to the existing `PolicyFindingCodesAreDiscoverable` or `PolicyConfigFieldsAreDiscoverable`.

## Dependencies

- M-0072 — the contract-skill drift fix must have landed so AC-5's "every `aiwf <verb>` resolves" check doesn't trip on stale `aiwf list contracts` mentions in `aiwf-contract/SKILL.md`.
- M-0073 — the new `aiwf-list` skill must exist so the policy's verb-coverage check passes (without it, `list` would have no skill and would need an allowlist entry, defeating the M-0073 work).
- E-0014 conventions for `internal/policies/` test wiring (`policies_test.go` runPolicy pattern).
...
- Plus `TestNoReintroducedDeadVerbForms_ContractsAndSkill` (the M-0072
  AC-8 sub-positional drift guard, scoped to the two M-0072 named files).

## References

- E-0020 epic spec (this milestone's parent).
- G-0061 — the gap this milestone closes.
- G-0085 — sibling drift case to G-0061: five sites advertise `aiwf status --kind gap`, a non-existent flag; M-0072 ships `aiwf list --kind gap` and AC-11 sweeps the prose. Closed by AC-11.
...
- **First-word verb resolution (AC-5).** The policy's mention-resolution
  ...
  G-0086 for the third file the test doesn't cover yet.
...
- **G-0087 placeholder allocation.** The allowlist's `show` entry was
...
- **AC-9 + AC-11 share their commit pass.** `CLAUDE.md` carries both
  the new *Skills policy* section (AC-9) and the swept G-0085 site at
  line 3 (AC-11) — folded into one diff because both touched the same
  file. The other four G-0085 sites and the AC-9 *What's enforced* row
...
`aiwf check` clean except 2 pre-existing warnings unrelated to E-0020.
...
- **ADR-0006 is `proposed`, not `accepted`.** The milestone spec
  ...
  ratification status.
- **G-0087 is the only allowlist entry whose rationale points at a gap.**
  The other 12 entries cite stable categories (ops verb, trivially-
  documented, etc.). When G-0087 closes (an `aiwf-show` skill ships),
  the allowlist's `show` entry is removed; that's the migration path
  the ADR's *Re-evaluation prompts* paragraph describes.
- **Dependency on M-0072 + M-0073 is real.** M-0072's contract-skill drift
  fix had to land first or this milestone's AC-5 check would fire on
  stale `aiwf list contracts` mentions. M-0073's `aiwf-list` skill had
  to land first or the AC-4 verb-coverage check would either need a
  `list` allowlist entry (defeating M-0073) or fire a violation. The
