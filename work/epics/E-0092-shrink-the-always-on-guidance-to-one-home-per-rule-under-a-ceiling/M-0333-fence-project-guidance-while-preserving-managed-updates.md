...
- Guidance prose changes here only where this milestone's own rules change what it states: the principle text, the evidence-rule statements, and the sentence naming the gates that judge commits. The gate lands on a file that still has everything else in it.
...
- Cutting, moving, or rewording guidance beyond the statements this milestone's rules change; that is M-0336, M-0335 and M-0337.
...
- A reference is a markdown link in any CommonMark form, or an `@` import in prose outside code; a path named any other way — in backticks, or in a sentence — is outside the ceiling's model. An import is followed only where Claude Code expands one: in its entry point and the files that imports, recursively. In any other document, and for Codex, the line is text. An import inside a managed block counts once, toward the aiwf-generated figure.
...
- The `CLAUDE.md` sentence naming the gates that judge commits rather than the working tree names the fence beside the provenance backstop and the co-author ban; it rides as its own trailered commit and is held at review.

## Validation

Environment: the aiwf devcontainer, Linux, `go1.25.11 linux/amd64`, at commit `327aeb53e` on `milestone/M-0333-fence-project-guidance-while-preserving-managed-updates`; the audit base is the epic branch's fork point from `main`, `a61f3d8de`.
...
- `go test -count=1 -run TestPolicy_GuidanceCeiling -v ./internal/policies/` — the AC-4 measure, in whitespace-separated words. Observed:
  - `claude-code: handwritten primed 9557 (ceiling 9557), aiwf-generated 2393, required reads [.guidance/project.md .guidance/index.md]`
  - `codex: handwritten primed 9686 (ceiling 9686), aiwf-generated 2430, required reads [.guidance/project.md .guidance/index.md CLAUDE.md]`
...
- The pins listed in `guidanceReaderList` are retired by the milestone that moves or deletes their passage: M-0335 AC-3 for a moved passage, M-0336 for a deleted one.

## Reviewer notes

- The read table's per-link cost for a host entry point's links was raised at review; the maintainer chose that split, and it stands.
- The reader rule does not treat package-level values as nodes of its graph. Doing so would let it see a constant set to another constant, a function held in a package variable and a climbing path held in a constant, which its doc comment records as unseen, at the price of matching more names by spelling; the recorded limits stand.
- The disposition block of `de177ca9f` names "or the cross-referenced design docs" as deleted, while only "cross-referenced" went. The fence checks a block's shape, not its accuracy, and the commit stands.
- `git_test_env_harden.go`, `section_absence_predicate.go` and `violation_policy_id_literal.go` read a package directory with their own loop beside `parsePackageDir`. Each parses a different file set one file at a time and needs no comments, so none repeats `parsePackageDir`'s job, and they stay as they are.
- Doc-lint over the documents this milestone changed found no broken reference, link or invocation. By its link heuristic ADR-0053 is an orphan; E-0092, M-0333 and M-0350 cite it by id.
- Verdict: the deciding review's defects are fixed, each pinned by a test that fails without its fix; its judgment findings are fixed or recorded above.
