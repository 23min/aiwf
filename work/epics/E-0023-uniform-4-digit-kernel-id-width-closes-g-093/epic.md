...
G-0093 is the discovery framing; ADR-0008 is the policy decision and `aiwf rewidth` verb shape. This epic is the implementation.
...
- **`aiwf rewidth` verb** per ADR-0008. `--apply` commits; default dry-run. Active-tree only; archives untouched. Idempotent. Single commit per `--apply` invocation, trailer `aiwf-verb: rewidth`. Cobra completion wiring per E-0014's drift test.
...
- [ ] `aiwf add epic --title "..."` allocates the next id at canonical 4-digit form (e.g., E-0023 if E-0022 was previous).
