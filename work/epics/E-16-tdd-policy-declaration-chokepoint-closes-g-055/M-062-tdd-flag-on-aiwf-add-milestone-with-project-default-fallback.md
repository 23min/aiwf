...
The change is verified by the discoverability policy test (`internal/policies/PolicyFindingCodesAreDiscoverable` and the broader skill-doc enumeration from G-021) which already enforces that every kernel surface is mentioned in at least one channel CLAUDE.md names.
