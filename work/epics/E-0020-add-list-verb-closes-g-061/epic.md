---
id: E-0020
title: Add list verb (closes G-0061)
status: done
---

# E-0020 — Add list verb (closes G-0061)
...
- Closure of G-0068 (discoverability policy misses dynamic finding subcodes). Different policy, different fix shape, kept out so this epic stays scoped.
...
- [M-0072](M-072-aiwf-list-verb-status-filter-helper-refactor-contract-skill-drift-fix.md) — `aiwf list` verb, status filter-helper refactor, contract-skill drift fix · `tdd: required` · depends on: —
- [M-0073](M-073-aiwf-list-skill-aiwf-status-skill-tightening.md) — New `aiwf-list` embedded skill; tighten `aiwf-status` description and body · `tdd: advisory` · depends on: M-0072
- [M-0074](M-074-skill-coverage-policy-judgment-adr-claude-md-skills-section-g-061-closure.md) — `internal/policies/skill_coverage.go`, judgment ADR, `CLAUDE.md` *Skills policy* section, follow-up gap for `aiwf-show` skill, G-0061 closure · `tdd: required` · depends on: M-0073

## ADRs produced (optional)

- [ADR-0006](../../../docs/adr/ADR-0006-skills-policy-per-verb-default-topical-multi-verb-when-concept-shaped-no-skill-when-help-suffices.md) — Skills policy: per-verb default, topical multi-verb when concept-shaped, no skill when --help suffices, discoverability priority justifies splitting within a topical group. Status: `proposed` at epic close (M-0074 AC-8).

## References

- G-0061 — *Generic `aiwf list <kind>` verb referenced as canonical in contracts plan and shipped contract skill, but never implemented* (`work/gaps/G-061-*.md`). Closed by this epic.
...
- E-0013 — *Status report*. Established `aiwf status` and the curated-narrative framing this epic clarifies.
- E-0014 — *Cobra and completion*. Established the `cmd.RegisterFlagCompletionFunc` pattern and the `cmd/aiwf/completion_drift_test.go` chokepoint that V1's `--kind` and `--status` completion wires through.
