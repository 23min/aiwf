...
- **CLI auto-completion-friendly.** Per CLAUDE.md, the new `--branch` flag (and any new closed-set values it accepts) wires through `RegisterFlagCompletionFunc`; the completion-drift test in `internal/cli/integration/completion_drift_test.go` catches missing wiring.
