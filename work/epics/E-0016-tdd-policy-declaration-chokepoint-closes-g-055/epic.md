...
- **Retroactive audit of historical milestones.** Existing milestones with no `tdd:` field stay grandfathered as `tdd: none`. The new check finding surfaces them for visibility but does not retroactively engage `acs-tdd-audit` against their already-met ACs. The grandfather rule is load-bearing for not lighting up E-0014's tree on upgrade.
- **`--tdd-reason` flag** for explaining `--tdd none` choices. Mentioned in G-0055 as a deferrable refinement; not in this epic.
