...
**Epic branch:** epic/E-21-recommended-sequence
**Merge commit:** `ca0d601` (epic/E-21 merged into main with `--no-ff` from the parent worktree at `/Users/peterbru/Projects/ai-workflow-v2`; auto-merge resolved `.gitignore` and `CLAUDE.md` cleanly, with the parent's mid-flight Q&A-tightening edit on `CLAUDE.md` and the AC-mechanical-evidence + cross-repo plugin testing additions both preserved; `STATUS.md` regenerated post-merge to reconcile divergence)

## Milestones delivered

- **M-078** — Planning-conversation skills design ADR (placement, tiering, name rationale) — merged via patch wrap commit `d447e0f`
- **M-079** — aiwfx-whiteboard skill: classification rubric, output template, Q&A gate — merged via patch wrap commit `d447e0f`
- **M-080** — Whiteboard skill fixture validation; retire critical-path.md; close E-21 — promoted in commit `89791e1`

## Summary

E-21 graduated the open-work synthesis pattern from a one-off `critical-path.md` snapshot into the reproducible `aiwfx-whiteboard` skill. The skill now ships in the rituals plugin, materialises into the marketplace cache via `aiwf init` / `aiwf update`, and answers natural-language direction questions (*"what should I work on next?"*, *"draw the whiteboard"*) by synthesising tree state into a tiered landscape, recommended sequence, first-decision fork, and Q&A-gated pending decisions. The original holding doc `work/epics/critical-path.md` is retired; the standing `unexpected-tree-file` warning that lived from 2026-05-08 to 2026-05-09 is gone. ADR-0007 captures the placement/tiering rationale (rituals-plugin placement; pure-skill first; kernel verb only when usage demands it). Three follow-up gaps (G-088, G-089→addressed, G-090) carry the loose ends forward.

The epic also surfaced two cross-cutting doctrines that landed in CLAUDE.md mid-flight: the *AC promotion requires mechanical evidence* rule (born from M-078's "wrapped without tests" episode and the user's *"everything should be tested, not assumed via conversation"* correction) and the *Cross-repo plugin testing* pattern (fixture in this repo as the canonical authoring location during the milestone; deploy step at wrap; drift-check test against the marketplace cache). Both rules are now codified and apply to every future plugin-side milestone.

## ADRs ratified

- *(none — `ADR-0007` was authored under M-078 and remains at status `proposed` per the spec's explicit deferral; ratification to `accepted` is a separate decision the operator may take post-wrap)*
...
- **G-088** — Kernel skill-coverage policy walks `internal/skills/embedded/` only; plugin skills are not policed by the kernel. Per-skill equivalent invariants must be re-applied in test code as M-079 did. *Open*; small milestone to expand kernel policy scope.
- **G-090** — `TestAiwfxWhiteboard_AC8_MaterialisationDriftCheck` has three branches not unit-tested (cache-root absent skip, plugin dir read error skip, fixture/cache drift FAIL); refactor lookup to take cache root as parameter for hermetic testing with synthetic temp dirs. *Open*; wf-patch.

G-089 (whiteboard gitignored cache + AC-6 anti-pattern revision) was *addressed* by patch `cbd3021`/`8f5b946` mid-epic.
...
- `docs/adr/ADR-0007-*.md` — new ADR; cross-references to ADR-0006, M-079, E-21, and CLAUDE.md sections all resolve.
...
- Three pending decisions in the live whiteboard output that warrant near-term attention:
  - First-decision fork: A (E-16 TDD chokepoint), B (E-19 parallel TDD subagents — needs milestones planned), C (verb-hygiene umbrella from ADR-0005).
  - Should the Tier-1 hygiene sweep (G-080, G-083, G-088, G-090) be one batched wf-patch or one per gap?
...
- G-088 / G-090 stay open as filed; trigger their work when the friction shows up or they bundle into a Tier-1 sweep.
