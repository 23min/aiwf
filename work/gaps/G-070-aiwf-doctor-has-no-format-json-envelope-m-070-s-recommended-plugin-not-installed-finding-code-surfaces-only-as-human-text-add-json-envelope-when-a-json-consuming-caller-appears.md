---
id: G-070
title: aiwf doctor has no --format=json envelope; M-070's recommended-plugin-not-installed finding-code surfaces only as human text. Add JSON envelope when a JSON-consuming caller appears
status: open
discovered_in: M-070
---
...
A `--format=json` flag on `aiwf doctor` that emits the kernel's standard JSON envelope (`{ tool, version, status, findings, result, metadata }` per `CLAUDE.md`'s CLI conventions). M-070's AC-3 spec explicitly references the envelope shape:
...
Deferred (not blocking) because there's no JSON-consuming caller today — the forcing function hasn't appeared. Filed as a gap so it surfaces on `aiwf status` when it does. When implemented, M-070's AC-3 contract is automatically satisfied without spec changes.
