...
the whole path, so nine documents outside the deliberately-exempt tiers are
silently absent from link-check.
...
Eight of the nine are Normative tier, five of them accepted ADRs. The entries
whose intent is a directory — `docs/explorations`, `docs/research`,
`docs/archive`, `internal/check/testdata` — happen to behave as intended because
no other path contains those substrings, so the list looks correct at a glance
and is not.
...
None of the nine carries a `docs/`-to-`work/` link today, so no link is currently
...
Anchoring is the obvious fix and is not free. `"work"` → `"^work/"` and `".git"`
→ `"^\.git/"` restores the nine to the checked set, and any broken link they
already carry becomes a new failure in a workflow that is red already — so the
change wants to land with, or after, the red being cleared. The alternative is to
leave the entries as they are and record that these files are outside link
discipline, which at least stops the list reading as something it is not.
...
- `internal/policies/m0317_docs_link_coverage_test.go` — `firstMatch` models this
  matching so the coverage guard sees what lychee sees; it follows the config
