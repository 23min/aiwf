...
- **Combined with the G-0056-class pollution** (a render run on a stale init), `git status` becomes unreadable — burying real changes.
...
Once shipped, promote G-0057 to `addressed` with the implementing commit as `--by-commit`.
...
- [G-0056](G-056-aiwf-render-output-site-is-not-gitignored-pollutes-consumer-working-tree.md) — same class of defect (framework-produced artifact without gitignore coverage); html out_dir reconciliation shipped in `056139d`.
