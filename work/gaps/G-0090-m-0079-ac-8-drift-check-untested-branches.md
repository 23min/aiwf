...
3. **Fixture↔cache drift → FAIL.** The test's *load-bearing* assertion. When the cache contains a SKILL.md that differs from the in-repo fixture, the test must fail with a clear drift message. This branch was exercised manually during M-0079 deploy iteration (cache → mutate fixture → re-run shows drift); no unit test pins it.
...
- **Adding a CI-side drift check that doesn't require local plugin install.** M-0079's reviewer notes flagged this as a separate friction point in cross-repo deploy ergonomics. Could combine well with this gap's refactor but isn't strictly required to close it.

## References

- **M-0079** work log AC-8 — the cycle where the test was added.
- **M-0079** Deferrals section — explicit deferral of branch-coverage hardening; this gap is the deferred work item.
