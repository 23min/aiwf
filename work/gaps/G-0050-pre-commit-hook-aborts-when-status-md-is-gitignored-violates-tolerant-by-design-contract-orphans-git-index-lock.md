...
- Downstream report: flowtime-vnext consumer repo, 2026-05-05; commit `7e9cc97` ("stop tracking generated STATUS.md") gitignored the path; symptom thread covered M-0066 wrap and E-0026 epic creation.
- Hook header contract: the "tolerant by design — never blocks commits" promise on the kernel's pre-commit template.
- Adjacent: G-0045 (hook chaining via `.local` siblings) — same hook surface, different concern.
