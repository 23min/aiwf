...
Concrete: trunk has G-0001 at `open`. A feature branch makes a manual commit changing G-0001 to `wontfix`. Operator merges feature → trunk via `git merge --no-ff feature`. The merge commit's message lacks aiwf trailers; its `--name-only` is empty. The audit pass on trunk says nothing — even though the audit-trail hole is real (no `aiwf-verb:` trailer ever recorded G-0001's transition).
