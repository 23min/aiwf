...
## Friction evidence (2026-06-12)

Two operator incident reports landed, both squarely inside the fix shape above. Recorded against this gap's own activation criterion — "awaits its own epic when the friction is paid for":

1. **Dangling-on-cancel rot went six days unnoticed.** A planning triage cancelled several epics and their milestones via `aiwf cancel`. Five still-active draft milestones kept `depends_on` edges pointing at the cancelled milestones; `aiwf check` reported ok — no findings — and the rot sat until a manual frontmatter walk surfaced it. This is exactly the case the "Dangling-on-cancel detection" follow-on predicts. Severity delta from the original sketch: the report argues for **error**, not warning, when the *dependent* is non-terminal — the edge is unsatisfiable (the dependent can never legally start) or silently means nothing, and the planning surface lies either way; positive terminals (`done`) stay clean (dependency satisfied). Proposed hint text: retarget via `aiwf milestone depends-on <id> --on ...` or cancel the dependent milestone.

2. **The reserved cross-kind verb shape was guessed at and silently missing.** A re-plan produced three sequenced epics (foundations → robustness → operator surface). The sequencing exists only as body prose ("Depends on <epic>"), unenforced and invisible to status/render/check. The operator's triage script ran `aiwf epic depends-on <E-id> --on <E-id>` — the exact kind-prefixed shape M-0076 reserved — assuming the verb existed; it doesn't, so the planned edges silently never landed (the script didn't check exit codes). The workaround (cross-epic milestone edges encoding the order transitively) works but buries program-level structure at the wrong altitude. The report's asks map one-to-one onto the fix shape: optional `depends_on` (multi → epic) on the epic schema; `aiwf epic depends-on <E-id> --on <E-id,...>` mirroring the milestone verb (replace-not-append, `--clear`); status/render surfacing (e.g. `[proposed, after <epic>]`); the dangling-on-cancel rule applied to epics; and optionally a warning when an epic goes `active` while a `depends_on` epic isn't `done` (sovereign-overridable).

The prose-only fallback has now bitten in the wild, in script form — silently, which is the worst shape. The "friction paid for" condition in the Decision section above is met; the next planning pass should consider filing the implementation epic.

## Narrow slice extracted; cross-kind generalization stays deferred (2026-07-22)
