...
- Optionally accepts `--reason "..."` to land in the commit body for audit narratives like the M-066 rescope.
...
- **G-063's sub-decision #4 is currently impossible to enforce truthfully.** When `entity-body-empty` lands as a kernel-enforced finding, the operator who rescopes a milestone (like M-066) cannot update its title to match. The milestone passes the body-empty check (its body is non-empty) but the title field is stale, and there's no rule that catches stale titles. That's a quality gap a `aiwf retitle` verb would close.
- **AI-discoverability:** when a future AI assistant reads the rescoped M-066, the Goal section says one thing and the title says another. The skill ecosystem cannot reconcile that without a kernel-supported correction path.

### Predecessor / related references

- Triggering instance: M-066 rescope (2026-05-07), recorded in that milestone's body under "## Rescope note (per G-063)".
- Sibling: G-063 (open) — sub-decision #4 governs the rescope; its in-flight work is what surfaced this gap.
- Sibling: G-058 (open) — AC body chokepoint, becomes generalized via the M-066 rescope; uncovered the missing retitle path.
- Adjacent: G-061 (open) — `aiwf list <kind>` verb missing. Similar shape ("framework documents/assumes a verb that doesn't exist"); could be addressed under the same epic that adds verb-surface improvements.
