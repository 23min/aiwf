...
Observed concretely: in a working session, three gap closures (G-0021, G-0030, G-0031 in a separate consumer tree) were committed manually after `aiwf cancel` failed on `.git/index.lock` contention each time. The frontmatter reflects `wontfix`, but `aiwf history` returns "no history" for all three.
