...
Reconfirmed by the 2026-06-04 codebase health scorecard (D3 verdict: Weak; see `docs/pocv3/health-scorecard-2026-06-04.md`). The scorecard ranked the diff-coverage chokepoint as the single highest-leverage fix in the entire scorecard — it gates every future TDD discipline claim and is the prerequisite for trusting `tdd: required` as a guarantee.

Candidate mechanisms (rough, ordered cheap to invasive):

- **Diff-coverage CI gate + branch-coverage-audit policy test** (scorecard's recommended path, 2026-06-04): wire a diff-coverage tool into CI (`go-test-coverage` with a per-file or diff-scoped threshold, or `diff-cover --fail-under=80 --compare-branch=origin/main coverage.out`) so any PR/push whose changed lines+branches fall below the floor fails the `test-race` job; pair with `internal/policies/branch_coverage_audit_test.go` that walks Go source under specified packages, enumerates conditional branches in functions whose surrounding lines changed since `origin/main`, and asserts each is either exercised by a test or annotated `//coverage:ignore <reason>`. Strictly stronger than the subagent option below because (a) it doesn't depend on a subagent invocation that the operator could skip, and (b) it lives at the same CI chokepoint as race/lint/build. The policy test is the kernel-side companion to the CI gate — both must agree before a milestone can wrap.
- **Branch-coverage subagent**: `wf-tdd-cycle`'s audit step delegates to a fresh subagent with only the diff and the AC contract. Subagent enumerates branches, points at tests, returns a list. Bounded scope, fresh eyes, no investment in the impl just written. Fastest path; addresses the specific M-0066/AC-1 failure but still LLM-dependent. Worth keeping as a development-loop signal even after the CI gate lands.
