...
85 test sites across 12 packages still write an executable stand-in with a bare `os.WriteFile(path, …, 0o755)` instead of `testsupport.WriteExecutable`, leaving them exposed to the `ETXTBSY` race G-0491 measured and fixed.
...
Counted 2026-08-26 with the `test-executable-write` detector's own predicate rather than by grep, so string literals that merely look like calls are excluded:
...
Several write something the suite then really runs — git hooks, a `git` wrapper placed on `PATH`, statusline scripts — so the exposure is not theoretical.
...
Three packages have already been bitten and are fixed: `internal/stresstest`, where the race was first measured at roughly one full tagged-package run in four; `internal/contractverify`, which failed during the verification run for the fix itself; and `internal/policies`, which fired repeatedly against one hook-test stand-in (G-0562, G-0570, G-0578) before G-0641 swept the rest of the package. Nothing distinguished either of the later two from the packages in the table above beforehand — each was on exactly this list until the day it failed. That is the reason to treat the remainder as unswept rather than as safe.
