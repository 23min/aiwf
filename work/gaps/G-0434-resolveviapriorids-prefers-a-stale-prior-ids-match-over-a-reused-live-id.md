...
**Caveat for the implementer.** `resolveViaPriorIDs`'s own doc comment (`internal/check/provenance.go:363-367`) says the prior-ids preference is deliberate for the G-0118 parallel-allocation-collision case: when `ByID(id)` and `ByPriorID(id)` disagree because two branches independently allocated the same id and one side was reallocated forward, the function needs to reach the *renumbered-forward* entity, not whichever entity a plain `ByID` happens to return. That scenario produces the same observable shape as this gap's id-reuse case (`ByID` and `ByPriorID` disagree) — so a blind "prefer `ByID` whenever it resolves" swap risks silently reintroducing G-0118's original failure while fixing this one. The two cases likely need to be distinguished by *when* the commit being resolved was authored relative to the reallocate event (before it → wants forward resolution via prior_ids; after it → wants the live id directly), not by tree shape alone. Confirm the fix doesn't regress G-0118's fixture before landing.