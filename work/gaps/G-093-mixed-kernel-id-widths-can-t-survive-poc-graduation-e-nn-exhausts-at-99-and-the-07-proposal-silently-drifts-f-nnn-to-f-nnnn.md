...
2. **`aiwf rewidth` verb for migration** — distributed with the kernel binary, idempotent, runs on-demand per consumer. Renames active-tree files to canonical width and rewrites in-body references in a single commit. This repo runs it as one of N consumers; downstream consumers (already adopted aiwf with narrow-width trees) run it post-upgrade when they're ready. New consumers post-graduation are born canonical and never need it.
3. **Drift-check rule** — `aiwf check` rule `entity-id-narrow-width` uses tree-state-based detection: silent on uniform-narrow active tree (pre-migration legacy); silent on uniform-canonical active tree (post-migration clean); warns on narrow files only when the active tree is mixed (some canonical alongside some narrow — "you've started; finish it"). Archive entries grandfathered per ADR-0004's forget-by-default; archive width never feeds the rule's mixed-state computation.
