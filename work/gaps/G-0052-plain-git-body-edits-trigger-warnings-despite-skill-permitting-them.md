...
Planning E-0014 produced one body-content commit (`c079801` on `poc/aiwf-v3`) that touched 8 entity files via plain `git commit` per the skill's permission. `aiwf check` immediately reported 8 `provenance-untrailered-entity-commit` warnings. The user (the intended audience for both the skill and the check) experienced this as the framework punishing them for following its own instructions.
...
1. **Add `--body-file` on `aiwf add` variants** so creation-time body content rides along with the create commit (also resolves G-0051's body-edit hop).
...
## Relationship to G-0051

These should be solved together. The `--body-file` flag is a single change that resolves both:
- Saves the extra body-content commit (G-0051).
- Eliminates the untrailered commit that triggers the warnings (G-0052).
