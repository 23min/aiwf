...
`aiwf archive` relocates entities into their per-kind `archive/` subdirectory and rewrites no links at all. `aiwf rename` and `aiwf retitle` do rewrite, through `planLinkRewriteWrites`, but that walk iterates the loaded tree's entities and skips archived paths — it never reads `docs/`. So a link written from a doc to an entity file has no maintainer.
...
**Prevention** is widening the rewrite: extend the walk past the entity tree so a move repairs `docs/` too, and call it from `archive`, which today rewrites nothing. This is the larger change, and on its own it still leaves hand-authored mistakes unreported.
...
- `internal/verb/archive.go` — invokes no rewrite today.
