...
A caller running `update` or `upgrade` on a pseudo-terminal with nobody behind it — a devcontainer lifecycle command, as G-0446 observed for `init`, or a script run under `script(1)` — hangs indefinitely, and every mutating `aiwf` verb on the repository refuses until the process is killed. The hang follows `aiwf update: done.`, so a caller watching for completion is told the run finished. The only escape is an undocumented stdin redirect, so a caller that knows no human is present has no supported way to say so.
