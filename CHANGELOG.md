...
- **M-0135 — `aiwf doctor` containerized-env awareness.** New `InContainer()` probe in `internal/cli/doctor/env.go` checks `/.dockerenv` + `AIWF_DEVCONTAINER` and emits an `env:` line on doctor output. When in container, a `shadowMountStatus(home)` probe inspects `<home>/.claude/plugins/` and emits a `plugin-index-mount:` line reporting `ok (N plugin entries cached)` / `empty` / `missing` (100+ cap on the count). Both lines are read-only — never increment doctor's problem count.
