...
A live snapshot of the entity tree (in flight, roadmap, open decisions, open gaps, warnings, recent activity) is generated by `aiwf status --format=md` and committed at [`STATUS.md`](STATUS.md). Run `make hooks` once after cloning to install a pre-commit hook that regenerates it on every commit; without aiwf on `$PATH` the hook is a no-op.
