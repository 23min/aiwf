...
- **Kernel functionality must be AI-discoverable.** Every verb, flag, JSON field, body-section name, finding code, trailer key, and YAML field is reachable via `aiwf <verb> --help`, embedded skills under the selected host’s `.claude/skills/aiwf-*` or `.agents/skills/aiwf-*`, or the cross-referenced design docs. If an AI must grep source to learn a capability, it's undocumented — ship `--help` + skill docs alongside the implementation.
...
## Writing docs, entity bodies, and code comments — state the conclusion, not the drafting history

**State the conclusion, not the drafting history.** Initiative docs, ADR / gap / decision bodies, and milestone specs record the current design, not how the text got there. Don't narrate a document's own revision history inline — no *"this session added…"*, *"an earlier draft claimed X, that's wrong"*, *"as of this session"*, *"in a later pass"*. If a corrected assumption explains a non-obvious aspect of the current design, keep the *reasoning* (why X holds, why Y isn't automatic) and state it as direct exposition — drop the draft-history framing around it. Provenance (when, from what conversation, superseding what) belongs in one clearly-labeled section (`## Provenance`, an ADR's date/decided-by header) — not scattered through the substantive prose as asides. **Code comments are held to the same bar**: a guard is documented by what it guarantees, not by the defect that motivated it — no *"used to"*, *"the bug this replaced"*, *"there was a third arm here"*, *"this has drifted twice"*. Most such defects existed only between review rounds and never shipped; `git blame`, the ADR, and the tracked gap already own that history, and the comment copy is the one no test pins. The single exception is a past state a reader can still **encounter** — a legacy on-disk format, a supported older release, an external contract — which is current truth about the input space and reads in the present tense as such. Judgment call, not mechanically checkable (`wf-doc-lint` is grep-based); caught at review the same way prose coherence and design soundness already are.
...
- **Normative** — current-truth, kept in lockstep with the code: `docs/adr/` (terminal-status ADRs move to `docs/adr/archive/` under the same forget-by-default convention as the Archival tier below), `docs/design/`, `docs/reference/` (generated references), `docs/architecture.md`, `docs/overview.md`, `docs/workflows.md`, `docs/skill-author-guide.md`.
- **Forward-looking** — captured ideas awaiting promotion to a real epic/gap entity: `docs/initiatives/` (an initiative whose every thread has been promoted to a tracked entity moves to `docs/initiatives/archive/` with `status: realized`, under the same forget-by-default convention as the Archival tier below — promotion, not closure of every descendant, is the bar).
...
- **Archival** — frozen historical snapshot per ADR-0004; cross-references inside it are not fixed when their targets move (same forget-by-default convention as `CHANGELOG.md`'s append-only exemption): `docs/archive/` (including `docs/archive/pocv3/`, and `docs/archive/migration/`, entry point [`from-prior-systems.md`](docs/archive/migration/from-prior-systems.md) — it specifies the `aiwf import` manifest contract, and that verb still ships, so a reader can reach the surface it documents).
...
2. **Stable ids that survive rename, cancel, and collision.** Every kernel id emits at a uniform canonical 4-digit width (ADR-0008); parsers tolerate narrower legacy widths on input, while renderers and allocators always emit canonical width. That tolerance is permanent: it is what keeps live cross-references into entities archived before the width convention resolving, since no verb widens an id in place. A narrow id in the *active* tree is a defect, reported at error severity by `entity-id-narrow-width`. The id is the primary key, the slug is display; rename preserves the id; "removal" means a terminal status, not deletion; collisions are detected by `aiwf check` and resolved by `aiwf reallocate`.
...
5. **Selected-host framework artifacts, refreshed explicitly.** `aiwf init` / `aiwf update` select Claude Code and/or Codex from an explicit `hosts` list or executable detection on PATH; `hosts: []` selects none. Claude uses `.claude/` and a managed guidance import in `CLAUDE.md`; Codex uses `.agents/` and native managed guidance in `AGENTS.md`. Root wiring defaults on, with `guidance.wire_claudemd` and `guidance.wire_agentsmd` opt-outs. Preserve user content and retained unselected artifacts. Core Git hooks remain host-independent. See [host setup](README.md#2-host-setup-and-embedded-rituals).
...
7. **Every mutating verb produces exactly one git commit, or none** — per-mutation atomicity for free. A failed mutation aborts before the commit; a request already satisfied converges to a NoOp at exit 0 and writes none (ADR-0036, and §"Same-state convergence" below). The invariant the oracles check is *one mutation ⇒ exactly one commit; a NoOp ⇒ zero*, with `metadata.commit_sha` present only in the first case.
...
Deliberately excluded (each addable later when real friction demands it): an events.jsonl / append-only event log, a graph-projection or hash-chain file, a cross-branch monotonic ID counter, a module/capability registry, a third-party skill registry, tombstones beyond terminal statuses, CRDT primitives / custom merge drivers / server-side hooks, GitHub-Issues or Linear sync, full FSM-as-YAML. If you reach for any of these, check design-decisions.md §"What's deliberately not in the PoC" — there's almost certainly a simpler way.
...
The profile-driven gates — the diff-scoped coverage audit chief among them — run inside `make ci` off the profile `test-cov` just built, so they cost seconds rather than the second instrumented suite run `make coverage-gate` pays for. That second run costs the full suite on a changed tree, and on Darwin costs it every time: `TEST_EXEC` names the Darwin-signing wrapper there and is empty elsewhere, and `-exec` with a program is outside the cacheable flag set `go help test` defines — the coverage flags themselves are all cacheable, replayed profile included. Gating off the profile `test-cov` already wrote is what keeps the marginal cost at seconds on either host, rather than at the mercy of cache state. The gates diff the **working tree** against the base, not `HEAD`, which is what lets `make ci` gate a change before the commit that carries it. The exceptions judge commits in the range, since their subject is a trailer or a message an uncommitted edit does not carry: the skill-edit provenance backstop, the co-author ban and the guidance fence.
...
- **Hermetic scenarios run on every push**, untagged, inside the ordinary `go test ./...`. The workflow-legality group is what the harness exists for and is the composition coverage G-0121 asks for: `verb-sequence` (a seeded random walk of `aiwf promote` across all six kinds, asserting the FSM's legality verdict is honored and that `aiwf check` reports no error-severity finding and no warning outside a curated baseline — an absolute allowlist judging each state on its own terms, not a comparison against the prior step) plus `archive-during-active-scope`, `force-override-durability`, `head-drift`, and `promote-on-wrong-branch-detection`. Six more qualify on the same test and stay untagged despite their subject matter: `disk-fault` (permission-denied write, no timing at all), `parallel-branch-reallocate` and `cross-worktree-edit-body-race` (collisions deterministic by construction — two clones computing the same `max(id)+1`, two sequential commits to one body field), `reachability-isolation` (a byte-identical `aiwf check` outcome either side of an invisible sibling commit), and `concurrent-id-allocation` and `concurrent-move`, which race real subprocesses but between them judge only load-invariant facts: refusals carrying repolock's busy code, a commit count matching exactly the actors that succeeded, at least one actor through, and — each in its own scenario — no id allocated twice and every milestone reported moved really landed under the target epic. Category name is not the criterion; oracle shape is.
- **Scenarios that drive real, concurrently-scheduled subprocesses carry `//go:build stress`** and run only via `make stress-tests` (or `go test -tags stress ./internal/stresstest/ ./cmd/stresstest/`). Five qualify: `concurrent-milestone-race`, `concurrent-writer-at-scale`, `cross-worktree-id-race`, `mid-write-kill`, and `lock-kill`. The reason is cost, not correctness: sharing a runner with `go test ./...` roughly doubles their wall time. All five judge load-invariant facts, by different means. `concurrent-milestone-race` excuses the documented `repo-lock-busy` refusal an actor takes when another holds the lock, bounds each transition at at-most-one, and asks only that some actor got through. `concurrent-writer-at-scale` retries past that refusal instead of excusing it. `cross-worktree-id-race` races two worktrees, which hold two separate lockfiles, so lock contention does not arise between them. `mid-write-kill` and `lock-kill` bound their observation on the watched process rather than a clock, so slowness delays their verdict instead of falsifying it. In `cmd/stresstest`, the whole-catalog runner tests (`TestRunRun_ScenarioAll_*`) carry the tag because `--scenario all` executes this class, and so does the lock-kill runner test, which drives a real lock-holding subprocess; `--scenario all`'s *selection* claim stays pinned untagged by `TestResolveScenarios_All_NamesEveryCatalogEntryInOrder`.

Adding a scenario means choosing its lane, and since no oracle now measures the machine that choice is a cost decision rather than a correctness one: a scenario goes tagged when it drives real subprocesses and would slow every push, untagged otherwise. An oracle that asserts how *many* actors get through, or how fast, is the thing to refuse at review — that is the runner's throughput, not aiwf's behavior (G-0468).

The tag applies per file, so a decision test sharing a file with a scenario driver leaves the every-push path with it — silently, since it still passes whenever anyone runs it. `PolicyStressLaneCensus` (`internal/policies/stress_lane_census.go`) is the chokepoint across both stress packages: a `stress`-tagged test that starts no process, waits on no clock and runs no goroutine fails the policy suite, naming the test to move. Driving `RunScenario` / `RunRepeated` / `Setup` counts only alongside a real scenario constructor, since all three take a `Scenario` and this package's own hermetic tests hand them fakes. Split the file rather than tagging around it.

```bash
make stress                          # run the whole catalog, 5 attempts per scenario by default (override: STRESS_REPEAT=N make stress)
make stress-tests                    # run the `stress`-tagged Go tests: the real-subprocess scenario drivers + the whole-catalog runner tests
...
- Materialized host artifacts — regenerate through `aiwf init`/`update`; see the [host setup guide](README.md#2-host-setup-and-embedded-rituals) for selected-host paths and managed root guidance.
...
When a ritual (`wf-patch`, milestone, epic) or an ad-hoc fix creates a branch, default to an in-repo worktree (`.claude/worktrees/<branch>/`, per ADR-0023) rather than switching the main checkout in place — create it with `aiwf worktree add <branch> --base <base>`, which materializes the selected hosts’ supported artifacts into the new worktree atomically, in one step. This applies to the session's own direct work, not just subagent dispatch. Skip only when explicitly told to work in the main checkout for that invocation.
...
Rituals (`aiwfx-*` / `wf-*` skills, agents, templates) are **authored directly** at `internal/skills/embedded-rituals/plugins/<plugin>/skills/<skill>/SKILL.md`, embedded via `go:embed`, and materialized by `aiwf init` / `aiwf update` into the [selected hosts’ supported paths](README.md#2-host-setup-and-embedded-rituals) (ADR-0014). A ritual edit is one commit here — no cross-repo coordination. The upstream `23min/ai-workflow-rituals` repo is archived (ADR-0016); the embedded snapshot IS the single source of truth. When a milestone's deliverable is ritual content, the authoring location is the embedded snapshot itself, and AC tests under `internal/policies/` assert against the embedded bytes via path constants.

**Every embedded-rituals `SKILL.md` edit must ride a commit that names the entity it belongs to.** This is mechanical, not vigilance: the `skill-edit-provenance-backstop` policy fails the profile-driven gate step when a commit adds or modifies a `SKILL.md` under `internal/skills/embedded-rituals/**` carrying no `aiwf-entity:` trailer, or carrying one whose value resolves to no entity in the tree. Commit such an edit with `--trailer "aiwf-entity: <id>"` naming the epic, milestone, gap, or decision that owns it. The gate is diff-scoped over the commits between the base ref and HEAD — an uncommitted edit carries no provenance yet, so it is not judged — and it runs in `make ci` as well as CI (the property is an aiwf-repo invariant, meaningless in a consumer tree).

`aiwf-entity` is the whole of the requirement. No `aiwf-verb:` is asked for, because a ritual `SKILL.md` is source rather than an entity file and no aiwf verb commits it — the closed set `trailer-verb-unknown` enforces carries no value meaning "I edited a shipped surface", so requiring one would mandate exactly the fabricated trailer G-0150 closed. No `aiwf-actor:` is asked for either: where no verb ran, "who ran the verb" is undefined — an edit directed in conversation has the human as principal and the assistant as a tool, so there is no separate actor to record. The owning entity's *status* is not consulted, only its existence — see D-0071 (`aiwf show D-0071`) for why keying on status would rot, and D-0070 for why content correctness in shipped prose is held at review instead.
...
- **"How to OPERATE aiwf in any repo"** (gate-per-mutation, reallocate-not-`git mv`, AC-mechanical-evidence, one-decision-at-a-time, never-pause, `body-prose-id`, cross-branch id allocation) is consumer-facing and **ships**: the always-on subset lives in the embedded guidance source (`internal/skills/embedded-guidance/aiwf-guidance.md`, composed with host-specific fragments and delivered through Claude's guidance import or Codex's managed `AGENTS.md` block); lower-frequency detail routes to on-demand skills. Because this repo dogfoods the same materialized guidance, an operating rule placed in the embedded source is followed here *and* shipped — one source, no fork. Placed in this `CLAUDE.md` instead, it forks.
...
- **Substring assertions are not structural assertions.** A grep for a literal proves it exists *somewhere*, not in the right *place*. Parse HTML (`x/net/html`) / walk the heading hierarchy and assert presence inside the named section/attribute; for multi-section pages name *which* section. Use a structural assertion whenever the literal is short/generic. **Over a shipped surface the answer is not to scope the substring but to drop it** — D-0070 retires prose- and heading-presence assertions across the embedded skill, ritual, template, agent-card, guidance, and statusline trees, because scoping a phrase still pins a reading that rewording breaks and nothing catches. What survives there is the relationship check, comparing two artefacts so either one moving goes red: a citation resolved against the document it names, or an expectation derived by running the code. Enforced by `shipped-prose-assertion` (`internal/policies/shipped_prose_assertion.go`), whose two exemption maps carry the trigger phrases that drive skill dispatch and the derived-expectation checks. Development guidance is held by D-0091, as E-0092 extends it from `CLAUDE.md`: every test that reads `CLAUDE.md`, `AGENTS.md`, the project router or a document it routes to from the repository root is listed in `guidanceReaderList` as a pin to retire, a relationship check or an absence check, and `guidance-readers` fails on a reader the list does not name. Everywhere else — the design docs, generated output — this rule stands unchanged.
...
- **`testsupport.WriteExecutable` for every executable stand-in**, never a bare `os.WriteFile` with an exec bit. A plain write holds a writable descriptor the whole time it is open; a concurrent fork inherits it, and `execve` on a file held open for writing fails with `ETXTBSY`, so the fixture step fails and the property under test is never evaluated (G-0491). The helper holds `syscall.ForkLock` across the write, which excludes exactly those forks. Writing to a temp name and renaming does *not* help — `ETXTBSY` is enforced against the inode, which the rename carries along. Chokepoint: `internal/policies/test_executable_write.go` (diff-scoped); `//exec:ok <reason>` when the mode itself is the subject.
...
**No `Co-Authored-By:` line names a non-human agent** (D-0096). The agent that ran a verb is recorded in `aiwf-actor:`, and the principal it acts for is the human; a co-author line is a second copy of that fact which nothing re-derives, and it contradicts the provenance model's reading of an LLM directed in conversation — a tool, not a co-author. This **overrides an assistant harness's own attribution default**: where a harness instructs the session to append such a line, this rule wins and the line is omitted. A human co-author is unaffected. Enforced rather than remembered: `provenance.refuse_coauthors` in `aiwf.yaml` lists the addresses, the `commit-msg` hook refuses one at composition, and `coauthor-trailer-ban` (`internal/policies/`) judges the same property over the pushed range for the cases the hook cannot reach — an uninstalled hook, `--no-verify`, or a hook shelling a release older than the refusal. The knob is absent by default, so a consumer repo is unaffected until it names an address.

### Release process

Releases are git tags `vX.Y.Z` on `main`. Before tagging, in one `release(aiwf): vX.Y.Z` commit edit [`CHANGELOG.md`](CHANGELOG.md): rename `## [Unreleased]` to `## [X.Y.Z] — YYYY-MM-DD`, add a fresh empty `## [Unreleased]` above it, verify the moved entries summarize the user-visible delta. Then push the commit, `git tag vX.Y.Z`, `git push origin vX.Y.Z`. The [`changelog-check.yml`](.github/workflows/changelog-check.yml) workflow fails a `v*` tag on two counts. Its first job wants a matching `## [X.Y.Z]` heading — even pure-mechanical patches need a one-line entry. Its second runs `make changelog-audit`, which fails the tag when a change to the embedded content since the last release is cited by no entity id in the release's own notes; a milestone's delta is cited by its parent epic, never by its own id, and Go under those trees is the materializer rather than shipped content and owes nothing. The audit works out which section is current from the changelog rather than from a tag, so it reads the same one either side of the tag push: `[Unreleased]` while the topmost version heading is the release it measures from, and that newer heading once the release commit has moved the entries into it. Run `make changelog-audit` before tagging rather than meeting it at the push — `AIWF_CHANGELOG_BASE=<ref> make changelog-audit` audits a past range. It also reports, without failing, any shipped-surface commit carrying no `aiwf-entity` trailer: with no entity named the audit cannot tell whether an entry covers it (D-0087).
...
- **A NoOp claims the verb's effect already holds, so every surface that effect spans must hold.** The baseline is the **loaded tree**: `tree.Load` reads the working tree, so the loaded entity is what every verb and every check rule sees, and a structured-state verb comparing it is comparing current state rather than a stale projection. (That the loaded tree is the *baseline* does not mean `aiwf check` validates every property of it — a `parent:` disagreeing with the file's directory, a title carrying a newline, and a drifted canonical H1 all pass at 0 errors. Being the baseline and being validated are separate claims.) A verb whose effect spans more than one surface checks each: `retitle` compares the stored title *and* the body H1, so a drifted heading still gets repaired rather than converged over; `move` compares the `parent:` field *and* the file's location under the epic's directory; a `--superseded-by` re-run does not converge while the reciprocal back-link is still missing. `edit-body --body-file` additionally compares HEAD, because it is a *commit-this-content* verb whose supported input includes "the content is already on disk" — a disk match alone would not prove the content is committed. That HEAD conjunct is verb-specific reasoning rather than a rule to copy into every guard — but not because the converging path is safe. Measured, a loaded-only comparison against HEAD-divergent frontmatter fails in both directions: asking for the value already on the dirty disk reports "already set; nothing to change" while HEAD says otherwise, dropping the operator's mutation behind a message that is false about the record; and asking for HEAD's value commits a tree byte-identical to its parent. Whether a verb should run at all against uncommitted frontmatter is a **precondition** question rather than a convergence one, and it belongs at one shared seam ahead of the guards instead of duplicated into each — where that seam sits relative to the same-state comparison decides whether any conjunct is needed at all. That seam is two seams, because one placement cannot cover both cases ([ADR-0038](docs/adr/ADR-0038-refuse-verb-writes-over-head-divergent-entity-content.md)): a commit-side guard at `verb.Apply` covers every path a plan carries, including nested entities no verb named, and a claim-side guard in each verb's prelude covers the converging case, which returns before a plan exists and so never reaches `Apply`. `PolicyClaimGuardPresence` fails CI on a converging route that reaches neither, and `noOpClaimScopes` records what each verb's claim is scoped to. The multi-entity sweeps decline per candidate rather than refusing per verb, so one mid-edit file costs its own candidate and not the sweep.
...
**Shipped surfaces cite no real entity id, filesystem path, or inline lifecycle status, and carry no development history, provenance narrative, or rationale/war-story — only imperative, consumer-scoped instruction.** "Shipped surface" is the full set the id chokepoint scans, not just `SKILL.md` bodies: those bodies *and* their `description:` frontmatter, entity templates, role-agent cards, the always-on guidance fragment, and the statusline's `#` comments. Every one materializes into consumer repos (via `aiwf init` / `aiwf update`), where aiwf's own ids are meaningless and rot as entities change status or move to archive — and where a note about *this* repo's development history, or an argued rationale for a past choice, is context-free noise. Illustrative content uses canonical `<prefix>-NNNN` placeholders (the letter-N form) or shape-descriptions; the one carve-out is a markdown link to a design/ADR doc (id rides in the destination, visible text stays descriptive). Provenance, history, and rationale belong in this `CLAUDE.md`, the design docs, and commit trailers — not in a consumer-facing surface. Chokepoint: the `skill-body-id` check (`internal/check`) fires pre-push over every `*.md` under `internal/skills/embedded{,-rituals,-guidance}/**` (frontmatter included) plus the `#` comments of `embedded-statusline/*.sh`, is the mirror of `body-prose-id`, and is inert in a consumer repo. It masks non-prose link carriers, which is what gives the doc-link carve-out; code constructs are NOT exempt, since a command example ships to a consumer exactly as prose does. It fires two shapes — a real id, and a placeholder at any width other than canonical — at **error** severity, so either one blocks the push. Only the id-shaped-token subset has a stable machine shape and is enforced there; the history/provenance/rationale content class is held at review.

### What's enforced and where

Rules are enforced at named chokepoints, not by remembering a checklist. **Fire as early as the class allows:** pre-commit (shape-only) → pre-push (full `aiwf check` + lint + gitleaks) → CI (`internal/policies/` Go tests, coverage gates, build/vet/race matrix) → release tag. The last tier holds the one rule that cannot fire earlier: a milestone's delta is legitimately absent from the notes until its epic wraps, so asking before a release would need an in-flight-epic exemption. An `aiwf check` finding catches in-context before the work leaves the machine; a CI-only policy test is a backstop that fires after the trunk push has landed. For judgment classes no check can cover (prose coherence, design soundness, cross-reference correctness) the timely catch is the in-context wrap-ritual review, which feeds the human gate.

- **Blocking via CI lint / test / build:** the full `golangci-lint` set + `go vet` + `go test -race` + `go build` + `aiwf doctor --self-check` + `govulncheck`; and the `internal/policies/` Go tests — repo-specific invariants (trailer keys, sovereign acts, FSM wiring/no-cycle, `-parallel 8` cap, setup_test presence, git-test-env harden, stress-lane census, enum-literal adoption, closed-set status constants, trailer-order sync, atomic-write chokepoint, version single-source, validate/is-never-writes, layering direction, no-`time.Now`-in-core, `Violation.Policy` spelled as a string literal so the firing-fixture inventory can see every policy), the diff-scoped coverage gate + firing-fixture meta-gate + `test-executable-write` scan, and the skill/finding-code discoverability + ritual `skill_edit_provenance_backstop.go` backstop + `shipped_prose_assertion.go` ban + `coauthor_trailer_ban.go` range check + guidance-anchor + trailer-commit-drift policies. The `shipped-prose-assertion` ban judges *test source*, not prose, so it is immune to rewording; it scans every test package rather than `internal/policies` alone, because D-0070 scopes itself by surface and a package-scoped ban would make "wrote the test elsewhere" an exception nothing records. `test-executable-write` is CI-tier rather than pre-push only because the hook's existing diff-scoped block is comment-history-specific down to its failure message; `internal/stresstest`, `internal/contractverify` and `internal/policies` — the packages where the `ETXTBSY` flake has been observed firing, rather than merely being reachable — are additionally held clean whole-file by `TestAdoptedPackagesRouteThroughWriteExecutable`, since a diff-scoped rule forgives every line a change does not touch.
- **Blocking at commit composition:** the `commit-msg` hook's refusal grounds — a fabricated `aiwf-verb:` value, a subject claiming an AC its `aiwf-entity:` trailer does not name, an aiwf trailer block a blank line has hidden from git, a shipped-ritual edit naming no entity, and a `Co-Authored-By:` address listed under `provenance.refuse_coauthors` (D-0096). Each costs a retype here and an amend or a rebase once the commit exists.
- **Blocking pre-commit / pre-push:** `aiwf check --shape-only` (pre-commit); full `aiwf check` incl. `body-prose-id` and `skill-body-id` (pre-push; both block); the `golangci-lint` pre-push hook (`make install-hooks`, pinned by `prepush_lint_hook_test.go`); gitleaks path/secret scan (pre-push + CI); the diff-scoped `comment-history-attrition` scan (pre-push on Go-source pushes + CI coverage-gate) — a comment on a changed line narrating a superseded state blocks the push; `//history:ok <reason>` is the deliberate exception. Its whole-tree sibling `PolicyCommentHistoryAttritionTree` runs in the ordinary policy suite (`make check-fast` / `make ci` / CI), so the tree stays clean rather than merely not-getting-worse. It enumerates the tree through git, which Go's test cache cannot observe, so a local run can replay a pass that predates a newly-committed file; CI never caches (the workflows name the wrapper) and both hooks pass `-count=1`, so the stale window is local and add-only. `make comment-history-audit` is the focused run of it, passes `-count=1`, and is what the `wf-codebase-health` rubric's F2 reaches for during an audit.
- **Blocking at the release tag:** `make changelog-audit`, the second job of [`changelog-check.yml`](.github/workflows/changelog-check.yml) — a change to embedded content since the last release that the release's notes cite by no entity id fails the pushed `v*` tag. Its other finding, a shipped-surface commit carrying no `aiwf-entity` trailer, is reported without failing: with no entity named the audit cannot say whether an entry covers it (D-0087).
- **Advisory (code review):** `context.Context` first-arg, no new package-level mutable state, one-line dep justification, deliberate `go.mod` floor bumps, docs, entity bodies, and code comments stating the conclusion rather than the drafting history (§"Writing docs, entity bodies, and code comments") — the *judgment* half only; the phrase-shaped subset of the code-comment rule is mechanically gated (see the two bullets above).
