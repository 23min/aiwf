...
- **Advisory (code review):** `context.Context` first-arg, no new package-level mutable state, one-line dep justification, deliberate `go.mod` floor bumps, docs, entity bodies, and code comments stating the conclusion rather than the drafting history (§"Writing docs, entity bodies, and code comments").
