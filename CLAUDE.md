...
- **Active streamlined-cadence override (operator-directed; interim, pending G-0314).** For **all epics and milestones** — the standing default forward, until streamlining is formalized — `tdd: required` AC phase/met promotes flow **live without a per-promote gate** — run them as the cycle progresses and report. The implementation commit, the merge, and the push each remain their own gate; **push is never batched or streamlined**. This overrides the "timing-bearing promotes each its own gate" rule globally, recorded here so it survives `/compact`. The durable form is the `tdd.promote_gate: streamlined | strict` knob (G-0314); delete this note when it lands.
