...
- **Advisory (code review):** `context.Context` first-arg, no new package-level mutable state, one-line dep justification, deliberate `go.mod` floor bumps, docs, entity bodies, and code comments stating the conclusion rather than the drafting history (§"Writing docs, entity bodies, and code comments"). `make comment-history-audit` is the whole-tree, always-exit-0 companion to that last one — it runs the same policy against the empty tree so every line counts as changed, and is what the `wf-codebase-health` rubric's F2 reaches for during an audit.
