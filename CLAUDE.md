...
- **Scenarios that drive real, concurrently-scheduled subprocesses carry `//go:build stress`** and run only via `make stress-tests` (or `go test -tags stress ./internal/stresstest/ ./cmd/stresstest/`). Five qualify: `concurrent-milestone-race`, `concurrent-writer-at-scale`, `cross-worktree-id-race`, `mid-write-kill`, and `lock-kill`. Sharing a runner with `go test ./...` roughly doubles their wall time, which is reason enough — but for one of them it is also a correctness matter: `concurrent-milestone-race` requires N racing actors to serialize inside `AcquireRepoLock`'s two-second deadline, so a loaded runner turns contention into a reported violation. The other four judge only load-invariant facts. `mid-write-kill` and `lock-kill` reached that by bounding their observation on the watched process rather than a clock, so slowness delays their verdict instead of falsifying it. In `cmd/stresstest`, the whole-catalog runner tests (`TestRunRun_ScenarioAll_*`) carry the tag because `--scenario all` executes this class, and so does the lock-kill runner test, which drives a real lock-holding subprocess; `--scenario all`'s *selection* claim stays pinned untagged by `TestResolveScenarios_All_NamesEveryCatalogEntryInOrder`.

Adding a scenario means choosing its lane: a hermetic oracle goes untagged, a timing-shaped one carries the tag. G-0468 tracks making the remaining tagged oracle hermetic — `concurrent-milestone-race` — at which point every scenario's lane becomes a cost decision rather than a correctness one.
