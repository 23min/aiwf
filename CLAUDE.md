...
5. **Skills materialized into the consumer repo's `.claude/skills/wf-*` directory and gitignored.** Regenerated only on explicit `aiwf init` / `aiwf update`. Stable across `git checkout` by design.
