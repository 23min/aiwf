...
- **Gate discipline survives compaction.** Each mutating action — commit, push, merge, promote, archive, tag, `gh pr create`, deleting a branch — is its own gate. Prior approvals never carry forward, including across `/compact`. The default is to ask before each action, even when the next step looks like an obvious continuation of an approved sequence — obviousness is when the temptation to bundle is strongest, and bundling is when the user loses the ability to redirect mid-sequence.
  ...
  **The one sanctioned exception: the wf-patch wrap gate (declared-sequence gate).** At the end of a `wf-patch`, after the commit gate has passed, a single gate may cover the patch's terminal sequence — the local merge to mainline, the tracker-closure promote (`aiwf promote G-NNNN addressed --by-commit <sha>`) when the patch closes a gap, and cleanup (local branch deletion, worktree removal) — **provided the gate question enumerates every action verbatim**. Approval binds to exactly the enumerated list; the user can approve a subset ("all except the promote") because the enumeration makes partial approval expressible. **Push to origin is never part of the sequence** — it is the only action that leaves the machine, and it always stands as its own gate. Any deviation — a merge conflict, a check finding, unexpected dirty state, any action not on the list — aborts the sequence and re-gates from the point of deviation. The consolidation is sound only because the mechanical gates carry the safety load the per-action gates used to carry: `make ci` green at the verify step, the pre-push lint hook, and `aiwf check` pre-push (G-0179). Scope is wf-patch only; milestone and epic wraps keep per-action gates until the shape has proven itself there too.
