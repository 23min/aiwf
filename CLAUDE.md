...
- **A NoOp claims the verb's effect already holds, so every surface that effect spans must hold.** The baseline is the **loaded tree** — `tree.Load` reads the working tree and `aiwf check` validates it, so on-disk state *is* aiwf's notion of current state, and a structured-state verb comparing the loaded entity is comparing current state rather than a stale projection of it. A verb whose effect spans more than one surface checks each: `move` compares the `parent:` field *and* the file's location under the epic's directory, and a `--superseded-by` re-run does not converge while the reciprocal back-link is still missing. `edit-body --body-file` additionally compares HEAD, because it is a *commit-this-content* verb whose supported input includes "the content is already on disk" — a disk match alone would not prove the content is committed. That HEAD conjunct is verb-specific reasoning, not a general rule: generalized to the structured-state verbs it would refuse a same-state re-run against uncommitted frontmatter while a *different*-status one still committed the hand-edit, hardening the harmless path and leaving the harmful one. Whether a verb should run at all against uncommitted frontmatter is a **precondition** question, not a convergence one — and an open one: today every structured-state verb commits a hand-edited field as its own (G-0466).
