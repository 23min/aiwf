...
`aiwf-entity` is the whole of the requirement. No `aiwf-verb:` is asked for, because a ritual `SKILL.md` is source rather than an entity file and no aiwf verb commits it — the closed set `trailer-verb-unknown` enforces carries no value meaning "I edited a shipped surface", so requiring one would mandate exactly the fabricated trailer G-0150 closed. No `aiwf-actor:` is asked for either: where no verb ran, "who ran the verb" is undefined, and git's own author field already carries it. The owning entity's *status* is not consulted, only its existence — see [D-0071](work/decisions/D-0071-enforce-provenance-not-content-at-the-skill-edit-backstop.md) for why keying on status would rot, and [D-0070](work/decisions/D-0070-retire-prose-content-assertions-over-shipped-surfaces.md) for why content correctness in shipped prose is held at review instead.
