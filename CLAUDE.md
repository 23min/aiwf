...
- **Blocking pre-commit / pre-push:** `aiwf check --shape-only` (pre-commit); full `aiwf check` incl. `body-prose-id` and `skill-body-id` (pre-push); the `golangci-lint` pre-push hook (`make install-hooks`, pinned by `prepush_lint_hook_test.go`); gitleaks path/secret scan (pre-push + CI); the diff-scoped `comment-history-attrition` scan (pre-push on Go-source pushes + CI coverage-gate) — a comment on a changed line narrating a superseded state blocks the push; `//history:ok <reason>` is the deliberate exception. Its whole-tree sibling `PolicyCommentHistoryAttritionTree` runs in the ordinary policy suite (`make check-fast` / `make ci` / CI), so the tree stays clean rather than merely not-getting-worse; `make comment-history-audit` is the focused run of it, and is what the `wf-codebase-health` rubric's F2 reaches for during an audit.
