...
This directory holds three kinds of historical material:
...
3. **Specifications for surfaces no longer in use** — a shipped surface a reader can still reach, whose documentation is no longer kept in lockstep with the code. [`migration/`](migration/) is the case: it specifies the `aiwf import` manifest contract, and no project remains to import.

All three are kept because the reasoning is useful, not because they describe the active design. New readers should start with [`../working-paper.md`](../working-paper.md).
...
- [`migration/`](migration/) — the `aiwf import` manifest contract and the producer-side adoption guide. Entry point: [`from-prior-systems.md`](migration/from-prior-systems.md).
