...
> **Role in this repo.** This is the *rubric* for the periodic codebase
> health scorecards under [`docs/pocv3/`](..) — see
> [`health-scorecard-2026-06-04.md`](../health-scorecard-2026-06-04.md) and
> [`health-scorecard-2026-06-16.md`](../health-scorecard-2026-06-16.md), each
> of which scores aiwf against the A1–G3 principles below. It is a **field
> guide, not a chokepoint.** aiwf's mechanically-enforced rules live in
> [`CLAUDE.md`](../../../CLAUDE.md) and `internal/policies/`; a large
> overlapping subset of these principles is already pinned there (layering,
> atomic writes, single-source-of-truth, branch coverage, FSM closed-sets,
> trailer shape, …). This guide is the broader, stack-agnostic backdrop those
> rules are an instance of — consult it when designing a new module, planning
> a refactor, reviewing a non-trivial diff, writing a milestone spec that
> introduces new boundaries, or running the next scorecard. It is **not**
> auto-loaded into per-turn guidance and its principles are **not** all lifted
> into `CLAUDE.md`'s rules; where it goes beyond what is mechanically pinned,
> it binds only by being consulted. Not archival.
