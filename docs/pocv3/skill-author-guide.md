...
- **Adding an unknown frontmatter field** (e.g. `completed: 2026-04-30` on an epic, or `decided_by: <person>` on a decision). aiwf's schema is strict; the field is rejected and the file fails to parse. Fixed in aiwf via [G14](../poc-gaps.md) (parse failures no longer cascade) and [G15](../poc-gaps.md) (`aiwf schema` published). Avoid by following Rule 2.
...
- **Putting state in frontmatter that should be in body prose.** Dates, narrative explanations, decided-by labels, completion notes — these are body content. Frontmatter is for fields aiwf reads. Verified by [G16](../poc-gaps.md) (path/id-consistency) and the schema verb.
