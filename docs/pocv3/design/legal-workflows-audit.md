...
| Rule id | Category | Scope | Statement | Chokepoints | Severity | Facets | Notes |
|---|---|---|---|---|---|---|---|
| R-RULE-149 | FSM-meta | All kinds | **`fsm-history-consistent` check rule** — walks `git log` per entity; for each frontmatter `status:` change in history, verifies the (from-state, to-state) tuple is legal under the entity's FSM and that the commit carries the expected verb / actor / force trailers. Three disjoint subcodes (per D-0008): `illegal-transition` (FSM-illegal flip with no `aiwf-force:`), `forced-untrailered` (sovereign-act shape by a non-human actor without `aiwf-force:` — predicate mirrors M-0095's `requireHumanActorForSovereignAct` verb gate), and `manual-edit` (FSM-legal flip without `aiwf-verb:`, cleared by a chrono-descendant `aiwf-audit-only` commit). Merge commits are skipped (per D-0010). This is the chokepoint that makes the FSM a *tree-invariant* rather than just a verb-precondition. | Check: `fsm-history-consistent` in `internal/check/fsm_history_consistent.go` (M-0130) | Per-subcode: `illegal-transition` = error (hard-reject), `forced-untrailered` = error (hard-reject), `manual-edit` = warning (cleared by audit-only acknowledgment) | (new — no R-AUDIT facets) | **Implemented in M-0130 (E-0033); closes G-0132.** The three subcodes partition the violation space disjointly: illegal-transition catches FSM-illegal flips, forced-untrailered catches sovereign-act-shape flips lacking discipline, manual-edit catches FSM-legal flips lacking verb mediation. The existing `provenance-untrailered-entity-commit` (warning) remains as the broader trailer-absence chokepoint covering non-FSM trailers. |
