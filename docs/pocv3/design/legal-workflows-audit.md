...
>
> 1. **FSM-as-tree-invariant adopted** (review #3). §10.1's rules now read as history-invariants, not just verb-preconditions. R-RULE-019 is **restated**: the "markdown edit is the back-out path" framing is removed; the rule now says markdown edits that bypass the FSM are illegal, and the back-out paths are reversal entities / `aiwf reallocate` / `--force --reason`. New rule R-RULE-149 records the `fsm-history-consistent` check that closes the chokepoint — currently **unimplemented; tracked as G-0128, scheduled as M-0126** in E-0033.
...
>
> **Honest sweep (post-revision-2, 2026-05-18):** Re-opened verb files only read via `--help` during §4 extraction. Added 5 net-new rules (R-RULE-152..156): `aiwf milestone depends-on` (a real mutating verb missed in §4); `aiwf list --archived` filter semantics; `aiwf schema`/`template` no-consumer-repo relaxation; `aiwf whoami` git-config dependency; the `aiwf-tests:` trailer schema (write-strict / loose-read / aggregation rule). Also added an **enforcement-status legend at the top of §10.1** distinguishing the verb-time chokepoint (active today) from the history-walk chokepoint (target, pending G-0128) so §10.1's "hard-reject" severity doesn't outpace the code's actual behavior.
...
**Framing (revision 2):** The FSM constrains the **history of every entity**, not just the preconditions of `aiwf promote` / `aiwf cancel`. Any state change visible in the working tree must trace to an FSM-legal transition in the git history. Direct markdown edits that bypass the verb path are illegal — they leave a state with no FSM-legal predecessor. The `fsm-history-consistent` check (R-RULE-149, currently unimplemented) closes this chokepoint.

**Enforcement status legend (sweep addition):** "hard-reject" in §10.1 rows means the legality claim is enforced through *two* chokepoints, only one of which is active today:

- **Verb-time path (active):** `aiwf promote` / `aiwf cancel` consult `ValidateTransition`; illegal (from, to) tuples return an error and the verb refuses to commit. This is the chokepoint that exists in the code today.
- **History-walk path (target, pending G-0128):** `fsm-history-consistent` walks `git log` and validates each frontmatter `status:` change against the FSM, catching manual markdown edits that skip the verb. This is R-RULE-149's implementation. Until G-0128 lands, the closest partial coverage is `provenance-untrailered-entity-commit` (warning), which catches the *trailer* absence but not the specific FSM violation.

Both paths are required for the kernel's "correctness must not depend on the LLM choosing to enforce" commitment. §10.1 rows say "hard-reject" because that is the *target* severity; today's actual coverage of manual edits is "check-warning via partial chokepoint." This document tracks the target; G-0128 tracks the remaining implementation work.
...
| R-RULE-021 | FSM | Cancel target | `aiwf cancel` targets per (kind, current state). Single-lifecycle kinds: Epic → `cancelled`; Milestone → `cancelled`; Gap → `wontfix`. Multi-lifecycle kinds (state-aware): ADR/Decision `proposed\|accepted` → `rejected`; Contract `proposed\|accepted` → `rejected`, **Contract `deprecated` → `retired`** (the natural lifecycle endpoint; cancel-from-deprecated to rejected would violate the FSM since `deprecated → rejected` is not legal). | transition.go L110-120 (CancelTarget) — **see Notes: current code is not state-aware; tracked as G-0129** | hard-reject | R-AUDIT-0031, 0032, 0033 | **Code bug (G-0129).** Current `CancelTarget` returns flat `rejected` for Contract regardless of state. The right fix is state-aware `CancelTarget`; alternative is to refuse `aiwf cancel` on `deprecated` and force the operator to use `aiwf promote retired`. |
...
| Rule id | Category | Scope | Statement | Chokepoints | Severity | Facets | Notes |
|---|---|---|---|---|---|---|---|
| R-RULE-149 | FSM-meta | All kinds | **`fsm-history-consistent` check rule** (NEW, tracked as **G-0128**) — walks `git log` for each entity; for each frontmatter `status:` change in history, verifies the (from-state, to-state) tuple matches a legal FSM transition AND the commit carries the expected verb / actor / force trailers. Untrailered state changes (manual markdown edits without verb mediation), FSM-illegal transitions, and missing `--force` trailers on otherwise-illegal transitions all fire. This is the chokepoint that makes the FSM a *tree-invariant* rather than just a verb-precondition; without it, R-RULE-001..018's "hard-reject" framing is aspirational. | (none yet — pending G-0128 resolution; partial coverage today via the warning-level `provenance-untrailered-entity-commit`) | hard-reject (target severity once implemented; currently *unimplemented*) | (new — no R-AUDIT facets) | **Implementation tracked as G-0128.** Without this check, R-RULE-019 reads inconsistently with R-RULE-001..018; revision 2 commits to the (b) interpretation and accepts the implementation gap. The existing `provenance-untrailered-entity-commit` (warning) is the closest partial chokepoint. |
