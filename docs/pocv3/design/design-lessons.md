...
3. ~~**Verb-design checklist update**.~~ **Done.** [`CLAUDE.md`](../../../CLAUDE.md) now has a "Designing a new verb" subsection between Type design and the Pre-commit checklist; it asks "what verb undoes this?" with four acceptable answers and one unacceptable one (we'll figure that out later → not ready). Cites back to this document for the principle.
