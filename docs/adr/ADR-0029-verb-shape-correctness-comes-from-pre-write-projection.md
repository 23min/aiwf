...
- `verb.Apply` itself performs no *entity-content* validation. It trusts that any `*Plan` it receives already passed its producing verb's projection check, which is what keeps `Apply` a single, uniform seam (M-0186/AC-3, AC-5) rather than a place every verb must re-implement its own shape check. It is not, however, a bare write-then-commit: it refuses ahead of any filesystem work when the plan's trailers fail `CheckForceTrailerCoherence` (ADR-0040), when a git operation is in progress, on a staged conflict, on a carried symlink, and on a HEAD-divergent path (ADR-0038). What those five share is that none of them is answerable from the entity's content — each is a fact about the trailer set or the repository, visible only once the whole plan has been assembled, which is why they belong here and shape does not.
...
- `verb.Apply` staying free of *shape* validation is intentional and should not be "fixed" by adding a defensive shape check inside it — that would duplicate the real gate and mask a bug in whichever verb skipped its own projection check, rather than surfacing it at the verb layer where it belongs. This is a rule about entity shape, not a general rule that `Apply` may refuse nothing: [ADR-0040](ADR-0040-prevent-sovereign-acts-at-the-verb-route-ratify-at-the-history-route.md) places the sovereign-force coherence guard here precisely because a trailer set is not visible to any single verb, so the test for whether a guard belongs in `Apply` is whether the fact it checks can be seen before the plan is assembled — not whether `Apply` is allowed to refuse.
