...
5. **Concurrent-append safety, not atomic-replace.** The log file is a shared, append-only, multi-writer stream — this repo's general atomic-write discipline (`pathutil.AtomicWriteFile`: write a sibling temp file, then rename) is the wrong pattern here, not merely a stricter-than-needed one. Temp+rename requires reading and replacing the *entire* file per write, which is both wasteful and unsafe under concurrent writers (aiwf routinely runs many concurrent worktrees against one repo, and diagnostic logging must never become a new source of cross-process corruption). Instead, `internal/logger`'s file handle opens with `O_APPEND`, and every log record is emitted as exactly one `Write()` call carrying the fully-formed line. POSIX guarantees a single `write(2)` under `O_APPEND` is atomic with respect to the file offset, so concurrent processes appending to the same daily file are serialized by the kernel and never interleave or truncate each other's lines. This is a hard constraint on the implementation, not a suggestion: a buffered `io.Writer` that could split one record across two `Write()` calls (e.g. a `bufio.Writer` flushed mid-line) voids the guarantee and must not be used for this file handle.

6. **Structured fields, not interpolation.** Every emit binds context: `logger.Info("verb.commit", "verb", "promote", "entity", "M-0090", "sha", short)`. Never `logger.Info(fmt.Sprintf("promote committed %s for %s", sha, entity))`. The structured form is greppable, JSON-renderable, and survives a future "I want this in a dashboard" pivot.

7. **Per-verb bound logger.** Every top-level verb starts with `logger := slog.With("verb", v, "entity", id, "actor", actor, "run_id", uuid())`. Sub-functions log against the bound logger. The `run_id` is the same value that goes into the JSON envelope's `metadata.correlation_id` so an envelope line and a log line are cross-referenceable.
