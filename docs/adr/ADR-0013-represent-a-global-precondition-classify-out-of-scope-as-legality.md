...
The design goal is that **every per-cell consumer iterates `Rules()` (cells only) untouched, with zero per-rule skips** — while code- and verb-oriented drift checks union the two accessors:
...
- **Code-oriented AC-5 arms union the accessors.** `specIllegalErrorCodes()` (feeding the fourth arm `TestM0123_AC5_ImplToSpec_LegalityCodesReferenced`) and the spec→impl `ErrorCodesResolve` arm iterate `Rules()` ∪ `GlobalRules()`, so the reclassified legality code `provenance-authorization-out-of-scope` carried by the global rule is covered.
- **Verb coverage unions the accessors.** A verb can be represented entirely by global preconditions: D-0077 places `authorize` outside the transition table. The verb-coverage drift check reads both accessors so that moving such a rule does not require an exemption.
...
- No change to the `Rule` struct. Global rules live in the `GlobalRules()` accessor; every per-cell consumer reads `Rules()` (cells only) unchanged, while code- and verb-oriented drift checks read `Rules()` ∪ `GlobalRules()`. No per-cell consumer needs a global-rule exception.
