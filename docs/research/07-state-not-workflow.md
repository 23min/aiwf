...
The early empirical record is consistent with this shift, where it's been measured. Anthropic's December 2025 internal study reports engineers describing their own work as "70%+ to being a code reviewer/reviser rather than a net-new code writer" and framing the future role as "taking accountability for the work of 1, 5, or 100 Claudes." The MSR 2026 study of agent-authored pull requests found that 58% of human intervention effort on agent PRs was guidance-level (restricting agent behavior, enforcing project conventions) and only 17% was direct code changes — its authors summarized the trend as "collaboration with coding agents is shifting developer work from implementation to supervision, guidance and quality control." (Both citations carried by [`surveys/workflow-work-division-and-who-implements`](surveys/workflow-work-division-and-who-implements.md).) These data points don't settle the question, but they line up with the production-to-judgment shift this section claims.
...
The framework's positioning, then, is for **teams where LLM amplification has thinned the workflow shape**: solo developers, small teams doing trunk-based work, small product organizations, research-flavored engineering. These are not the only teams that exist; framing the framework as universal would be the same overreach as treating Spec Kit, Kiro, or ACE as one-size-fits-all. The DORA 2025 report's central finding — that AI is an *amplifier* of existing organizational strengths and weaknesses, raising both throughput and instability depending on platform maturity — is the same boundary stated from outside: the framework helps teams whose surrounding discipline lets state-keeping work, and won't rescue teams whose surrounding discipline doesn't (citation in [`surveys/workflow-work-division-and-who-implements`](surveys/workflow-work-division-and-who-implements.md)).
