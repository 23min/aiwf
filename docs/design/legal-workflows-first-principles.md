...
| R-FP-0068 | `aiwf add ac` | A new AC is seeded at the pre-cycle empty phase (`tdd_phase` absent) regardless of the parent's tdd policy; `red` means a failing test exists, recorded later by the live `"" → red` promote. | Codified in design-decisions.md. The kernel seeds the pre-cycle entry state and otherwise doesn't impose policy. | load-bearing | n/a |
...
| R-FP-0100 | frontmatter | Body validation is narrow: a required section that is present and empty is reported, and the born-complete kinds refuse an empty body at creation. Membership is refused where bodies are written — `aiwf add` on a body omitting a section, `aiwf edit-body` on a write dropping one HEAD carries, and the push on a commit that leaves one out (`entity-body-section-dropped`) — and by no tree-wide rule. | Codified in design-decisions.md ("Bodies are validated narrowly … Past that line, prose is the human's responsibility; the framework guarantees structural and referential stability of frontmatter"); the debt a write-time-only rule leaves standing is recorded in ADR-0049. | load-bearing | n/a |
...
| R-FP-0167 | anti-rule | A milestone is **not** required to enter `in_progress` with its ACs at any particular `tdd_phase`; ACs rest at the pre-cycle empty phase until their live `"" → red` promote. The kernel guards only the outcome (`met` requires `done`). | Codified in design-decisions.md §"What's not a kernel rule." The flow is the rituals plugin's concern. | load-bearing | n/a |
