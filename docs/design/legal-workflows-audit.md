...
| R-AUDIT-0113 | internal/cli/promote/promote.go | promote E-NNNN active | `aiwf promote E-NNNN active` | Sovereign-act: requires a `human/...` actor — a human reaches this transition with no flag at all, since it is FSM-legal. `--force --reason "..."` does not substitute: it relaxes the sovereign-act gate rather than the FSM, and `verb.Apply` then refuses the force trailer it produces from a non-human actor, so agent acts cannot activate epics by any route (M-0095, ADR-0040) | hard-reject |
...
| R-AUDIT-0115 | internal/cli/cancel/cancel.go | cancel verb | `aiwf cancel <id>` | Promotes to the kind's terminal-cancel status (`CancelTarget` mapping); same provenance/sovereign rules as promote | hard-reject |
...
| R-RULE-078 | Sovereign | Epic activate | `promote E-NNNN active` is sovereign: requires a `human/...` actor, who needs no flag — the transition is FSM-legal. `--force --reason "..."` does not substitute: it relaxes the sovereign-act gate rather than the FSM, and the apply seam then refuses the force trailer it produces from a non-human actor, so agent acts cannot activate epics by any route | verb/promote_sovereign_act.go (`requireHumanActorForSovereignAct`, unforced route); verb.Apply → `CheckForceTrailerCoherence` (forced route); policies/aiwf_promote_epic_active_audit.go (static) | hard-reject + policy-block | R-AUDIT-0050, 0113 | — |
