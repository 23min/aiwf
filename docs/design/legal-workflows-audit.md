...
| Rule id | Source | Citation | Scope | Statement | Severity if violated |
|---|---|---|---|---|---|
| R-AUDIT-0050 | policies/aiwf_promote_epic_active_audit.go | auditUnforcedSovereignActPromote | Epic promote and cancel verbs | Automation-shaped source (CI workflow, scripts, Makefiles) must not invoke a sovereign-act-shape transition without `--force --reason "..."` on the same line. The scanned spellings are derived from `entity.SovereignActShapes()`: `aiwf promote E-<id> <to>` for every entry, plus `aiwf cancel E-<id>` for entries that `aiwf cancel` reaches | policy-block |
