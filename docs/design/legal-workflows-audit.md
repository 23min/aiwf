...
| R-AUDIT-0163 | design-decisions.md | §Body templates | All kinds | Body sections are starting points; validation is narrow — a required section present and empty is reported, and the born-complete kinds refuse an empty body at creation. Past that, prose is the human's responsibility | partly enforced (`entity-body-empty` on a present-and-empty required section — warning for epic and milestone, error for the born-complete kinds; plus the born-complete create-time gate. Membership — whether the section is there at all — is refused at the write seams and by no tree-wide rule, so entities predating that refusal keep their omissions, G-0571) |
