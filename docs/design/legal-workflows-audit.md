...
| R-AUDIT-0040 | transition.go | L137-146 | AC | Self-transitions, unknown `from`, and unknown `to` all return false from `IsLegalACTransition`. Since M-0281 the AC verbs converge a self-target to a NoOp above this consult; unknown `from`/`to` still reach it and are refused | verb-level NoOp (self-target); hard-reject (unknown state) |
| R-AUDIT-0041 | transition.go | L122-126 | AC | `--force --reason` is the documented relaxation path. NOTE: the `acs-transition` verb-projection finding this row and transition.go's own comment named was never shipped — no such rule exists in `internal/check`. Enforcement is verb-time: `promoteAC` and `cancelAC` consult `IsLegalACTransition` directly and refuse before any write | verb-time refusal (without `--force`) |
...
| Rule id | Source | Citation | Scope | Statement | Severity if violated |
|---|---|---|---|---|---|
| R-AUDIT-0050 | policies/aiwf_promote_epic_active_audit.go | auditUnforcedSovereignActPromote | Epic promote and cancel verbs | Automation-shaped source (CI workflow, scripts, Makefiles) must not invoke a sovereign-act-shape transition without `--force` on the same line (the scan tests for `--force` alone; `--reason` is not examined). The scanned spellings are derived from `entity.SovereignActShapes()`: `aiwf promote <prefix>-<id> <to>` for every entry, plus `aiwf cancel <prefix>-<id>` once per prefix | policy-block |
| R-AUDIT-0051 | policies/apply_callers_lock.go | PolicyApplyCallersAcquireLock | All mutating verbs | Every dispatcher under `internal/cli/` (helper layer `internal/cli/cliutil/` excluded) that reaches `verb.Apply`, directly or via `cliutil.FinishVerb` / `cliutil.DecorateAndFinish`, must also call `cliutil.AcquireRepoLock` — concurrent verb invocations against the same repo are forbidden | policy-block |
...
| R-AUDIT-0070 | verb/apply.go | `CheckForceTrailerCoherence` | Sovereign verbs (`--force --reason`, `--audit-only`, `--to` on authorize) | A sovereign act traces to a named human. Enforced at `verb.Apply` over the fully-assembled trailer set, so every route is covered without being enumerated (ADR-0040) — there is no dispatcher-layer assertion, and D-0061 records why one cannot exist: what `--force` means is verb- and state-specific, so a check keyed on the flag refuses invocations the kernel permits | hard-reject |
...
| R-AUDIT-0086 | check/entity_id_narrow_width.go | runEntityIDNarrowWidth | All kinds | An active entity id narrower than the canonical 4-digit width (per ADR-0008) fires `entity-id-narrow-width`, on either id axis — the filename or the frontmatter `id:` — one finding per entity; parser tolerates narrow on input, renderer always emits canonical | check-error |
...
| R-AUDIT-0105 | verb.Apply (`--force`); verb/auditonly.go (`--audit-only`) | `--force --reason`, `--audit-only` | Sovereign acts | Sovereign-act flags require a `human/...` actor; non-human actors are refused before any write. The two flags are caught at different seams: `--force` at the apply seam, which every verb's commit passes through, and `--audit-only` on the audit-only verb's own path — ADR-0040 keeps `audit-only-non-human` off the apply seam because it fails the satisfiability test the force rules pass | hard-reject |
...
| R-AUDIT-0113 | internal/cli/promote/promote.go | promote of an epic | `aiwf promote E-NNNN <active\|done\|cancelled>` — all four legal epic transitions | Sovereign-act: requires a `human/...` actor — a human reaches these transitions with no flag at all, since they are FSM-legal. `--force --reason "..."` does not substitute: it relaxes the sovereign-act gate rather than the FSM, and `verb.Apply` then refuses the force trailer it produces from a non-human actor, so agent acts cannot open or close an epic by any route (M-0095, ADR-0040, ADR-0047). Both `cancelled` edges are legal through promote and gated here; they are *additionally* reachable by `aiwf cancel`, under R-AUDIT-0115 | hard-reject |
| R-AUDIT-0114 | internal/cli/promote/promote.go | promote M-NNNN done | `aiwf promote M-NNNN done` | Requires all ACs at terminal status (per `MilestoneCanGoDone`); otherwise hard-rejected unless `--force --reason "..."` | hard-reject |
| R-AUDIT-0115 | internal/cli/cancel/cancel.go | cancel verb | `aiwf cancel <id>` | Promotes to the kind's terminal-cancel status (`CancelTarget` mapping); same provenance/sovereign rules as promote. The sovereign half is enforced rather than merely asserted: `Cancel` consults `requireHumanActorForSovereignAct` before writing, at the same point promote does — ahead of the cascade guards (ADR-0047) | hard-reject |
| R-AUDIT-0116 | internal/cli/editbody/editbody.go | edit-body verb | `aiwf edit-body <id>` | Frontmatter is untouched; body replacement only. `--body-file` must not start with `---` (refused as that would imply frontmatter rewrite) | hard-reject |
| R-AUDIT-0117 | internal/cli/rename/rename.go | rename verb | `aiwf rename <id> <new-slug>` | Renames on-disk slug; id preserved; updates references in cross-linked entities | hard-reject |
| R-AUDIT-0118 | internal/cli/move/move.go | move verb | `aiwf move M-id --epic E-id` | Moves a milestone to a different epic; preserves milestone id; updates parent reference; target epic must exist and be non-terminal | hard-reject |
| R-AUDIT-0119 | internal/cli/reallocate/reallocate.go | reallocate verb | `aiwf reallocate <id>` | Renumbers entity to a new id (resolving collisions); rewrites every cross-reference in the tree in the same commit | hard-reject |
| R-AUDIT-0120 | internal/cli/retitle/retitle.go | retitle verb | `aiwf retitle <id> "<title>"` | Updates frontmatter title in one commit; re-derives the on-disk slug from it only while the slug still tracks the title (G-0108, narrowed by ADR-0037 — a slug set with `aiwf rename` is preserved); new title subject to the title_max_length cap | hard-reject |
| R-AUDIT-0121 | (retired) | width-migration verb | — | **Retired** — the one-shot verb that widened narrow-legacy entity ids to canonical width. Canonical width is now the only legal width for an active entity, reported as an error by `entity-id-narrow-width`; that error is itself a refusal condition for any verb running an `aiwf check` preflight, so the migrator could not have operated on the trees it existed to convert | N/A |
| R-AUDIT-0122 | internal/cli/authorize/authorize.go | authorize verb | `aiwf authorize <id> --to <agent>` | Opens an autonomous-work scope; requires `human/...` actor; refused on terminal scope-entity unless `--force --reason "..."` | hard-reject |
| R-AUDIT-0123 | internal/cli/authorize/authorize.go | authorize verb | `aiwf authorize <id> --pause "..."` / `--resume "..."` / `--end` | Pause/resume the most-recently-opened active scope on `<id>`; `--end` retires one named non-ended scope without touching the entity's status. Together these cycle the scope FSM (active → paused → active → ended) | hard-reject |
| R-AUDIT-0124 | internal/cli/archive/archive.go | archive verb | `aiwf archive --apply` | Sweeps qualifying (terminal-status) entities into per-kind `archive/` subdirs in one commit (ADR-0004); decoupled from FSM promotion | hard-reject |
| R-AUDIT-0125 | internal/cli/initcmd/initcmd.go | init verb | `aiwf init` | One-time setup; refuses to run on a repo that already has `aiwf.yaml` (no overwrite-existing behavior) | hard-reject |
| R-AUDIT-0126 | internal/cli/update/update.go | update verb | `aiwf update` | Re-materializes embedded skills into `.claude/skills/aiwf-*/`; refuses to clobber user-modified consumer hooks (`# aiwf:<hook>` marker controls regeneration) | hard-reject |
| R-AUDIT-0127 | internal/cli/upgrade/upgrade.go | upgrade verb | `aiwf upgrade [--version vX.Y.Z]` | Fetches a newer binary via `go install`, re-execs into `aiwf update`; pre-release pseudo-versions accepted (G43) | hard-reject |
| R-AUDIT-0128 | internal/cli/importcmd/importcmd.go | import verb | `aiwf import <manifest>` | Bulk-creates entities from a YAML/JSON manifest; one commit by default. On-collision behavior controlled by `--on-collision <fail\|skip\|update>` (default fail). `--dry-run` validates without writing | hard-reject |
| R-AUDIT-0129 | internal/cli/contract/contract.go | contract bind | `aiwf contract bind <C-id>` | Adds/replaces a binding in aiwf.yaml; `--validator`, `--schema`, `--fixtures` required (or `--force` to replace existing) | hard-reject |
| R-AUDIT-0130 | internal/cli/contract/contract.go | contract recipe install | `aiwf contract recipe install <name>` | Installs a validator from the embedded set or `--from <path>`; `--force` required to overwrite an existing declared validator | hard-reject |
| R-AUDIT-0131 | internal/cli/contract/contract.go | contract recipe remove | `aiwf contract recipe remove <name>` | Removes a declared validator; errors when any contract binding still references it | hard-reject |
| R-AUDIT-0132 | internal/cli/render/render.go | render --write | `aiwf render roadmap --write` / `aiwf render --format=html` | When `--write` (roadmap) or HTML output is requested, files are written but only the roadmap commit is created; HTML output is regenerated artifact (gitignored) | hard-reject |
...
| R-AUDIT-0144 | ADR-0008 | §Decision | All kinds | Every kernel id (E, M, G, D, C, ADR, AC, F) emits at canonical 4-digit width on output. Parsers tolerate narrower legacy widths (≥1 digit) on input, which keeps cross-references into narrow archived entities resolving | check-error (`entity-id-narrow-width`) on any narrow id in the active tree; archive entries are excluded permanently |
| R-AUDIT-0145 | ADR-0008 | §Decision | All kinds | Renderers and allocators always emit canonical width, so no verb can introduce a narrow id; one appearing in the active tree came from a hand-edit or a file move | hard-reject (renderer contract) |
...
| R-AUDIT-0163 | design-decisions.md | §Body templates | All kinds | Body sections are starting points; validation is narrow — a required section present and empty is reported, and the born-complete kinds refuse an empty body at creation. Past that, prose is the human's responsibility | partly enforced (`entity-body-empty` on a present-and-empty required section — warning for epic and milestone, error for the born-complete kinds; plus the born-complete create-time gate. Membership — whether the section is there at all — is refused at the write seams and at the push (`entity-body-section-dropped`), and by no tree-wide rule, so entities predating those seams keep their omissions, ADR-0049) |
...
| R-AUDIT-0184 | design-decisions.md | §Provenance | Scope FSM | A scope ends by either route, both recorded by an `aiwf-scope-ends:` trailer: automatically, when a promote or cancel takes the scope-entity terminal, covering every non-ended scope on it; or deliberately, via `aiwf authorize <id> --end`. Un-canceling a scope-entity does not resurrect ended scopes (strict end-on-terminal) | hard-reject |
...
- R-AUDIT-0114 — promote-verb pre-check in `internal/cli/promote/promote.go`
...
|---|---|---|---|---|---|---|---|
| R-RULE-001 | FSM | Epic `proposed` | Legal targets: `active` (via promote), `cancelled` (via cancel). Any other target is illegal | FSM: transition.go L14; Validator: ValidateTransition; Verb: promote pre-check; Check: status-valid; Policy: aiwf_promote_epic_active_audit (scans automation-shaped source for both spellings) | hard-reject | R-AUDIT-0001, 0002, 0029, 0050, 0091, 0111, 0113 | Both targets are sovereign-act: `proposed → active` (M-0095) and `proposed → cancelled` (ADR-0047). Each requires a `human/...` actor, who needs no flag — `--force --reason` does not substitute; see R-RULE-078 |
| R-RULE-002 | FSM | Epic `active` | Legal targets: `done` (via promote), `cancelled` (via cancel). Any other target is illegal | FSM: transition.go L15; Validator + Verb + Check (same as R-RULE-001) | hard-reject | R-AUDIT-0003, 0004, 0029, 0091, 0111 | Both targets are sovereign-act per ADR-0047 — every edge into a terminal epic status requires a `human/...` actor; see R-RULE-078 |
...
| R-RULE-021 | FSM | Cancel target | `aiwf cancel` targets per (kind, current state). Single-lifecycle kinds: Epic → `cancelled`; Milestone → `cancelled`; Gap → `wontfix`. Multi-lifecycle kinds (state-aware): ADR/Decision `proposed` → `rejected`, **ADR/Decision `accepted` → ""** (no cancel target — `accepted` exits only via `promote → superseded`; the FSM forbids `accepted → rejected`, so the verb surfaces "no cancel target" rather than routing an illegal projection). Contract `proposed\|accepted` → `rejected`, **Contract `deprecated` → `retired`** (the natural lifecycle endpoint; cancel-from-deprecated to rejected would violate the FSM since `deprecated → rejected` is not legal). | transition.go L142-191 (`CancelTarget(kind, currentStatus)`); verb-level `IsTerminal` pre-flight in `Cancel` (promote.go) and reverse-lookup over `CancelTarget` in `CancelAuditOnly` (auditonly.go) keep the verbs in lock-step with the per-kind projection map | hard-reject | R-AUDIT-0031, 0032, 0033 | State-aware mapping for Contract landed in **M-0131** (closes **G-0131**). The ADR/Decision branch was made state-aware in **G-0163**: `accepted` returns "" because the FSM only exits via `promote → superseded`. |

### 10.2 AC FSM + TDD phase

| Rule id | Category | Scope | Statement | Chokepoints | Severity | Facets | Notes |
|---|---|---|---|---|---|---|---|
| R-RULE-022 | FSM | AC `open` | Legal targets: `met`, `deferred`, `cancelled` | transition.go L128; IsLegalACTransition; promote-verb dispatch (verb-time refusal; there is no `acs-transition` check rule) | hard-reject (without `--force --reason`) | R-AUDIT-0034, 0035, 0036, 0040, 0041 | — |
...
| R-RULE-042 | Body | All kinds | Body sections are starting points; validation is narrow — present-and-empty required sections are reported, born-complete kinds refuse an empty body at creation, membership is refused at the write and at the push, and what no seam reaches is recorded in ADR-0049 | verb/add.go `requireCompleteBody`; verb/editbody.go `sectionsDroppedSince`; check/entity_body_section_dropped.go | hard-reject (verb contract) + check-error | R-AUDIT-0163 | `entity-body-empty` fires on a present-and-empty required section; an absent one is refused at the write and at the push, and read by no tree-wide rule (ADR-0049) |
...
| R-RULE-045 | Body | All kinds | Empty body section (e.g., `## Acceptance criteria` with no content) fires | check.go `runEntityBodyEmpty` | check-warning for epic/milestone; check-error for the born-complete kinds (entity.IsBornComplete, G-0326) | R-AUDIT-0085 | — |

### 10.4 IDs + references + archive

| Rule id | Category | Scope | Statement | Chokepoints | Severity | Facets | Notes |
|---|---|---|---|---|---|---|---|
| R-RULE-046 | Ids | All kinds | Id format `<prefix>-NNNN` canonical 4-digit width on output; parsers tolerate 1-3 digit legacy widths on input (ADR-0008) | entity_id_narrow_width.go `entityIDNarrowWidth` (finding); renderers always emit canonical | check-error | R-AUDIT-0086, 0144, 0145 | — |
...
| R-RULE-073 | Scope FSM | All terminal promotes and cancels, plus `aiwf authorize --end` | A scope ends by either route, both recorded by an `aiwf-scope-ends:` trailer: automatically when the scope-entity reaches a terminal status, covering every non-ended scope on it; or deliberately, leaving the entity's status alone. Strict end-on-terminal: un-canceling does not resurrect ended scopes | design-decisions.md §Provenance | hard-reject | R-AUDIT-0184 | — |
...
| R-RULE-076 | Sovereign | All verbs | `--force` is human-only; non-human actors refused. Forced acts carry `aiwf-actor: human/...` + `aiwf-force:` trailers ONLY — no principal, no on-behalf-of | verb.Apply → `CheckForceTrailerCoherence` (internal/verb/coherence.go); policies/coherence_guard_chokepoint.go | hard-reject + policy-block | R-AUDIT-0070, 0105, 0185 | **Rationale (revision 2):** Sovereign = personal accountability. A human acting "on behalf of" their team in an emergency is asserting *personal* responsibility for the call; the team backs the decision politically, not provenance-wise. Allowing `aiwf-principal:` on a sovereign act would imply delegation, which contradicts the meaning of sovereign override. If delegation is genuinely needed, that's not sovereign — that's `aiwf authorize <id> --to <agent>` followed by a normal verb call. |
| R-RULE-077 | Sovereign | All verbs | `--audit-only --reason "..."` backfills audit trail when state was reached via manual commit; entity must already be at target state (no FSM transition); mutex with `--force`; human-only; produces empty-diff commit with `aiwf-audit-only:` trailer | verb/auditonly.go → `CheckTrailerCoherence` (the audit-only verb's own path, not the apply seam — ADR-0040 keeps `audit-only-non-human` off it deliberately) | hard-reject | R-AUDIT-0054, 0215 | — |
| R-RULE-078 | Sovereign | Epic opening and closure | Every legal epic transition is sovereign, so a human is the only actor who can open an epic or put one into a terminal status: `proposed → active` (M-0095), and every edge into a terminal status — `active → done`, `active → cancelled`, `proposed → cancelled` (ADR-0047). Each requires a `human/...` actor, who needs no flag — the transitions are FSM-legal. `--force --reason "..."` does not substitute: it relaxes the sovereign-act gate rather than the FSM, and the apply seam then refuses the force trailer it produces from a non-human actor, so agent acts cannot open or close an epic by any route. The two `cancelled` edges are reached by `aiwf cancel` rather than by promote, and that verb consults the same gate | verb/promote_sovereign_act.go (`requireHumanActorForSovereignAct`, unforced route, called from both promote and cancel); verb.Apply → `CheckForceTrailerCoherence` (forced route); policies/aiwf_promote_epic_active_audit.go (static, both spellings) | hard-reject + policy-block | R-AUDIT-0050, 0113, 0115 | ADR-0047 argues sovereignty from irreversibility rather than effort: `cancelled` is terminal whatever state it is reached from, so discarding a proposed epic is as unrecoverable as closing an active one. Milestones share the terminal shape and are deliberately excluded. |
...
| R-RULE-100 | Verb | `aiwf retitle` | Updates frontmatter title in one commit; re-derives the on-disk slug only while that slug is still the one the current title derives. A slug set with `aiwf rename` no longer tracks the title and is preserved (ADR-0037), so retitle owns the title and the canonical body H1 outright and the slug only as an unclaimed default. New title subject to title_max_length cap. | retitle verb | hard-reject | R-AUDIT-0120 | — |
...
| Rule id | Category | Scope | Statement | Chokepoints | Severity | Facets | Notes |
|---|---|---|---|---|---|---|---|
| R-RULE-149 | FSM-meta | All kinds | **`fsm-history-consistent` check rule** — walks `git log` over all reachable refs via the batched `gitops.BulkRevwalk` helper (M-0137/AC-1); reads frontmatter `status:` at each (commit, path) pair through the long-lived `gitops.BlobReader` cat-file pump (M-0137/AC-2). For each status change in history, verifies the (from-state, to-state) tuple is legal under the entity's FSM and that the commit carries the expected verb / actor / force trailers. **Four disjoint subcodes**: `illegal-transition` (FSM-illegal flip with no `aiwf-force:` — error), `forced-untrailered` (sovereign-act shape by a non-human actor without `aiwf-force:` — predicate mirrors M-0095's `requireHumanActorForSovereignAct` verb gate — error), `manual-edit` (FSM-legal flip without `aiwf-verb:`, cleared by a chrono-descendant `aiwf-audit-only` commit — warning), and `history-walk-error` (the walker hit a real failure reading the named entity's commit history — subprocess crash, blob-read protocol error, context cancelled mid-walk, or a non-zero blob the local object store cannot produce and git will not fetch, which is what a damaged store or a partial clone with lazy fetching off / its promisor remote unconfigured yields — a partial clone that still fetches backfills on read and never reaches this, and one whose promisor is configured but unreachable takes the subprocess-crash route above instead; the all-zero id naming the absent side of an add or delete is short-circuited ahead of the read, at either object-format width, and stays a silent skip — error). A status change landing in the same commit as a rename of the entity's file is observed like any other, via the diff record's pre-image blob — but only when that blob's frontmatter id names the same entity (directly or through `prior_ids`), since `-M` pairs a delete with an add by content similarity and would otherwise report a retired entity's status as an unrelated new one's prior state. The first three partition the legal-status-change observation space disjointly (per D-0008); `history-walk-error` is orthogonal — it surfaces walker failures so one transient subprocess error doesn't silently wipe the rule's findings (the M-0130 silent-swallow at `FSMHistoryConsistent:71-77`, closed in M-0137/AC-4+5 per CLAUDE.md §Engineering principles "Errors are findings, not parse failures"). Merge commits are skipped for the first three subcodes (per D-0010). This is the chokepoint that makes the FSM a *tree-invariant* rather than just a verb-precondition. | Check: `fsm-history-consistent` in `internal/check/fsm_history_consistent.go` (M-0130) + `internal/check/fsm_history_walker.go` (M-0137) | Per-subcode: `illegal-transition` = error (hard-reject), `forced-untrailered` = error (hard-reject), `manual-edit` = warning (cleared by audit-only acknowledgment), `history-walk-error` = error (one finding per failed (entity, commit) pair; partial findings preserved for the rest of the walk) | (new — no R-AUDIT facets) | **Implemented in M-0130 (E-0033); closes G-0132. Retrofitted in M-0137 (batched walker + history-walk-error subcode + partial-failure preservation); closes the fsm-history slice of G-0151.** The four subcodes partition the observation space: illegal-transition / forced-untrailered / manual-edit cover legal-status-change observations disjointly; history-walk-error covers the walker's own failure modes orthogonally. The existing `provenance-untrailered-entity-commit` (warning) remains as the broader trailer-absence chokepoint covering non-FSM trailers. |
| R-RULE-150 | FSM-meta | All FSMs | **Self-transitions are absent from every FSM** (entity, AC, TDD phase): `(from, from)` is never in any `transitions` map, so `ValidateTransition` / `IsLegalACTransition` / `IsLegalTDDPhaseTransition` all answer false for it. What the *verbs* do with that answer changed in M-0281 (ADR-0036): `promote` and `cancel` short-circuit **above** the FSM consult and converge a same-state request to a NoOp at exit 0, at entity and AC granularity alike, so a self-target is no longer operator-visible as a rejection. The FSM data is unchanged — the convergence is a verb-level identity case, not a self-edge. `promote --phase` still rejects (tracked separately). | transition.go (all `transitions` maps); `IsLegalACTransition`; `IsLegalTDDPhaseTransition`; verb-level NoOp guards in promote.go / cancel.go / ac.go | absent-from-FSM; verb-level NoOp | R-AUDIT-0029, 0040 (entity, AC); also implicit in TDD phase FSM | **Added in revision 2** for explicitness. The code already enforces this; the rule was previously stated only for ACs (R-AUDIT-0040) and was implicit for entity / TDD-phase FSMs. |
...
| Rule id | Category | Scope | Statement | Chokepoints | Severity | Facets | Notes |
|---|---|---|---|---|---|---|---|
| R-RULE-152 | Verb | `aiwf milestone depends-on <M-id> --on M-id,M-id` / `--clear` | Mutating verb that sets or clears a milestone's `depends_on` frontmatter list. **Replace-not-append** semantics — `--on` overwrites the existing list, does not extend it. `--on` and `--clear` are mutually exclusive. Forward-compatible with G-0073's cross-kind generalization (`aiwf <kind> depends-on <id> --on <ids>` extends to other kinds without renaming). Parent `aiwf milestone` is a verb group: naming it reaches no subverb, so it prints its help and reports a usage error, whether the subverb is missing or unrecognized (G-0528). | internal/cli/milestone/milestone.go `runMilestoneDependsOnCmd`; the group parent via `cliutil.NewVerbGroupCmd` | hard-reject | (no R-AUDIT facets — sweep addition) | **Sweep addition (2026-05-18).** Missed in §4; the verb is real and has specific legality rules. |
| R-RULE-153 | Verb | `aiwf list` | Read-only browser. Default semantic: list **non-terminal** entities (forward-compat with ADR-0004). `--archived` widens to include terminal-status entities. Filters: `--kind` (closed-set), `--status` (kind-aware via completion), `--parent` (entity id). `--format text\|json`; `--pretty` only with json. `--no-trunc` overrides terminal-width title truncation. | internal/cli/list/list.go `runListCmd`; flag-validation L126-138 | hard-reject (closed-set flag values) | (no R-AUDIT facets — sweep addition) | **Sweep addition.** Subsumed by R-RULE-084 (general read-only) but the `--archived` semantic (default-hide terminal) is its own rule. |
| R-RULE-154 | Verb | `aiwf schema [kind]` / `aiwf template [kind]` | Read-only printers for frontmatter contract (`schema`) and body-section template (`template`). Distinct from other read-only verbs: **do not require a consumer repo** — they print the kernel-embedded schema/template content and exit. No `aiwf.yaml` lookup, no tree load. | internal/cli/schema/schema.go; internal/cli/template/template.go | hard-reject (closed-set kind value when arg supplied) | (no R-AUDIT facets — sweep addition) | **Sweep addition.** §4 captured these as read-only verbs but missed their no-consumer-repo relaxation. |
| R-RULE-155 | Verb | `aiwf whoami` | Read-only; prints the resolved actor (from `git config user.email`, `--actor` override, or `AIWF_ACTOR` env if applicable) and the source it came from. Requires `git` to be on PATH and `git config user.email` to be set; otherwise prints diagnostic and exits non-zero. | internal/cli/whoami/whoami.go | hard-reject (git config dependency) | (no R-AUDIT facets — sweep addition) | **Sweep addition.** Provenance debug primitive; aligns with R-RULE-064 (runtime-derived identity). |
