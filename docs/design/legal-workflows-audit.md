...
| R-AUDIT-0123 | internal/cli/authorize/authorize.go | authorize verb | `aiwf authorize <id> --pause "..."` / `--resume "..."` | Pause/resume the most-recently-opened active scope on `<id>`; cycles the scope FSM (active → paused → active → ended) | hard-reject |
