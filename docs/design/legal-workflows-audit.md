...
| R-RULE-042 | Body | All kinds | Body sections are starting points; validation is narrow — present-and-empty required sections are reported, born-complete kinds refuse an empty body at creation, membership is refused at the write | verb/add.go `requireCompleteBody`; verb/editbody.go `sectionsDroppedSince` | hard-reject (verb contract) | R-AUDIT-0163 | `entity-body-empty` fires on a present-and-empty required section; an absent one is refused at the write and read by no tree-wide rule (G-0571) |
