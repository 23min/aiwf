...
| R-AUDIT-0070 | policies/sovereign.go | PolicySovereignDispatchersGuardHumanActor | Sovereign verbs (`--force --reason`, `--audit-only`, `--to` on authorize) | Every `internal/cli/` dispatcher package with a sovereign-act flag pair must call `cliutil.RefuseNonHumanSovereignForce` in code, so a non-human `--force` is refused before the repo lock and the tree load. The guard delegates to the rule `verb.Apply` enforces at the commit seam, so this layer adds a moment rather than a second opinion. Exempt: `add` (its `--force` is conditional, and a flag-keyed pre-check would refuse invocations that emit no force trailer) and `authorize` (already refuses every non-human actor, forced or not — strictly stronger); see `dispatchersGuardedElsewhere` for each reason and the test pinning it | policy-block |
...
| R-AUDIT-0113 | internal/cli/promote/promote.go | promote E-NNNN active | `aiwf promote E-NNNN active` | Sovereign-act: requires a `human/...` actor. `--force --reason "..."` does not substitute — it relaxes the FSM transition rule only, and `verb.Apply` refuses a force trailer from a non-human actor, so agent acts cannot activate epics by any route (M-0095, ADR-0040) | hard-reject |
...
| R-RULE-078 | Sovereign | Epic activate | `promote E-NNNN active` is sovereign: requires a `human/...` actor. `--force --reason "..."` does not substitute — it relaxes the FSM transition rule, and the apply seam refuses a force trailer from a non-human actor, so agent acts cannot activate epics by any route | verb/promote_sovereign_act.go (`requireHumanActorForSovereignAct`, unforced route); verb.Apply → `CheckForceTrailerCoherence` (forced route); policies/aiwf_promote_epic_active_audit.go (static) | hard-reject + policy-block | R-AUDIT-0050, 0113 | — |
