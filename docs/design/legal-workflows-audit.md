...
| R-AUDIT-0163 | design-decisions.md | §Body templates | All kinds | Body sections are starting points; validation is narrow — a required section present and empty is reported, and the born-complete kinds refuse an empty body at creation. Past that, prose is the human's responsibility | partly enforced (`entity-body-empty` on a present-and-empty required section — warning for epic and milestone, error for the born-complete kinds; plus the born-complete create-time gate. Membership — whether the section is there at all — is enforced nowhere, G-0571) |
...
| R-RULE-042 | Body | All kinds | Body sections are starting points; validation is narrow — present-and-empty required sections are reported, born-complete kinds refuse an empty body at creation, membership is unenforced | (design-decisions.md §Body templates) | partly enforced | R-AUDIT-0163 | `entity-body-empty` fires on a present-and-empty required section; an absent one is skipped by every surface (G-0571) |
