...
This is a self-contained summary of what the PoC commits to and why. It is the design context that motivates the build plan in [`poc-plan.md`](../archive/pocv3/poc-plan-pre-migration.md). The full research arc that produced these decisions lives on `main`; this branch deliberately does not include those documents to keep the working context focused.
...
Bodies are validated narrowly. `entity-body-empty` reports a required section that is present and empty, and the born-complete kinds (ADR, gap, decision, contract) refuse an empty body at creation, having no draft phase to fill one in later. Membership — whether the section is there at all — is held where bodies are written rather than by a tree-wide rule: `aiwf add` refuses a body omitting one, `aiwf edit-body` refuses a write that drops one the committed body carries, and the pre-push `aiwf check` refuses a push that leaves one out (`entity-body-section-dropped`). Every seam judges only what a write or a push changes, so a tree can still carry entities that omit a section (ADR-0049 measures the debt). Past that line, prose is the human's responsibility; the framework guarantees structural and referential stability of frontmatter.
...
Contracts ship as part of the PoC's mechanical-validation surface. The full design lives in [`contracts-plan.md`](../archive/pocv3/contracts-plan.md); the load-bearing decisions, in summary:
...
ACs are first-class but namespaced inside their milestone, addressable as `M-NNN/AC-N`. They are not a seventh entity kind; they are structured sub-elements of the milestone with composite ids, validated by `aiwf check` and reachable from `aiwf history`. The full design lives in [`acs-and-tdd-plan.md`](../archive/pocv3/acs-and-tdd-plan.md); the load-bearing decisions, in summary:
...
- **`aiwf add ac` seeds the pre-cycle empty phase.** A new AC is created with `tdd_phase` absent regardless of the parent's tdd policy — `red` means "a failing test exists," which a just-created AC has not written. The live `"" → red` promote records the failing test; auto-seeding `red` at creation would spend that one entry transition before any test exists (the FSM refuses `red → red`). The kernel makes no implicit TDD-policy decision at creation.
...
I2.5 sits between I2 (acceptance criteria + TDD) and I3 (governance HTML render). It addresses a gap that pre-I2.5 design conflated: the operator (who ran the verb) and the accountability-bearer (whose judgment authorized it). With humans, this is the same person; with LLM-mediated work, it isn't. The full design lives in [`provenance-model.md`](provenance-model.md); the build plan in [`provenance-model-plan.md`](../archive/pocv3/provenance-model-plan.md). Load-bearing decisions, in summary:
...
- **Three-layer trailer set.** `aiwf-actor:` (operator, unchanged) + `aiwf-principal:` (accountability-bearer, required when actor is non-human) + `aiwf-on-behalf-of:` and `aiwf-authorized-by:` (scope membership, required-together). Plus `aiwf-scope:` (state event on authorize verbs) and `aiwf-scope-ends:` (scope-termination markers, written by either route below). `aiwf-actor:` keeps its existing meaning — operator — so pre-I2.5 history reads unchanged.
- **Scope is a first-class FSM.** Closed states `active | paused | ended`. Authorization is a typed grant created by `aiwf authorize <id> --to <agent>`; pause/resume are first-class transitions. Multiple parallel scopes are supported. A scope ends by either of two routes, both recorded by an `aiwf-scope-ends:` trailer: automatically, when a promote or cancel takes the scope-entity to a terminal status, covering every non-ended scope on it; or deliberately, via `aiwf authorize <id> --end`, which retires one named scope and leaves the entity's status alone (ADR-0047). Strict end-on-terminal: un-canceling a scope-entity does not resurrect ended scopes.
...
- **`--force` is human-only.** Sovereign acts always trace to a named human. The kernel refuses an `aiwf-force:` trailer from any actor whose role is not `human/...`, at `verb.Apply`, before anything is written. The rule is keyed on the trailer rather than on the flag because the flag does not always produce one: `aiwf add --force` is inert on a kind with no body gate to bypass, and an invocation that overrides nothing records no sovereign act to refuse. Forced acts always carry only the existing `aiwf-actor: human/...` + `aiwf-force:` trailers; no principal, no on-behalf-of. A future delegated-force flag is filed as G23 and intentionally deferred.
...
- **Open extensions deliberately deferred.** Time-bound scopes, verb-set restrictions, pattern scopes, sub-agent delegation, bulk-import per-entity attribution, and delegated `--force` are filed as G22 (extension surface) and G23 (delegated force). YAGNI for the PoC; revisit when real friction shows up.

### Governance HTML render (added in I3)

A static-site generator (`aiwf render --format=html --out site/`) produces a per-repo governance page from canonical planning state. The render is a thin templating layer over `aiwf show <id> --format=json`; the kernel remains the truth layer. The full design lives in [`governance-html-plan.md`](../archive/pocv3/governance-html-plan.md); the load-bearing decisions, in summary:
...
| Materialized skill adapters (`.claude/skills/`, `.agents/skills/`) | In the consumer repo, gitignored | Composed for selected hosts from the binary on `aiwf init`/`update`; stable across `git checkout`. Ownership records identify generated files; a name or prefix alone never authorizes replacement. |
...
| Claude guidance fragment (`.claude/aiwf-guidance.md`) and root import; Codex native root guidance | Claude's fragment is gitignored; managed blocks live in user-owned `CLAUDE.md` and `AGENTS.md` | Composed/refreshed for selected hosts on `aiwf init`/`update`. Only aiwf's line-anchored marker block is maintained; surrounding content is preserved. Wiring defaults on, with independent `guidance.wire_claudemd` and `guidance.wire_agentsmd` opt-outs. These announced edits are reversible; delivery to a model is a separate observation. |
...
The materialization invariant is load-bearing: artifacts are regenerated only on explicit `aiwf init` / `aiwf update`, never implicitly on `git checkout` or every verb invocation. This keeps the AI's behavior stable when switching branches. Generated files are a cache of the embedded content; ownership metadata establishes which paths aiwf may refresh or retire. `aiwf doctor` reports drift by comparing generated files with their embedded versions. Marker-managed hooks refresh from their own templates.

The shared Claude/Codex artifact writer validates every selected family before
writing any of them. Each family's `.aiwf-owned` lists safe basenames: ASCII
letters, digits, hyphens, underscores and dots, excluding leading dots and the
reserved provenance `README.md`. Duplicates and unsafe paths are refused.
Output roots must be distinct, non-nested repository-relative directories. Every
existing path component below the supplied repository root is inspected without
following symlinks; even links within that root are refused. This preflight does
not lock out concurrent path replacement by another process.

A manifest entry owns only `SKILL.md` within a skill directory, or the named
flat agent/template file. Refresh replaces owned bytes; retirement removes only
that file and, for skills, its directory if empty. Supplemental files and foreign
siblings survive. A desired name occupied by unowned content causes an actionable
collision, even when its bytes match the current embed. Restore a trusted
ownership record or move the conflicting content before retrying; a filename
alone cannot establish ownership. The skills provenance README is separately
owned by its standalone generated marker. This deliberately tightens Claude's
legacy behavior, which overwrites same-name artifacts without prior ownership
and removes obsolete skill directories recursively. Ordinary successful Claude
artifact bytes and modes remain unchanged.

Before writing new artifacts, aiwf atomically writes a temporary `.aiwf-pending`
receipt in their family directory: the `aiwf-pending-v1` header followed by
SHA-256/name pairs. A retry claims a partial file only if its bytes match the
receipt. Verified recovered names are committed to the ownership manifest before
refresh, so another interruption remains recoverable even across binary changes.
The receipt is removed after the final ownership manifest succeeds; all three
Claude family receipts are gitignored. Existing manifest ownership remains
...
The full plan lives in [`upgrade-flow-plan.md`](../archive/pocv3/upgrade-flow-plan.md).
