...
| `tdd` | mapping | no | `require_test_metrics` (bool, default `false`) — opt in to the `acs-tdd-tests-missing` warning. When `true` and a milestone is `tdd: required`, ACs in `tdd_phase: done` whose first commit returned by `aiwf history` lacks an `aiwf-tests:` trailer produce a warning. When `false`, the trailer is purely informational and absence is not a finding. See [`governance-html-plan.md`](../archive/pocv3/governance-html-plan.md) §4. |
