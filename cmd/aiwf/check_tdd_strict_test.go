...
// (errors, exit exitFindings). The unit test on
...
func TestCheck_TDDStrict_EscalatesEntityBodyEmpty(t *testing.T) {
	...
	if rc := run([]string{"check", "--root", root}); rc != exitOK {
		t.Errorf("check without tdd.strict = %d, want exitOK (%d) — warnings should not block",
			rc, exitOK)
	}
	...
	// error severity; check exits with exitFindings.
	if rc := run([]string{"check", "--root", root}); rc != exitFindings {
		t.Errorf("check with tdd.strict = %d, want exitFindings (%d) — strict must escalate the rule to error",
			rc, exitFindings)
	}
}
