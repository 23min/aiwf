...
turn. The exception is **Code-health priming** at the end — read `.agents/skills/wf-codebase-health/SKILL.md` for the full rubric. When a workflow names a skill, load and follow its `SKILL.md` from `.agents/skills/<skill-name>/` before executing the steps. The operator can select skills with `/skills` or mention `$skill-name` in Codex CLI or the IDE; those are prompt controls, not shell commands.
...
- **Entity files are verb- and template-managed — never author one by copying a
  ...
  Create with `aiwf add`, filling the body from the kind's `.agents/aiwf/templates/`
