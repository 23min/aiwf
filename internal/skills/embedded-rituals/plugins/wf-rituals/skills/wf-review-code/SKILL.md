...
- Entity bodies and docs state what holds now, not that an earlier claim was wrong. Flag a paragraph retracting a claim the record used to make ("the previous note said Y, which was incorrect") — the false sentence goes, the current truth takes its place, and version control carries the provenance. The same standard as for comments, put as a question: would a reader who never saw the earlier version need this sentence?
- In each paragraph the diff adds or rewrites, flag every sentence that repeats what a check, a structured field, another document, or — for why the change was made — the commit message already holds, to be cut or replaced by a pointer to its owner, since nothing keeps the two in step: a defect if it already disagrees with that owner, a judgment if not. Keep what no other record carries — a judgment call, a rejected alternative, why the obvious approach fails, a dated measurement that is the point — and a summary a written project rule requires, such as a change-log entry, though one that disagrees with its source is still a defect.
