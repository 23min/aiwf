...
- A PR is open and the reviewer wants a structured walkthrough.

## Workflow

### 1. Understand the scope

- What change does this diff propose? Read the PR description, the issue, the spec, or whatever the project uses to capture the goal.
...
- Look at every changed file. `git diff <base>..HEAD`, or the PR diff in the host.
...
- If the project's TDD discipline expects red-before-green evidence (commit history, work log, linked PRs), the trail is present and consistent.
...
- **Blocking** — must be fixed before merge. Correctness, constraint violations, missing tests for AC, security issues.
- **Track for later** — worth recording somewhere durable (the project's gap log, an issue, a ticket), not this PR's scope. Note rough sizing if useful.
...
- <thing the reviewer noticed but the author already addressed in the PR>
...
- *Bundling blocking and non-blocking together.* The author needs to know what to fix before merge vs. what to consider afterwards. Keep them separate.
