...
1. Identify the docs root. Default: `docs/` **plus** the repo's hand-authored top-level narrative files — `README.md`, `CONTRIBUTING.md`. Some projects use `documentation/`, `book/`, or a flat top-level instead of `docs/`; read `README.md` or look for the obvious folder.
   - **Generated or gitignored root files stay out of scope** — a roadmap/status/whiteboard file regenerated by a project tool (e.g. `ROADMAP.md`, `STATUS.md`, `WHITEBOARD.md`) is never linted: if it drifts, the fix is to regenerate the source, not to file a lint finding.
   - **A working ordering file stays out of scope** — a hand-authored `TODO.md` or equivalent names work deliberately not done yet. Lint would read the not-yet as drift, and what clears it is the operator's next pass over the file, not a finding. Such a file is often neither generated nor gitignored, so the rule above does not reach it.
