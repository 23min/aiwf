...
- If the project uses aiwf and this cycle is driving a milestone AC:
    ...
    - Stop here — this cycle's job ends at `phase: done`. Whether the criterion counts as *met*, and when its implementation lands as a commit, are the calling milestone ritual's calls (e.g. `aiwfx-start-milestone` step 6) — this cycle produces the phase they read, and decides neither.
    - The kernel records the phase + status timeline via `aiwf history M-NNNN/AC-<N>`, and the implementation commit's entity trailer puts that commit in the same timeline — so nothing about the cycle needs writing down a second time.
