...
- Entity files are verb- and template-managed: create via `aiwf add` (fill the body from the kind's `.claude/templates/` file where one ships, else from `aiwf template <kind>`, which prints the required sections for any kind), edit body prose via `aiwf edit-body`, change frontmatter via the structured verb — never hand-write a file under `work/` or author one by copying a neighboring entity as a template.
