...
- `## Release note` — confirm it still describes what a consumer will observe after any corrective commits from step 2. A milestone that changed nothing user-facing writes that in those words; the section is never left blank, and a blank one blocks the promote to `done`.
