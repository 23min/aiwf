...
- `## Release note` — confirm it still describes what a consumer will observe after any corrective commits from step 2. A milestone that changed nothing user-facing writes that in those words; the section is never left blank. `aiwf check` reports a blank one once the milestone is `done`, which is a report rather than a gate — nothing downstream refuses it.
