...
- **Nothing is deleted at wrap.** Specs, the wrap artefact — all stay readable forever. Closure is a status change, not a deletion.
...
6. Neither the epic's own spec nor any milestone's left a gap open that it claims to close. Read each child milestone's `## Closes` section and check the status of every id it names — a named heading, so `aiwf show M-NNNN --format json` reaches it directly. `aiwfx-wrap-milestone`'s own wrap step should already have closed those; this is the backstop for a milestone wrapped under an older ritual version, or one closed outside the ritual. A milestone predating the section has no list to read, so fall back to its body prose. Nothing checks the epic's own spec earlier.

If precondition 1–5 fails, stop and report. Do not improvise around an unfinished epic. Precondition 6 is not a stop condition — the epic itself is otherwise ready — but a disposition to settle first: if a claimed-fixed gap surfaces still open, either close it (`aiwf promote G-NNNN addressed --by-commit <sha>`, citing the implementing commit), or, when the work advanced it without finishing it, correct the claim to say what landed and what remains. Don't close a partly-addressed gap to satisfy the check. Closing one here prints the live records whose bodies still name it: read what it prints, and correct a record whose claim the closure just falsified with `aiwf edit-body`, gated on its own since the declared sequence has not opened yet. Silence is the forbidden outcome: don't let it become a bare `## Follow-ups carried forward` entry instead.
...
The promote lists the live records whose bodies still name the epic; where none do, it prints nothing. Read what it prints before moving on — this is the one moment anyone holds both the closure and its consequences. Open each record named and look for a sentence that *depends* on the epic being live: work sequenced on it, a premise resting on it, something deferred to it. A record that merely mentions it in passing needs nothing. A record whose claim the closure just falsified needs `aiwf edit-body`, and the correction states what is so now — it does not add a paragraph saying the old claim was wrong.

Those edits are not on the declared sequence's list. Finish the enumerated actions (steps 8–9), then present the edits as their own gate, before the push (step 10) ships a body the closure just made stale. `aiwf edit-body` needs an active scope that reaches the record, and a citing record can sit anywhere in the tree, so a delegated actor may be refused; where it is, the human runs the correction. If nothing needs an edit, say so — an unremarked notice reads as unread.
