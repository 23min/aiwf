...
- *Skipping the Work log section.* One line per AC: the outcome, the commit as a locator, the test counts. The link from an AC to its implementation commit is answerable from that AC's own history once the commit carries the entity trailer above, so the line is a summary rather than the record. Reasoning belongs to the section that owns it — a trade-off or a rejected approach to `## Reviewer notes`, a decision to `## Decisions made during implementation`.
