...
5. **Replace the body with the rich template** at `{{aiwf:templates_dir}}/epic-spec.md` (materialized by `aiwf update`; if it's missing, run `aiwf update` rather than copying an existing epic spec). Fill in the sections below, each as the template directs beside it:
   - **Goal**
   - **Context**
   - **Scope**
   - **Out of scope**
