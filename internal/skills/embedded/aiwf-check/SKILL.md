...
| `trailer-verb-unknown` | A commit's `aiwf-verb:` trailer carries a value that is not in the closed set of registered verbs and subverbs — every command path from `aiwf`'s Cobra tree, joined by hyphens (e.g. `add`, `add-ac`, `milestone-depends-on`, `render-roadmap`). Typical sources: an LLM-fabricated value on a hand-rolled Conventional-Commits commit (e.g. `aiwf-verb: implement` on a `feat(...)` commit — the worked example from G-0150), or a plugin-side ritual verb that lives outside aiwf's CLI. **Warning severity at landing time** so the rule can introduce without retroactive breakage of historical fabricated trailers; promotion to error is contingent on cleaning history first. Fix: if the trailer was fabricated, amend the commit and drop the line — plain `feat(...)` / `fix(...)` code commits don't need an `aiwf-verb:` trailer. If a plugin emits the value, change the plugin to use a verb name aiwf registers (or to omit the trailer). Closes G-0150. |
