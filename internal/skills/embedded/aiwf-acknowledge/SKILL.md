...
The consuming rules walk HEAD's reachable history for `aiwf-force-for:` trailers and exempt findings whose offending commit appears in that set. The exemption is **DAG-scoped** (only trailers reachable from HEAD count, so a cherry-pick onto a branch lacking the original violation doesn't exempt it) and **per-SHA** — one ack covers every rule that consumes the blanket set, including every rule the provenance history audit raises, against that one commit and no other. So a reason written about one finding retires the commit's others too.

Two provenance-prefixed findings are outside the blanket set. `provenance-untrailered-entity-commit` is per-`(commit, entity)` and clears only under `--for-entity <id>`, whose binding the verb verifies against the commit's own diff; acking a SHA without it leaves the finding firing. `provenance-untrailered-scope-undefined` names no commit at all — it reports that the audit range was undetermined, so it is answered by configuring an upstream or passing `--since`, never by an ack.
...
```bash
aiwf acknowledge mistag G-NNNN \
