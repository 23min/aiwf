...
func TestM0147_AC3_GlobalRuleExercised(t *testing.T) {
	...
	// milestone; seed one so the positive arm below exercises the
	// scope machinery, not the AC-completeness guard.
	f.Must(verb.AddAC(ctx, f.Tree(), "M-0001", "Does the thing", human, nil))
	...
	// refusal under test must fire before the AC-completeness guard
	// would, not be masked by it.
	f.Must(verb.AddAC(ctx, f.Tree(), "M-0002", "Does the other thing", human, nil))
