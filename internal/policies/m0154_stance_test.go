...
// extractGoFuncWithDocComment returns the doc-comment block immediately
// preceding the named top-level Go function plus the function body, joined
// as a single string. Used by M-0154/AC-3 to scope its assertions to the
// `appendMarketplaceOverlapReport` region of doctor.go — the place that
// today states the un-narrowed stance in two surfaces (a doc-comment line
// and a user-facing fmt.Sprintf string). Empty string if the function is
// not found.
//
// Heuristic shape (works for Go's go-fmt'd output where top-level closing
// braces sit at column 0):
//
//   - Walk to the `func <name>(` line.
//   - Walk back over the contiguous block of `//` lines and blank lines
//     above the func, stopping at the first non-comment / non-blank line.
//   - Skip leading blanks back forward so the returned slice starts at the
//     first `//` line of the doc block.
//   - From the func line, walk forward to the first line that is exactly
//     `}` (top-level close); that closes the function body.
func extractGoFuncWithDocComment(src, funcName string) string {
	lines := strings.Split(src, "\n")
	prefixA := "func " + funcName + "("
	prefixB := "func " + funcName + " ("
	funcStart := -1
	for i, line := range lines {
		if strings.HasPrefix(line, prefixA) || strings.HasPrefix(line, prefixB) {
			funcStart = i
			break
		}
	}
	...
	for docStart < funcStart && strings.TrimSpace(lines[docStart]) == "" {
		docStart++
	}
	funcEnd := len(lines) - 1
	for k := funcStart + 1; k < len(lines); k++ {
		if lines[k] == "}" {
			funcEnd = k
			break
		}
...
// TestM0154_AC3_DoctorGoStanceSurfacesAmended asserts M-0154/AC-3:
// the two stance-mentioning surfaces inside doctor.go's
// `appendMarketplaceOverlapReport` — the doc comment that today states
// "it never edits settings.json itself", and the user-facing fmt.Sprintf
// string that today reads "aiwf will not edit your settings.json" — are
// both amended to the consent-gated stance. M-0154 is sole owner of this
// prose change; M-0156's wiring milestone does not touch it.
//
// Function-scoped per CLAUDE.md's "substring assertions are not structural
// assertions" rule — the assertions are bounded to the
// appendMarketplaceOverlapReport region (its doc comment + body), so a
// "consent" mention floating in some unrelated function would not
// erroneously satisfy this AC.
//
// Three layered checks:
//
//   - The region still mentions `settings.json` (the file the stance is
//     about — preserving the original subject).
//   - The region names a consent mechanism (`--wire-settings`, `[y/N]`,
//     or the word `consent`).
//   - The region cross-references `ADR-0015` — anchoring the runtime
//     prose to the ratified decision the same way CLAUDE.md does.
func TestM0154_AC3_DoctorGoStanceSurfacesAmended(t *testing.T) {
	t.Parallel()
	root := repoRoot(t)
	raw, err := os.ReadFile(filepath.Join(root, "internal", "cli", "doctor", "doctor.go"))
	if err != nil {
		t.Fatalf("reading doctor.go: %v", err)
	}
	region := extractGoFuncWithDocComment(string(raw), "appendMarketplaceOverlapReport")
	if region == "" {
		t.Fatal("AC-3: doctor.go must contain a top-level `appendMarketplaceOverlapReport` function — the stance prose lives in its doc comment and body")
	}

	if !strings.Contains(region, "settings.json") {
		t.Errorf("AC-3: doctor.go `appendMarketplaceOverlapReport` region must still mention `settings.json` (the file the consent-gated stance is about)")
	}

	hasConsentPhrase := strings.Contains(region, "--wire-settings") ||
		strings.Contains(region, "[y/N]") ||
		strings.Contains(region, "consent")
	if !hasConsentPhrase {
		t.Errorf("AC-3: doctor.go `appendMarketplaceOverlapReport` region must name a consent mechanism (`--wire-settings`, `[y/N]`, or the word `consent`) — both stance surfaces (doc-comment + user-facing string) need the amended language")
	}

	if !strings.Contains(region, "ADR-0015") {
		t.Errorf("AC-3: doctor.go `appendMarketplaceOverlapReport` region must cross-reference `ADR-0015` — runtime stance prose must be anchored to the ratified decision, not floating without a record")
	}
}
