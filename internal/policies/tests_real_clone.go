...
// tracking-refs case.
//
// Heuristic: any *_test.go file under cmd/aiwf/ that contains
...
// unit tests in internal/* may legitimately use update-ref to
...
func PolicyTestsRealCloneNotUpdateRef(root string) ([]Violation, error) {
	...
	for _, e := range cmdTests {
		...
		out = append(out, Violation{
			...
			File:   e.path,
			Detail: "uses `git update-ref refs/remotes/...` to fabricate a trunk-tracking ref but no test in cmd/aiwf/ uses `git init --bare` for a real clone scenario; synthetic single-repo trunk setups can pass while real first-push flows fail",
		})
	}
	return out, nil
}
