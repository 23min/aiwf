...
//     generated by running the suite over the working tree this audit
//     diffs, which is what makes the two agree. Required; an unset value
...
//     On a PR this is the merge-base with origin/main; on a trunk push it
//     is github.event.before. An empty or all-zero value means "no
...
// changed statement that is not annotated `//coverage:ignore`.
func branchCoverageViolations(root, profilePath, baseRef string) ([]Violation, error) {
	...
	// Untracked Go files count as wholly changed, for this audit only:
	// `git diff` cannot see them at any revision, and a file just written
	// is where an untested statement is most likely to live.
	if err := addUntrackedGoLines(root, changed); err != nil {
		return nil, err //coverage:ignore unreachable from here: changedLines already ran git against this same repo and returned, so a listing that fails now means git stopped resolving the repo mid-call. addUntrackedGoLines' own error path is unit-tested directly.
	}
...
// changedLines returns, per repo-relative path, the set of tracked Go
// line numbers that differ between baseRef and the working tree.
//
// The comparison target is the working tree, not HEAD, because every
// caller classifies content it reads from the working tree: this file's
// audit intersects these lines with a coverage profile produced by
// running the suite against that tree, and the comment history-attrition
// scan parses the same files off disk. Diffing HEAD while reading the
// working tree lines HEAD's numbering up against different content
// whenever the tree is dirty. Where the tree is clean — CI, and the push
// boundary — working tree and HEAD are the same tree, so the scope is
// identical.
//
// Untracked files are deliberately not included here. The coverage audit
// wants them and adds them itself; the comment history-attrition scans
// that share this helper do not, and the whole-tree one documents its
// subject as every *tracked* Go file. Widening the shared helper would
// have made an untracked scratch file fail `make check-fast`.
func changedLines(root, baseRef string) (map[string]map[int]bool, error) {
	result := map[string]map[int]bool{}
	if err := addDiffedGoLines(root, baseRef, result); err != nil {
		return nil, err
	}
	return result, nil
}

// addDiffedGoLines records the lines baseRef and the working tree differ
// on. It parses `git diff --unified=0`, taking each hunk's new-file range
// directly from the header (with -U0 there are no context lines, so the
// range is exactly the added/modified lines).
func addDiffedGoLines(root, baseRef string, result map[string]map[int]bool) error {
	cmd := exec.Command("git", "diff", "--unified=0", "--no-color", baseRef, "--", "*.go")
	cmd.Dir = root
	out, err := cmd.CombinedOutput()
	if err != nil {
		return fmt.Errorf("git diff %s in %s: %w\n%s", baseRef, root, err, out)
	}
	...
	if scErr := sc.Err(); scErr != nil { //coverage:ignore not portably triggerable: bufio.Scanner over an in-memory string with a 4MiB buffer errors only on a token longer than the buffer
		return fmt.Errorf("scanning git diff output: %w", scErr)
	}
	return nil
}

// addUntrackedGoLines records every line of each untracked, non-ignored Go
// file as changed. One git call up front, then pure file reads — the
// no-retry-loops-on-git policy keys on for-loops with git in the body.
func addUntrackedGoLines(root string, result map[string]map[int]bool) error {
	cmd := exec.Command("git", "ls-files", "--others", "--exclude-standard", "-z", "--", "*.go")
	cmd.Dir = root
	// Output, not CombinedOutput: this is the one git call here whose
	// parser is position-sensitive. A warning on stderr carries no NUL,
	// so folding it in would fuse it to the first path and silently drop
	// that file from the audit.
	out, err := cmd.Output()
	if err != nil {
		var stderr []byte
		...
		return fmt.Errorf("git ls-files --others in %s: %w\n%s", root, err, stderr)
	}
	...
	for _, rel := range strings.Split(strings.TrimRight(string(out), "\x00"), "\x00") {
		...
		src, srcErr := readSourceLines(filepath.Join(root, filepath.FromSlash(rel)))
		if srcErr != nil {
			// Listed but unreadable — gone between the listing and the
			// read, or a dangling symlink. Nothing to audit either way.
			continue
		}
		// A path can be listed here and also appear in the diff, when it
		// was removed from the index but kept on disk. That diff entry is
		// a deletion, which newFilePath records nothing for, so assigning
		// a fresh set cannot discard lines another arm found.
		set := map[int]bool{}
		result[rel] = set
		// readSourceLines splits on "\n", so a newline-terminated file
		// yields a trailing empty element that is not a line.
		for ln := 1; ln <= trailingTrimmedLen(src); ln++ {
			set[ln] = true
		}
	}
	return nil
...
// trailingTrimmedLen returns the number of real lines in a split source
// file, discounting the empty element a trailing newline produces.
func trailingTrimmedLen(src []string) int {
