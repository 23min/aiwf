...
// promote (M-0276, D-0047): it classifies the working-tree dirty paths against
// the configured tdd.test_paths globs and refuses when any non-test path is
// already dirty (implementation before test) or when nothing is dirty at all (a
// red phase with no failing test written).
//
