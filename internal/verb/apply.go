...
func Apply(ctx context.Context, root string, p *Plan) (sha string, err error) {
	// Coherence is checked here, not inside each verb, because a verb's
	// trailer set is incomplete when the verb returns: the CLI layer
	// appends aiwf-principal, aiwf-on-behalf-of, aiwf-authorized-by and
	// aiwf-scope-ends to the plan afterwards. Apply is the one seam
	// downstream both of that decoration and of the verbs that assemble a
	// complete set themselves, so it is the only point where the whole
	// set is visible while nothing has been written yet.
	//
	// It runs first so a refusal costs no filesystem work and leaves HEAD
	// where it was.
	if cohErr := CheckTrailerCoherence(p.Trailers); cohErr != nil {
		return "", cohErr
	}
