...
func NewCmd(correlationID string) *cobra.Command {
  ...
  aiwf authorize E-NNNN --to ai/claude --reason "delegated cobra+completion epic"

  # Pause the most-recently-opened active scope
  aiwf authorize E-NNNN --pause "blocked on review feedback"

  # Resume the most-recently-paused scope
  aiwf authorize E-NNNN --resume "review feedback addressed"

  # End the entity's only non-ended scope, leaving its status alone
  aiwf authorize E-NNNN --end --reason "taking this back in-loop"

  # End one scope by name when the entity carries several
  aiwf authorize E-NNNN --end --scope 1a2b3c4 --reason "agent reassigned"`,
	...
	cmd.Flags().BoolVar(&end, "end", false, "end a non-ended scope on <id> without changing the entity's status; requires --reason, and --scope when the entity carries more than one candidate")
	...
	cmd.Flags().StringVar(&branch, "branch", "", "ritual branch the scope is bound to; when set, the authorize commit carries an aiwf-branch: trailer with this value. From `main` or a ritual-shape current branch (epic/milestone/patch), naming a ritual-shape future branch is accepted, since the start-epic and start-milestone rituals authorize before they cut their branch. The named branch is cut by a later step of the ritual.")
