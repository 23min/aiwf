...
func Run(id, newSlug, actor, principal, root string, out cliutil.OutputFormat) (code int) {
	rootDir, actorStr, code, ok := cliutil.ResolvePrelude("aiwf rename", root, actor)
	if !ok {
		return code
	}
	...
	finish := cliutil.BeginVerbDiag(rootDir, "rename", id, actorStr, out.CorrelationID)
	var sha string
	...
	code, sha = cliutil.DecorateAndFinish(ctx, rootDir, "aiwf rename", tr, result, err, pctx, out)
	return code
}
