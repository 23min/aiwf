...
func TestRetitle_Reason(t *testing.T) {
	...
	if rc := cli.Execute([]string{"history", "E-0001", "--root", root}); rc != cliutil.ExitOK {
...
func TestRetitle_TopLevel_BodyH1Sync(t *testing.T) {
	...
	if rc := cli.Execute([]string{"edit-body", "E-0001", "--body-file", bodyFile, "--actor", "human/test", "--root", root}); rc != cliutil.ExitOK {
		t.Fatalf("edit-body to inject H1: %d", rc)
	}

	if rc := cli.Execute([]string{"retitle", "E-0001", "Refocused Foundations", "--actor", "human/test", "--root", root}); rc != cliutil.ExitOK {
...
func TestRetitle_TopLevel_NoH1_BodyUnchanged(t *testing.T) {
	...
	if rc := cli.Execute([]string{"retitle", "E-0001", "Refocused", "--actor", "human/test", "--root", root}); rc != cliutil.ExitOK {
		t.Fatalf("retitle: %d", rc)
	}
...
func TestRetitle_AC_FrontmatterAndBody(t *testing.T) {
	t.Parallel()
	root := retitleSetup(t)
	if rc := cli.Execute([]string{"add", "ac", "M-0001", "--title", "original ac title", "--actor", "human/test", "--root", root}); rc != cliutil.ExitOK {
...
func TestRetitle_SlugSyncedToTitle(t *testing.T) {
	...
	if rc := cli.Execute([]string{"history", "G-0001", "--root", root}); rc != cliutil.ExitOK {
...
func TestRetitle_DispatcherSeam_Composite(t *testing.T) {
	t.Parallel()
	root := retitleSetup(t)
	if rc := cli.Execute([]string{"add", "ac", "M-0001", "--title", "original", "--actor", "human/test", "--root", root}); rc != cliutil.ExitOK {
		t.Fatalf("add ac: %d", rc)
	}
	...
	if rc := cli.Execute([]string{"history", "M-0001/AC-1", "--root", root}); rc != cliutil.ExitOK {
