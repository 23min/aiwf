...
func TestAddAC_BodyFile_BinaryEndToEnd(t *testing.T) {
	...
	if out, err := testutil.RunBin(t, root, binDir, nil, "add", "epic", "--title", "Body epic"); err != nil {
		t.Fatalf("add epic: %v\n%s", err, out)
	}
	if out, err := testutil.RunBin(t, root, binDir, nil, "add", "milestone", "--tdd", "none", "--epic", "E-0001", "--title", "Body milestone"); err != nil {
	...
	headBefore, err := testutil.RunGit(root, "rev-list", "--count", "HEAD")
	if err != nil {
	...
	headAfter, err := testutil.RunGit(root, "rev-list", "--count", "HEAD")
	if err != nil {
...
func TestAddAC_BodyFile_CountMismatch_RefusesPreAllocation(t *testing.T) {
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			out, runErr := testutil.RunBin(t, root, binDir, nil, tc.args...)
			if runErr == nil {
...
func TestAddAC_BodyFile_Stdin_SingleTitle_Succeeds(t *testing.T) {
	...
	out, err := testutil.RunBinStdin(t, root, binDir, strings.NewReader(stdinText),
		"add", "ac", "M-0001",
		"--title", "Stdin AC",
		"--body-file", "-")
	if err != nil {
...
func TestAddAC_BodyFile_Stdin_MultiTitle_Refused(t *testing.T) {
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			...
			out, runErr := testutil.RunBinStdin(t, root, binDir,
