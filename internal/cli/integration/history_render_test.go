...
func TestRenderHistory_AuthorizationFlow(t *testing.T) {
	...
	out, err := testutil.RunBin(t, root, binDir, nil, "history", "E-0001")
	if err != nil {
		t.Fatalf("history: %v\n%s", err, out)
	}
	...
	mout, err := testutil.RunBin(t, root, binDir, nil, "history", "M-0001")
	if err != nil {
	...
	// full 40-char SHA.
	mout2, err := testutil.RunBin(t, root, binDir, nil, "history", "--show-authorization", "M-0001")
	if err != nil {
	...
	logSHA, gErr := testutil.RunGit(root, "log", "--reverse", "-E",
