...
// the same dispatcher production uses (`cli.Execute([]string{...})`), parses
...
func TestEnvelopeSchemaConformance_AllJSONVerbs(t *testing.T) {
	standardSetup := func(t *testing.T, root string) {
		...
		if rc := cli.Execute([]string{"add", "ac", "M-0001", "--title", "AC sample", "--actor", "human/test", "--root", root}); rc != cliutil.ExitOK {
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			...
			captured := testutil.CaptureStdout(t, func() {
				if rc := cli.Execute(args); rc != cliutil.ExitOK && rc != cliutil.ExitFindings {
