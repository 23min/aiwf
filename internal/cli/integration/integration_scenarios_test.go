...
func initRepoFor(t *testing.T, email string) (root, binDir string) {
	...
	root = testutil.SetupGitRepoWithUpstream(t, email)
...
func TestScenario_TerminalPromoteEndsMultipleParallelScopes(t *testing.T) {
	...
	openersOut, err := testutil.RunGit(root, "log", "--reverse", "-E",
	...
	out, err := testutil.RunBin(t, root, binDir, nil, "show", "--format=json", "E-0001")
	if err != nil {
...
func TestScenario_PivotMidFlight(t *testing.T) {
	...
	if out, err := testutil.RunBin(t, root, binDir, nil, "authorize", "E-0002", "--pause", "back to E-01"); err != nil {
...
func authorizedByOf(t *testing.T, root, rev string) string {
	t.Helper()
	out, err := testutil.RunGit(root, "log", rev, "-1",
...
func TestScenario_ReallocatePreservesAuthorization(t *testing.T) {
	...
	if out, err := testutil.RunBin(t, root, binDir, nil, "reallocate", "E-0002"); err != nil {
...
func TestScenario_MultiCloneIdentity(t *testing.T) {
	...
	if out, err := testutil.RunBin(t, rootA, binDirA, nil, "add", "epic", "--title", "Alice's epic"); err != nil {
		t.Fatalf("alice add: %v\n%s", err, out)
	}
	if out, err := testutil.RunBin(t, rootB, binDirB, nil, "add", "epic", "--title", "Bob's epic"); err != nil {
		t.Fatalf("bob add: %v\n%s", err, out)
	}
...
func TestScenario_AuthorizeWithOnBehalfOfNeutralAtCheck(t *testing.T) {
	...
	authSHAout, err := testutil.RunGit(root, "rev-parse", "HEAD")
	if err != nil {
