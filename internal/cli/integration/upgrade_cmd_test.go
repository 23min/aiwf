...
func TestRunUpgrade_CheckOnly_NoNetworkRequired(t *testing.T) {
	...
	rc, stdout, _ := testutil.CaptureRun(t, func() int {
		return cli.Execute([]string{"upgrade", "--check"})
	})
...
`
	...
	rc, _, stderr := testutil.CaptureRun(t, func() int {
		return cli.Execute([]string{"upgrade", "--version", "v0.4.0", "--root", tmp})
	})
...
`
	...
	rc, _, stderr := testutil.CaptureRun(t, func() int {
		return cli.Execute([]string{"upgrade", "--version", "v9.9.9", "--root", tmp})
	})
	if rc != cliutil.ExitInternal {
...
func TestRunUpgrade_BadFlag(t *testing.T) {
	rc, _, _ := testutil.CaptureRun(t, func() int {
		return cli.Execute([]string{"upgrade", "--nope"})
	})
