...
func milestoneDependsOnSetup(t *testing.T) string {
	...
	for i, slug := range []string{"First", "Second", "Third"} {
		_ = i
		if rc := cli.Execute([]string{"add", "milestone", "--epic", "E-0001", "--tdd", "none", "--title", slug, "--actor", "human/test", "--root", root}); rc != cliutil.ExitOK {
...
func TestMilestoneDependsOn_Replace(t *testing.T) {
	...
	if rc := cli.Execute([]string{"milestone", "depends-on", "M-0003", "--on", "M-0001", "--actor", "human/test", "--root", root}); rc != cliutil.ExitOK {
		t.Fatalf("first set: %d", rc)
	}
	if rc := cli.Execute([]string{"milestone", "depends-on", "M-0003", "--on", "M-0002", "--actor", "human/test", "--root", root}); rc != cliutil.ExitOK {
...
func TestMilestoneDependsOn_Clear(t *testing.T) {
	...
	if rc := cli.Execute([]string{"milestone", "depends-on", "M-0003", "--on", "M-0001,M-0002", "--actor", "human/test", "--root", root}); rc != cliutil.ExitOK {
		t.Fatalf("set initial: %d", rc)
	}
	if rc := cli.Execute([]string{"milestone", "depends-on", "M-0003", "--clear", "--actor", "human/test", "--root", root}); rc != cliutil.ExitOK {
		t.Fatalf("clear: %d", rc)
	}
...
func TestMilestoneDependsOn_DispatcherSeam_AddFlag(t *testing.T) {
	...
	if rc := cli.Execute([]string{"history", "M-0004", "--root", root}); rc != cliutil.ExitOK {
...
func TestMilestoneDependsOn_DispatcherSeam_Verb(t *testing.T) {
	...
	if rc := cli.Execute([]string{"history", "M-0003", "--root", root}); rc != cliutil.ExitOK {
