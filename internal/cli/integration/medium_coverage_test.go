...
func TestShow_CompositeIdWithScopes(t *testing.T) {
	...
	out, err := testutil.RunBin(t, root, binDir, nil, "show", "--format=json", "M-0001/AC-1")
	if err != nil {
...
func TestShow_AncestorScopeNotInheritedWithoutAct(t *testing.T) {
	...
	mout, mErr := testutil.RunBin(t, root, binDir, nil, "show", "--format=json", "M-0001")
	if mErr != nil {
...
func TestShow_MultipleScopesSorted(t *testing.T) {
	...
	if out, err := testutil.RunBin(t, root, binDir, nil, "authorize", "E-0001", "--pause", "switching agent"); err != nil {
