...
func makeClone(t *testing.T, bare, name string) string {
	...
	for _, args := range [][]string{
		...
		if out, err := testutil.RunGit(dir, args...); err != nil {
...
func aiwfInitClone(t *testing.T, clone, binDir string) {
	t.Helper()
	if out, err := testutil.RunBin(t, clone, binDir, nil, "init"); err != nil {
		t.Fatalf("aiwf init in %s: %v\n%s", clone, err, out)
	}
	// aiwf init does not create a commit, so commit the framework
	// files and push them to origin.
	if out, err := testutil.RunGit(clone, "add", "-A"); err != nil {
		t.Fatalf("git add -A: %v\n%s", err, out)
	}
	if out, err := testutil.RunGit(clone, "commit", "-q", "-m", "aiwf init"); err != nil {
		t.Fatalf("git commit init: %v\n%s", err, out)
	}
	if out, err := testutil.RunGit(clone, "push", "-q", "-u", "origin", "main"); err != nil {
...
func aiwfAddMilestone(t *testing.T, clone, binDir, title, epicID string) string {
	t.Helper()
	out, err := testutil.RunBin(t, clone, binDir, nil, "add", "milestone", "--tdd", "none", "--title", title, "--epic", epicID)
	if err != nil {
...
func aiwfAddEpic(t *testing.T, clone, binDir, title string) string {
	t.Helper()
	out, err := testutil.RunBin(t, clone, binDir, nil, "add", "epic", "--title", title)
	if err != nil {
...
func aiwfAddDecision(t *testing.T, clone, binDir, title string, relatesTo []string) string {
	...
	out, err := testutil.RunBin(t, clone, binDir, nil, args...)
	if err != nil {
...
func aiwfReallocateByPath(t *testing.T, clone, binDir, path string) string {
	t.Helper()
	out, err := testutil.RunBin(t, clone, binDir, nil, "reallocate", path)
	if err != nil {
...
func aiwfCheck(t *testing.T, clone, binDir string) (string, error) {
	t.Helper()
	return testutil.RunBin(t, clone, binDir, nil, "check")
}
...
func pushAll(t *testing.T, clone string) {
	t.Helper()
	if out, err := testutil.RunGit(clone, "push", "-q", "origin", "main"); err != nil {
...
func fetchOrigin(t *testing.T, clone string) {
	t.Helper()
	if out, err := testutil.RunGit(clone, "fetch", "-q", "origin"); err != nil {
...
func TestIntegrationG37_AllocatorSkipsTrunkAfterFetch(t *testing.T) {
	...
	if out, err := testutil.RunGit(cloneA, "add", "-A"); err != nil {
		t.Fatalf("git add: %v\n%s", err, out)
	}
	pushAll(t, cloneA)
...
func TestIntegrationG37_MixedKindsAcrossTrunk(t *testing.T) {
	...
	if out, err := testutil.RunGit(cloneB, "rebase", "-q", "origin/main"); err != nil {
...
func TestIntegrationG37_ReallocateUsesTrunkView(t *testing.T) {
	...
	out, err := testutil.RunBin(t, cloneB, binDir, nil, "reallocate", g004Path)
	if err != nil {
	...
	trailers, gitErr := testutil.RunGit(cloneB, "log", "-1", "--format=%(trailers)", "HEAD")
	if gitErr != nil {
...
func TestIntegrationG37_CleanFetchAndMergeRoundTrip(t *testing.T) {
	...
	if out, err := testutil.RunGit(cloneA, "pull", "-q", "--ff-only", "origin", "main"); err != nil {
		t.Fatalf("A pull: %v\n%s", err, out)
	}
...
func TestIntegrationG37_ReallocateRewritesRefsAndHistoryThreads(t *testing.T) {
	...
	histOld, err := testutil.RunBin(t, cloneB, binDir, nil, "history", "G-0001")
	if err != nil {
	...
	histNew, err := testutil.RunBin(t, cloneB, binDir, nil, "history", "G-0003")
	if err != nil {
...
func TestIntegrationG37_ReallocateTiebreakerPicksLocalSide(t *testing.T) {
	...
	out, err := testutil.RunBin(t, cloneB, binDir, nil, "reallocate", "G-0001")
	if err != nil {
...
func TestIntegrationG37_ReallocateTiebreakerAmbiguousNeitherInTrunk(t *testing.T) {
	...
	// aiwf-init).
	if out, err := testutil.RunGit(cloneB, "remote", "add", "peer", cloneA); err != nil {
		t.Fatalf("git remote add peer: %v\n%s", err, out)
	}
	if out, err := testutil.RunGit(cloneB, "fetch", "-q", "peer"); err != nil {
		t.Fatalf("git fetch peer: %v\n%s", err, out)
	}
	// A's HEAD is the only commit on top of init we want; cherry-pick
	// it into B's main.
	aHead, headErr := testutil.RunGit(cloneA, "rev-parse", "HEAD")
	if headErr != nil {
		t.Fatalf("rev-parse on A: %v", headErr)
	}
	if out, pickErr := testutil.RunGit(cloneB, "cherry-pick", strings.TrimSpace(aHead)); pickErr != nil {
...
func TestIntegrationG37_HistoryWalksLineageChain(t *testing.T) {
	...
	for _, q := range []string{"G-0001", "G-0002", "G-0003"} {
		out, err := testutil.RunBin(t, clone, binDir, nil, "history", q)
		if err != nil {
...
func TestIntegrationG37_PrePushHookCatchesCollision(t *testing.T) {
	...
	if out, err := testutil.RunBin(t, cloneB, binDir, nil, "update"); err != nil {
