...
func TestAddMilestone_TDDFlagRequired(t *testing.T) {
	...
	got := cli.Execute([]string{"add", "milestone", "--epic", "E-0001", "--title", "Bootstrap", "--actor", "human/test", "--root", root})
...
func TestAddMilestone_TDDValueValidation(t *testing.T) {
	...
	got := cli.Execute([]string{"add", "milestone", "--epic", "E-0001", "--title", "Bogus", "--tdd", "bogus", "--actor", "human/test", "--root", root})
	if got != cliutil.ExitUsage {
	...
	for _, val := range []string{"required", "advisory", "none"} {
		t.Run(val, func(t *testing.T) {
			...
			rc := cli.Execute([]string{"add", "milestone", "--epic", "E-0001", "--title", "Bootstrap " + val, "--tdd", val, "--actor", "human/test", "--root", subRoot})
...
func TestAddMilestone_TDDPersisted_Required(t *testing.T) {
	...
	if rc := cli.Execute([]string{"add", "milestone", "--epic", "E-0001", "--title", "Bootstrap", "--tdd", "required", "--actor", "human/test", "--root", root}); rc != cliutil.ExitOK {
...
func TestAddMilestone_TDDPersisted_Advisory(t *testing.T) {
	...
	if rc := cli.Execute([]string{"add", "milestone", "--epic", "E-0001", "--title", "Bootstrap", "--tdd", "advisory", "--actor", "human/test", "--root", root}); rc != cliutil.ExitOK {
		t.Fatalf("add milestone: %d", rc)
	}
	mPath := filepath.Join(root, "work", "epics", "E-0001-foundations", "M-0001-bootstrap.md")
	body, err := os.ReadFile(mPath)
...
func TestAddMilestone_TDDPersisted_None(t *testing.T) {
	...
	if rc := cli.Execute([]string{"add", "milestone", "--epic", "E-0001", "--title", "Bootstrap", "--tdd", "none", "--actor", "human/test", "--root", root}); rc != cliutil.ExitOK {
		t.Fatalf("add milestone: %d", rc)
	}
	mPath := filepath.Join(root, "work", "epics", "E-0001-foundations", "M-0001-bootstrap.md")
	body, err := os.ReadFile(mPath)
