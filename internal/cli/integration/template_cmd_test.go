...
func TestRunTemplate_OneKindRaw(t *testing.T) {
	out := string(testutil.CaptureStdout(t, func() {
		if rc := cli.Execute([]string{"template", "epic"}); rc != cliutil.ExitOK {
			t.Fatalf("rc = %d", rc)
		}
	}))
...
func TestRunTemplate_AllKindsHasHeaders(t *testing.T) {
	out := string(testutil.CaptureStdout(t, func() {
		if rc := cli.Execute([]string{"template"}); rc != cliutil.ExitOK {
			t.Fatalf("rc = %d", rc)
		}
	}))
...
func TestRunTemplate_UnknownKind(t *testing.T) {
	t.Parallel()
	if rc := cli.Execute([]string{"template", "nonsense"}); rc != cliutil.ExitUsage {
...
func TestRunTemplate_TooManyArgs(t *testing.T) {
	t.Parallel()
	if rc := cli.Execute([]string{"template", "epic", "milestone"}); rc != cliutil.ExitUsage {
		t.Errorf("rc = %d, want %d", rc, cliutil.ExitUsage)
	}
}
...
func TestRunTemplate_BadFormat(t *testing.T) {
	t.Parallel()
	if rc := cli.Execute([]string{"template", "--format", "yaml"}); rc != cliutil.ExitUsage {
		t.Errorf("rc = %d, want %d", rc, cliutil.ExitUsage)
	}
}
...
func TestRunTemplate_PrettyWithoutJSONIsHarmless(t *testing.T) {
	t.Parallel()
	if rc := cli.Execute([]string{"template", "--pretty", "epic"}); rc != cliutil.ExitOK {
...
func TestRunTemplate_JSONEnvelope(t *testing.T) {
	out := testutil.CaptureStdout(t, func() {
		if rc := cli.Execute([]string{"template", "--format", "json"}); rc != cliutil.ExitOK {
			t.Fatalf("rc = %d", rc)
		}
	})
	var env struct {
...
func TestRunTemplate_JSONOneKind(t *testing.T) {
	out := testutil.CaptureStdout(t, func() {
		if rc := cli.Execute([]string{"template", "--format", "json", "epic"}); rc != cliutil.ExitOK {
			t.Fatalf("rc = %d", rc)
		}
	})
	var env struct {
