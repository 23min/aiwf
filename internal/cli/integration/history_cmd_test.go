...
func TestRun_HistoryJSON(t *testing.T) {
	...
	captured := testutil.CaptureStdout(t, func() {
		if rc := cli.Execute([]string{"history", "--root", root, "--format=json", "E-0001"}); rc != cliutil.ExitOK {
...
func TestRun_HistoryMilestonePrefixMatchesACs(t *testing.T) {
	...
	if rc := cli.Execute([]string{"add", "ac", "--actor", "human/test", "--root", root, "M-0001", "--title", "AC one"}); rc != cliutil.ExitOK {
...
func TestRun_HistoryReadsAiwfToAndForce(t *testing.T) {
	...
	if rc := cli.Execute([]string{"add", "epic", "--title", "Bar", "--actor", "human/test", "--root", root}); rc != cliutil.ExitOK {
		t.Fatalf("add 2: %d", rc)
	}
	if rc := cli.Execute([]string{"promote", "--actor", "human/test", "--root", root, "E-0002", "done", "--force", "--reason", "sandbox emergency"}); rc != cliutil.ExitOK {
...
func TestRun_HistoryTextOutputIncludesForceLine(t *testing.T) {
	...
	if rc := cli.Execute([]string{"promote", "--actor", "human/test", "--root", root, "E-0001", "done", "--force", "--reason", "policy override"}); rc != cliutil.ExitOK {
		t.Fatalf("forced promote: %d", rc)
	}
...
func TestRun_HistoryUnknownIDIsEmpty(t *testing.T) {
	...
	if rc := cli.Execute([]string{"history", "--root", root, "E-0099"}); rc != cliutil.ExitOK {
...
func TestRun_HistoryReallocateBridgesBothIDs(t *testing.T) {
	...
	if rc := cli.Execute([]string{"reallocate", "--actor", "human/test", "--root", root, "E-0001"}); rc != cliutil.ExitOK {
