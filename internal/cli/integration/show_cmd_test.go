...
func TestRun_ShowArchivedIndicatorJSON(t *testing.T) {
	...
	out := testutil.CaptureStdout(t, func() {
		if rc := cli.Execute([]string{"show", "--format=json", "--root", root, "G-0099"}); rc != cliutil.ExitOK {
	...
	out2 := testutil.CaptureStdout(t, func() {
		if rc := cli.Execute([]string{"show", "--format=json", "--root", root, "G-0001"}); rc != cliutil.ExitOK {
...
func TestRun_ShowArchivedIndicatorTextHeader(t *testing.T) {
	...
	out := testutil.CaptureStdout(t, func() {
		if rc := cli.Execute([]string{"show", "--root", root, "G-0099"}); rc != cliutil.ExitOK {
	...
	out2 := testutil.CaptureStdout(t, func() {
		if rc := cli.Execute([]string{"show", "--root", root, "G-0001"}); rc != cliutil.ExitOK {
...
`
	...
	out := testutil.CaptureStdout(t, func() {
		if rc := cli.Execute([]string{"history", "--format=json", "--root", root, "G-0001"}); rc != cliutil.ExitOK {
...
func TestRun_ShowMilestoneAggregatesACsHistoryFindings(t *testing.T) {
	...
	if rc := cli.Execute([]string{"add", "milestone", "--tdd", "none", "--epic", "E-0001", "--title", "Engine warning", "--body-file", mBody, "--actor", "human/test", "--root", root}); rc != cliutil.ExitOK {
		t.Fatalf("add milestone: %d", rc)
	}
	if rc := cli.Execute([]string{"add", "ac", "--actor", "human/test", "--root", root, "M-0001", "--title", "AC one", "--body-file", acBody1}); rc != cliutil.ExitOK {
		t.Fatalf("add ac 1: %d", rc)
	}
	if rc := cli.Execute([]string{"add", "ac", "--actor", "human/test", "--root", root, "M-0001", "--title", "AC two", "--body-file", acBody2}); rc != cliutil.ExitOK {
	...
	out := testutil.CaptureStdout(t, func() {
		if rc := cli.Execute([]string{"show", "--root", root, "M-0001"}); rc != cliutil.ExitOK {
			t.Fatalf("show: %d", rc)
		}
	})
	s := string(out)
...
func TestRun_ShowCompositeIDRendersACSlice(t *testing.T) {
	...
	if rc := cli.Execute([]string{"add", "milestone", "--tdd", "none", "--epic", "E-0001", "--title", "First", "--actor", "human/test", "--root", root}); rc != cliutil.ExitOK {
		t.Fatalf("add milestone: %d", rc)
	}
	if rc := cli.Execute([]string{"add", "ac", "--actor", "human/test", "--root", root, "M-0001", "--title", "Just one"}); rc != cliutil.ExitOK {
	...
	out := testutil.CaptureStdout(t, func() {
		if rc := cli.Execute([]string{"show", "--root", root, "M-0001/AC-1"}); rc != cliutil.ExitOK {
			t.Fatalf("show: %d", rc)
		}
	})
	s := string(out)
...
func TestRun_ShowJSONEnvelope(t *testing.T) {
	...
	captured := testutil.CaptureStdout(t, func() {
		if rc := cli.Execute([]string{"show", "--root", root, "--format=json", "E-0001"}); rc != cliutil.ExitOK {
			t.Fatalf("show: %d", rc)
		}
	})
	var env struct {
...
func TestRun_ShowReferencedByPopulated(t *testing.T) {
	...
	if rc := cli.Execute([]string{"add", "milestone", "--tdd", "none", "--epic", "E-0001", "--title", "Second", "--actor", "human/test", "--root", root}); rc != cliutil.ExitOK {
...
func TestRun_ShowReferencedByEmptyIsPresent(t *testing.T) {
	...
	if rc := cli.Execute([]string{"add", "epic", "--title", "Lonely", "--actor", "human/test", "--root", root}); rc != cliutil.ExitOK {
		t.Fatalf("add epic: %d", rc)
	}
...
func TestRun_ShowFindingsScopedToEntity(t *testing.T) {
	...
	if rc := cli.Execute([]string{"add", "milestone", "--tdd", "none", "--epic", "E-0001", "--title", "Done milestone", "--actor", "human/test", "--root", root}); rc != cliutil.ExitOK {
		t.Fatalf("add milestone: %d", rc)
	}
	if rc := cli.Execute([]string{"add", "ac", "--actor", "human/test", "--root", root, "M-0001", "--title", "Open AC"}); rc != cliutil.ExitOK {
		t.Fatalf("add ac: %d", rc)
	}
...
func TestRun_ShowMilestoneACDescriptionsParsed(t *testing.T) {
	...
	if rc := cli.Execute([]string{"add", "ac", "--actor", "human/test", "--root", root, "M-0001", "--title", "Engine starts"}); rc != cliutil.ExitOK {
		t.Fatalf("add ac: %d", rc)
	}
	...
	captured := testutil.CaptureStdout(t, func() {
		if rc := cli.Execute([]string{"show", "--root", root, "--format=json", "M-0001"}); rc != cliutil.ExitOK {
			t.Fatalf("show: %d", rc)
		}
	})
	var env struct {
	...
	// payload.
	captured = testutil.CaptureStdout(t, func() {
		if rc := cli.Execute([]string{"show", "--root", root, "--format=json", "M-0001/AC-1"}); rc != cliutil.ExitOK {
