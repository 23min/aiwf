...
`)
	...
	captured := testutil.CaptureStdout(t, func() {
		if rc := cli.Execute([]string{"import", "--root", root, "--actor", "human/test", manifest}); rc != cliutil.ExitOK {
...
`)
	...
	captured := testutil.CaptureStdout(t, func() {
		if rc := cli.Execute([]string{"import", "--root", root, "--actor", "human/test", "--dry-run", manifest}); rc != cliutil.ExitOK {
...
`)
	...
	if rc := cli.Execute([]string{"import", "--root", root, "--actor", "human/test", manifest}); rc != cliutil.ExitFindings {
...
func TestRun_ImportPerEntityCommit(t *testing.T) {
	...
	if rc := cli.Execute([]string{"add", "epic", "--title", "Seed", "--actor", "human/test", "--root", root}); rc != cliutil.ExitOK {
