...
// the verb through `cli.Execute([]string{"archive", ...})` (the in-process
...
func TestArchive_DryRunByDefault(t *testing.T) {
	...
	if rc := cli.Execute([]string{"archive", "--root", root, "--actor", "human/test"}); rc != cliutil.ExitOK {
...
func TestArchive_ApplyProducesSingleCommit(t *testing.T) {
	...
	before := archiveCommitCount(t, root)
	if rc := cli.Execute([]string{"archive", "--apply", "--root", root, "--actor", "human/test"}); rc != cliutil.ExitOK {
...
func TestArchive_KindGapScopesSweep(t *testing.T) {
	...
	if rc := cli.Execute([]string{"archive", "--apply", "--kind", "gap", "--root", root, "--actor", "human/test"}); rc != cliutil.ExitOK {
...
func TestArchive_NoPositionalIDArg(t *testing.T) {
	t.Parallel()
	root := setupCLITestRepo(t)
	if rc := cli.Execute([]string{"archive", "G-0010", "--root", root, "--actor", "human/test"}); rc != cliutil.ExitUsage {
...
func TestArchive_NonHumanActorRequiresPrincipal(t *testing.T) {
	t.Parallel()
	root := setupCLITestRepo(t)
	if rc := cli.Execute([]string{"archive", "--root", root, "--actor", "ai/claude"}); rc != cliutil.ExitUsage {
...
func TestArchive_HumanActorForbidsPrincipal(t *testing.T) {
	t.Parallel()
	root := setupCLITestRepo(t)
	if rc := cli.Execute([]string{"archive", "--root", root, "--actor", "human/test", "--principal", "human/test"}); rc != cliutil.ExitUsage {
...
func TestArchive_InvalidKindRejected(t *testing.T) {
	t.Parallel()
	root := setupCLITestRepo(t)
	if rc := cli.Execute([]string{"archive", "--kind", "widget", "--root", root, "--actor", "human/test"}); rc != cliutil.ExitUsage {
...
func TestArchive_ExplicitDryRunFlag(t *testing.T) {
	...
	before := archiveCommitCount(t, root)
	if rc := cli.Execute([]string{"archive", "--dry-run", "--root", root, "--actor", "human/test"}); rc != cliutil.ExitOK {
		t.Fatalf("archive --dry-run rc = %d, want cliutil.ExitOK", rc)
	}
	after := archiveCommitCount(t, root)
	if delta := after - before; delta != 0 {
