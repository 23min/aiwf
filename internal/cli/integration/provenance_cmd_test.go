...
func TestProvenance_AuthorizedAgentPromote(t *testing.T) {
	...
	if out, err := testutil.RunBin(t, root, binDir, nil, "add", "milestone", "--tdd", "none", "--title", "Cache warmup", "--epic", "E-0001"); err != nil {
...
func TestProvenance_AgentRefusedOutOfScope(t *testing.T) {
	...
	if out, err := testutil.RunBin(t, root, binDir, nil, "add", "epic", "--title", "Unrelated"); err != nil {
		t.Fatalf("aiwf add epic E-02: %v\n%s", err, out)
	}
	if out, err := testutil.RunBin(t, root, binDir, nil, "promote", "E-0002", "active"); err != nil {
	...
	if out, err := testutil.RunBin(t, root, binDir, nil, "add", "ac", "M-0001", "--title", "Out-of-scope AC"); err != nil {
		t.Fatalf("aiwf add ac M-001: %v\n%s", err, out)
	}
...
func TestProvenance_ScopeEntityFollowsPriorEntityChain(t *testing.T) {
	...
	if out, err := testutil.RunBin(t, root, binDir, nil, "add", "ac", "M-0001", "--title", "First AC"); err != nil {
		t.Fatalf("aiwf add ac M-001: %v\n%s", err, out)
	}
...
func TestProvenance_TerminalPromoteEmitsScopeEnds(t *testing.T) {
	...
	out, runErr := testutil.RunBin(t, root, binDir, nil, "promote", "E-0001", "done")
	if runErr != nil {
