...
//
// Subprocess form (not in-process `cli.Execute([]string{...})`) is required:
...
func TestSeam_InitThenDoctorSelfCheck(t *testing.T) {
	...
	if out, err := testutil.RunBinaryAt(repo, bin, "init", "--actor", "human/test"); err != nil {
	...
	// here.
	out, err := testutil.RunBinaryAt(repo, bin, "doctor", "--self-check")
	if err != nil {
