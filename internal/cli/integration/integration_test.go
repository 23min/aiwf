...
func TestIntegration_FreshRepoLifecycle(t *testing.T) {
	...
	if addOut, addErr := testutil.RunBin(t, root, binDir, nil, "add", "epic", "--title", "Foundations"); addErr != nil {
		t.Fatalf("aiwf add: %v\n%s", addErr, addOut)
	}
...
func TestIntegration_TrunkExplicitMissingIsHardError(t *testing.T) {
	...
	if out, gitErr := testutil.RunGit(root, "remote", "add", "origin", "https://example.invalid/x.git"); gitErr != nil {
