...
// --- AC-1 local helpers -----------------------------------------
//
// The framework's existing helpers (OpenBoundScope, AICommit,
// SimulateAIEscape, CherryPick, etc.) target branch-choreography
// scenarios; the four below cover frontmatter / filesystem
// inspection that reallocate scenarios require. Kept local to the
// AC-1 file because they have no consumer outside reallocate
// scenarios today; if M-0161 or a follow-up surfaces a second
// caller, lift them into branch_scenarios_helpers_test.go in that
// milestone's refactor pass.
...
func findEntityFile(t *testing.T, env *ScenarioEnv, entityID string) string {
	...
	if err != nil {
		...
		// walk problem worth logging (reviewer nit M-0160/AC-1
		// T-nit-3: the prior sentinel-string comparison was fragile).
...
// from replaceStatusInFrontmatter in branch_scenarios_helpers_test.go
// (reviewer nit M-0160/AC-1 T-nit-4).
