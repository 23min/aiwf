...
// the verb through `cli.Execute([]string{"rewidth", ...})` (the in-process
...
func TestRewidth_DryRunByDefault(t *testing.T) {
	...
	if rc := cli.Execute([]string{"rewidth", "--root", root, "--actor", "human/test"}); rc != cliutil.ExitOK {
...
func TestRewidth_ApplyProducesSingleCommit(t *testing.T) {
	...
	before := rewidthCommitCount(t, root)
	if rc := cli.Execute([]string{"rewidth", "--apply", "--root", root, "--actor", "human/test"}); rc != cliutil.ExitOK {
...
func TestRewidth_NonHumanActorRequiresPrincipal(t *testing.T) {
	t.Parallel()
	root := setupCLITestRepo(t)
	if rc := cli.Execute([]string{"rewidth", "--root", root, "--actor", "ai/claude"}); rc != cliutil.ExitUsage {
...
func TestRewidth_HumanActorForbidsPrincipal(t *testing.T) {
	t.Parallel()
	root := setupCLITestRepo(t)
	if rc := cli.Execute([]string{"rewidth", "--root", root, "--actor", "human/test", "--principal", "human/test"}); rc != cliutil.ExitUsage {
...
func TestRewidth_PreflightApply_BailsOnAiwfCheckError(t *testing.T) {
	...
	before := rewidthCommitCount(t, root)
	rc := cli.Execute([]string{"rewidth", "--apply", "--root", root, "--actor", "human/test"})
...
func TestRewidth_PreflightApply_SkipChecksBypasses(t *testing.T) {
	...
	before := rewidthCommitCount(t, root)
	rc := cli.Execute([]string{"rewidth", "--apply", "--skip-checks", "--root", root, "--actor", "human/test"})
...
func TestRewidth_PreflightDryRun_NoGate(t *testing.T) {
	...
	before := rewidthCommitCount(t, root)
	rc := cli.Execute([]string{"rewidth", "--root", root, "--actor", "human/test"})
	if rc != cliutil.ExitOK {
