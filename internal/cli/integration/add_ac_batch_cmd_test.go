...
func TestAddAC_RepeatedTitle_BinaryEndToEnd(t *testing.T) {
	...
	if out, err := testutil.RunBin(t, root, binDir, nil, "add", "milestone", "--tdd", "none", "--epic", "E-0001", "--title", "Batch"); err != nil {
	...
	for _, acID := range want {
		histOut, histErr := testutil.RunBin(t, root, binDir, nil, "history", acID)
		if histErr != nil {
...
func TestAddAC_SingleTitle_BinaryUnchanged(t *testing.T) {
	...
	if out, err := testutil.RunBin(t, root, binDir, nil, "add", "milestone", "--tdd", "none", "--epic", "E-0001", "--title", "Single"); err != nil {
		t.Fatalf("add milestone: %v\n%s", err, out)
	}
	if out, err := testutil.RunBin(t, root, binDir, nil, "add", "ac", "M-0001", "--title", "lone criterion"); err != nil {
	...
	if subj != `aiwf add ac M-0001/AC-1 "lone criterion"` {
...
func TestAddAC_MissingTitle_RejectsCleanly(t *testing.T) {
	...
	out, err := testutil.RunBin(t, root, binDir, nil, "add", "ac", "M-0001")
	if err == nil {
