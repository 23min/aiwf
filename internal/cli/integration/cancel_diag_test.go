...
func TestCancelDiag_EmitsVerbCompletedEvent(t *testing.T) {
	...
	if rec.SHA == "" {
		t.Error("sha missing or empty from the diagnostic record (cancel produces a commit)")
	}
}

// TestCancelDiag_FailedRunEmitsFailedEvent pins M-0238/AC-6: a cancel
// that never reaches a successful outcome (here, a nonexistent entity)
// emits "verb.failed" — never "verb.completed" — carrying the exit
// code's error class.
func TestCancelDiag_FailedRunEmitsFailedEvent(t *testing.T) {
	root := setupCLITestRepo(t)
	mustRun(t, "init", "--root", root, "--actor", "human/test", "--skip-hook")
	...
	if rc != cliutil.ExitUsage {
		t.Fatalf("aiwf cancel on a nonexistent entity: rc=%d, want ExitUsage (%d)", rc, cliutil.ExitUsage)
	}
