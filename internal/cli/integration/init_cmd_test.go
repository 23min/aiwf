...
func TestRun_InitThroughDispatcher(t *testing.T) {
	...
	rc := cli.Execute([]string{"init", "--root", root, "--actor", "human/test", "--skip-hook"})
...
func TestRun_InitDryRun(t *testing.T) {
	...
	captured := testutil.CaptureStdout(t, func() {
		if rc := cli.Execute([]string{"init", "--root", root, "--actor", "human/test", "--skip-hook", "--dry-run"}); rc != cliutil.ExitOK {
