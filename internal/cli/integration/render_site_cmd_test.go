...
func TestRun_RenderHTML_DeterministicAcrossInvocations(t *testing.T) {
	...
	for _, rel := range []string{"index.html", "E-0001.html", "M-0001.html", "assets/style.css"} {
		a := testutil.ReadFileT(t, filepath.Join(out1, rel))
		b := testutil.ReadFileT(t, filepath.Join(out2, rel))
		if a != b {
...
func TestRun_Render_DispatcherDistinguishesSubcommandFromFormat(t *testing.T) {
	...
	if rc := cli.Execute([]string{"render", "nope", "--root", root}); rc == cliutil.ExitOK {
...
func TestRun_Render_HelpFlag(t *testing.T) {
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			var rc int
			captured := testutil.CaptureStdout(t, func() {
				rc = cli.Execute([]string{"render", tc.arg})
			})
