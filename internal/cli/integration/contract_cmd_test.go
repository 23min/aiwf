...
`), 0o644); err != nil {
	...
	captured := testutil.CaptureStdout(t, func() {
		if rc := cli.Execute([]string{"contract", "verify", "--root", root}); rc != cliutil.ExitOK {
...
`), 0o644); err != nil {
	...
	captured := testutil.CaptureStderr(t, func() {
		_ = testutil.CaptureStdout(t, func() {
			if rc := cli.Execute([]string{"contract", "verify", "--root", root}); rc != cliutil.ExitFindings {
...
`), 0o644); err != nil {
	...
	if rc := cli.Execute([]string{"contract", "unbind", "--root", root, "--actor", "human/test", "C-0001"}); rc != cliutil.ExitOK {
	...
	if rc := cli.Execute([]string{"check", "--root", root}); rc == cliutil.ExitInternal {
...
`), 0o644); err != nil {
	...
	captured := testutil.CaptureStderr(t, func() {
		if rc := cli.Execute([]string{"contract", "recipe", "remove", "--root", root, "--actor", "human/test", "fake"}); rc != cliutil.ExitUsage {
