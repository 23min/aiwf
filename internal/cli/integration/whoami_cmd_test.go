...
func TestRunWhoami_FromFlag(t *testing.T) {
	out := string(testutil.CaptureStdout(t, func() {
		if rc := cli.Execute([]string{"whoami", "--actor", "human/peter"}); rc != cliutil.ExitOK {
			t.Fatalf("rc = %d", rc)
		}
	}))
...
func TestRunWhoami_LegacyConfigActorIgnored(t *testing.T) {
	...
	out := string(testutil.CaptureStdout(t, func() {
		if rc := cli.Execute([]string{"whoami", "--root", root}); rc != cliutil.ExitOK {
			t.Fatalf("rc = %d", rc)
		}
	}))
...
func TestRunWhoami_FromGitConfig(t *testing.T) {
	...
	out := string(testutil.CaptureStdout(t, func() {
		if rc := cli.Execute([]string{"whoami"}); rc != cliutil.ExitOK {
			t.Fatalf("rc = %d", rc)
		}
	}))
...
func TestRunWhoami_NoActorAvailable(t *testing.T) {
	...
	if rc := cli.Execute([]string{"whoami"}); rc != cliutil.ExitFindings {
...
func TestRunWhoami_InvalidActorFlag(t *testing.T) {
	t.Parallel()
	if rc := cli.Execute([]string{"whoami", "--actor", "no-slash"}); rc != cliutil.ExitFindings {
		t.Errorf("rc = %d, want %d", rc, cliutil.ExitFindings)
	}
}
