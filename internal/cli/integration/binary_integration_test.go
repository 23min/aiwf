...
func TestBinary_VersionVerb_FallsBackToBuildInfo(t *testing.T) {
	...
	doctorOut, err := testutil.RunBinary(bin, "doctor", "--root", tmp)
	...
	row := testutil.ExtractRow(doctorOut, "binary:")
	if row == "" {
...
func TestBinary_MutatingVerbs_Subprocess(t *testing.T) {
	...
	runVerb := func(name string, args ...string) {
		t.Helper()
		out, err := testutil.RunBinaryAt(repo, bin, args...)
		if err != nil {
			t.Fatalf("aiwf %s: %v\n%s", name, err, out)
		}
	}
...
func TestBinary_DoctorSelfCheck_Passes(t *testing.T) {
	...
	out, err := testutil.RunBinary(bin, "doctor", "--self-check")
	if err != nil {
...
func TestBinary_ReadOnlyVerbs_ExitOK(t *testing.T) {
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			out, err := testutil.RunBinary(bin, tc.args...)
			got := 0
			if err != nil {
				if !testutil.ExitedWithCode(err, tc.want) {
...
func TestBinary_RenderHTML_EndToEnd(t *testing.T) {
	...
	mHTML := testutil.ReadFileT(t, filepath.Join(siteDir, "M-0001.html"))
