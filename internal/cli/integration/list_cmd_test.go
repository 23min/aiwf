...
// integration test that drives `cli.Execute([]string{"list", ...})` and asserts
...
func TestRun_List_CoreFlagsEndToEnd(t *testing.T) {
	...
	if rc := cli.Execute([]string{"add", "epic", "--title", "Active epic", "--actor", "human/test", "--root", root}); rc != cliutil.ExitOK {
		t.Fatalf("add epic E-01: %d", rc)
	}
	if rc := cli.Execute([]string{"add", "epic", "--title", "Planned epic", "--actor", "human/test", "--root", root}); rc != cliutil.ExitOK {
	...
	if rc := cli.Execute([]string{"add", "milestone", "--epic", "E-0001", "--title", "M one", "--tdd", "none", "--actor", "human/test", "--root", root}); rc != cliutil.ExitOK {
		t.Fatalf("add milestone M-001: %d", rc)
	}
	if rc := cli.Execute([]string{"add", "milestone", "--epic", "E-0002", "--title", "M two", "--tdd", "advisory", "--actor", "human/test", "--root", root}); rc != cliutil.ExitOK {
	...
	t.Run("no-args prints per-kind counts", func(t *testing.T) {
		...
		out := testutil.CaptureStdout(t, func() {
			rc = cli.Execute([]string{"list", "--root", root})
		})
	...
	t.Run("--kind milestone lists only milestones", func(t *testing.T) {
		var rc int
		out := testutil.CaptureStdout(t, func() {
			rc = cli.Execute([]string{"list", "--kind", "milestone", "--root", root})
		})
		if rc != cliutil.ExitOK {
			t.Fatalf("rc = %d, want cliutil.ExitOK", rc)
		}
	...
	t.Run("--status active scopes by status", func(t *testing.T) {
		var rc int
		out := testutil.CaptureStdout(t, func() {
			rc = cli.Execute([]string{"list", "--kind", "epic", "--status", "active", "--root", root})
		})
		if rc != cliutil.ExitOK {
			t.Fatalf("rc = %d, want cliutil.ExitOK", rc)
		}
	...
	t.Run("--parent scopes to children of an epic", func(t *testing.T) {
		var rc int
		out := testutil.CaptureStdout(t, func() {
			rc = cli.Execute([]string{"list", "--kind", "milestone", "--parent", "E-0001", "--root", root})
		})
		if rc != cliutil.ExitOK {
			t.Fatalf("rc = %d, want cliutil.ExitOK", rc)
		}
	...
	t.Run("--format=json --pretty parses as a JSON envelope", func(t *testing.T) {
		var rc int
		out := testutil.CaptureStdout(t, func() {
			rc = cli.Execute([]string{"list", "--kind", "milestone", "--format=json", "--pretty", "--root", root})
		})
		if rc != cliutil.ExitOK {
			t.Fatalf("rc = %d, want cliutil.ExitOK", rc)
		}
...
func TestRun_List_JSONResultIsArrayOfSummaryObjects(t *testing.T) {
	...
	out := testutil.CaptureStdout(t, func() {
		if rc := cli.Execute([]string{"list", "--kind", "milestone", "--format=json", "--root", root}); rc != cliutil.ExitOK {
...
func TestRun_List_ArchivedFlag(t *testing.T) {
	...
	if rc := cli.Execute([]string{"add", "milestone", "--epic", "E-0001", "--title", "Live", "--tdd", "none", "--actor", "human/test", "--root", root}); rc != cliutil.ExitOK {
	...
	if rc := cli.Execute([]string{"add", "ac", "M-0001", "--title", "Live check", "--actor", "human/test", "--root", root}); rc != cliutil.ExitOK {
	...
	if rc := cli.Execute([]string{"cancel", "--actor", "human/test", "--root", root, "M-0002"}); rc != cliutil.ExitOK {
		t.Fatalf("cancel M-002: %d", rc)
	}

	t.Run("default excludes terminal-status entities", func(t *testing.T) {
		out := testutil.CaptureStdout(t, func() {
			if rc := cli.Execute([]string{"list", "--kind", "milestone", "--root", root}); rc != cliutil.ExitOK {
				t.Fatalf("list rc != cliutil.ExitOK")
			}
		})
		s := string(out)
	...
	t.Run("--archived includes terminal-status entities", func(t *testing.T) {
		out := testutil.CaptureStdout(t, func() {
			if rc := cli.Execute([]string{"list", "--kind", "milestone", "--archived", "--root", root}); rc != cliutil.ExitOK {
	...
	t.Run("no-args counts exclude terminal entities", func(t *testing.T) {
		out := testutil.CaptureStdout(t, func() {
			if rc := cli.Execute([]string{"list", "--root", root}); rc != cliutil.ExitOK {
...
func TestRun_List_BadFormat(t *testing.T) {
	t.Parallel()
	root := setupCLITestRepo(t)
	if rc := cli.Execute([]string{"list", "--root", root, "--format=xml"}); rc != cliutil.ExitUsage {
...
func TestRun_List_BadKind(t *testing.T) {
	t.Parallel()
	root := setupCLITestRepo(t)
	if rc := cli.Execute([]string{"list", "--root", root, "--kind", "milestoneish"}); rc != cliutil.ExitUsage {
		t.Errorf("rc = %d, want cliutil.ExitUsage (%d)", rc, cliutil.ExitUsage)
	}
}
...
func TestSeam_ListAndStatusAgreeOnOpenGaps(t *testing.T) {
	...
	for _, r := range list.BuildListRows(context.Background(), tr, "gap", "open", "", "", "", false) {
