...
`), 0o644); err != nil {
	...
	out := testutil.CaptureStdout(t, func() {
		if rc := cli.Execute([]string{"status", "--root", root}); rc != cliutil.ExitOK {
...
func TestRunStatus_BadFormat(t *testing.T) {
	t.Parallel()
	rc := cli.Execute([]string{"status", "--format=xml"})
...
func TestReadRecentActivity_SkipsProseMentions(t *testing.T) {
	...
	if out, err := testutil.RunGit(root, "commit", "--allow-empty", "-m", realMsg); err != nil {
	...
	if out, err := testutil.RunGit(root, "commit", "--allow-empty", "-m", proseMsg); err != nil {
