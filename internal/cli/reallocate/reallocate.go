...
func Run(target, actor, principal, root string, out cliutil.OutputFormat) (code int) {
	rootDir, actorStr, code, ok := cliutil.ResolvePrelude("aiwf reallocate", root, actor)
	if !ok {
		return code
	}
	...
	finish := cliutil.BeginVerbDiag(rootDir, "reallocate", target, actorStr, out.CorrelationID)
	var sha string
	...
	code, sha = cliutil.DecorateAndFinish(ctx, rootDir, "aiwf reallocate", tr, result, err, pctx, out)
	return code
}
