...
func NewRootCmd(correlationID string) *cobra.Command {
	...
	cmd.AddCommand(history.NewCmd(correlationID))
	...
	cmd.AddCommand(list.NewCmd(correlationID))
...
Verbs:
  ...
  render roadmap [--write]       print ROADMAP.md (markdown of epics + milestones); --write updates the file on disk only (no commit — the caller commits it)
