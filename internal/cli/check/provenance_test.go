...
func TestRunProvenanceCheck_TrailerVerbUnknown_FiresOnUnpushedFabrication(t *testing.T) {
	...
	findings, err := RunProvenanceCheck(ctx, root, &tree.Tree{}, c0, registered, nil, nil, nil, check.WalkHeadCommits(ctx, root))
	if err != nil {
...
func TestRunProvenanceCheck_TrailerVerbUnknown_PostCutoffEmitsError(t *testing.T) {
	...
	findings, err := RunProvenanceCheck(ctx, root, &tree.Tree{}, c0, registered, nil, nil, postCutoff, check.WalkHeadCommits(ctx, root))
	if err != nil {
		t.Fatalf("RunProvenanceCheck: %v", err)
	}
	var found *check.Finding
...
func TestRunProvenanceCheck_EmptyRepoIsNoop(t *testing.T) {
	t.Parallel()
	findings, err := RunProvenanceCheck(context.Background(), t.TempDir(), &tree.Tree{}, "", nil, nil, nil, nil, nil)
	if err != nil {
		t.Fatalf("RunProvenanceCheck: %v", err)
	}
	if len(findings) != 0 {
