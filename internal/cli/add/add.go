...
func NewCmd(correlationID string) *cobra.Command {
  ...
  aiwf add contract --linked-adr ADR-0001 --title "Render envelope" \
	...
	cmd.Flags().BoolVar(&force, "force", false, "bypass the born-complete-kind empty-body gate (gap/decision/adr/contract only — G-0326: these kinds have no draft phase, so an empty body is refused at creation); requires --reason; inert on epic/milestone (no gate to bypass there); sovereign, so the actor must be human/... — a force trailer from a non-human actor is refused before anything is written")
