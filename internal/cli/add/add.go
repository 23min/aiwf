...
func NewCmd(correlationID string) *cobra.Command {
  ...
  aiwf add contract --linked-adr ADR-0001 --title "Render envelope" \
	...
	cmd.Flags().BoolVar(&force, "force", false, "bypass the body-completeness gate: a body omitting a section its kind requires (every kind), or a born-complete kind's section that is present and empty (gap/decision/adr/contract — G-0326: these kinds have no draft phase, so an empty body is refused at creation); requires --reason; inert where the body would have passed anyway, including every create that takes the kind's scaffold; forcing past the absence half exempts the entity permanently, since aiwf edit-body compares an edit against the committed body and finds the section already gone, and no check rule reports an absent one; sovereign, so the actor must be human/... — a force trailer from a non-human actor is refused before anything is written")
