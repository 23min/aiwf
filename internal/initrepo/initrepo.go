...
// setup for a consumer repo. See docs/archive/pocv3/poc-plan-pre-migration.md Session 3 for the
// full contract.
//
...
// init. The hook resolves the aiwf binary via `command -v aiwf` at
// hook-fire time (PATH-relative) rather than baking an absolute path
// at install time — install-time bake broke across multi-context dev
// (host ↔ devcontainer ↔ worktree) where install paths differ. Fail
// loud on not-found: pre-push validation must not silently skip.
//
...
//
// The aiwf binary resolves via `command -v aiwf` at hook-fire time
// (see preHookScript for the multi-context rationale). Drift between
// the installed body and this template is detected by `aiwf doctor`
// and remediated by `aiwf update`.
//
...
# during commit. Non-zero exit blocks: an error finding (only under
# tree.strict: true) or an aiwf.yaml that cannot be read; warnings
...
//
// The aiwf binary resolves via `command -v aiwf` at hook-fire time
// (see preHookScript for the multi-context rationale). The hook is
// tolerant by design: missing aiwf binary on PATH, parse errors, or
// a failed `aiwf status` invocation all exit 0 so the user's commit
// completes cleanly. The missing-binary case logs to stderr (visible
// during git commit) so an operator can investigate without losing
// their commit. Drift between the installed body and this template
// is detected by `aiwf doctor` and remediated by `aiwf update`.
//
...
func Init(ctx context.Context, root string, opts Options) (*Result, error) {
	...
	if slices.Contains(selection.Hosts, config.HostClaudeCode) {
		...
		claudeCreated = claudeStep.Action == ActionCreated
	...
	// The preview reads the unchanged disk; account for the CLAUDE.md that
	// the preceding scaffold step would create before guidance is wired.
	if opts.DryRun && claudeCreated {
		for i := range refresh.Steps {
			step := &refresh.Steps[i]
			if step.What == guidanceImportWhat && step.Action == ActionCreated {
...
// Guidance import markers wrap the aiwf-owned `@.claude/aiwf-guidance.md`
// line in the consumer's CLAUDE.md. Everything between the markers is
// aiwf-managed and refreshed on update; content outside them is the
// user's and is preserved verbatim (M-0164).
const (
	guidanceImportWhat        = "CLAUDE.md (aiwf guidance import)"
	guidanceImportStartMarker = "<!-- aiwf:guidance:START - DO NOT EDIT, regenerated by aiwf update -->"
	guidanceImportEndMarker   = "<!-- aiwf:guidance:END -->"
	guidanceImportPrefix      = "<!-- aiwf:guidance:"
)

// GuidanceMarkers returns the start and end markers of the aiwf:guidance
// managed block and the prefix every marker of that block family carries,
// in the shape pathutil.ManagedBlockSpan takes. The same block wraps the
// Claude import in CLAUDE.md and the rendered fragment in AGENTS.md.
func GuidanceMarkers() (start, end, prefix string) {
	return guidanceImportStartMarker, guidanceImportEndMarker, guidanceImportPrefix
}

// guidanceImportBlock is the full marker-wrapped import block written
// into CLAUDE.md. The import path equals skills.GuidanceFile, so the
// `@…` line always resolves to the materialized fragment (M-0164/AC-5).
func guidanceImportBlock() string {
...
func ensureGitignore(root string, selection config.HostSelection, statusMdAutoUpdate, dryRun bool) (StepResult, error) {
	...
	// aiwf.example.yaml (M-0232/AC-4) is unconditionally gitignored —
	// unlike STATUS.md there is no opt-out, so there is no remove case.
	addExampleYAML := !haveLine(config.ExampleFileName)

	if len(missing) == 0 && !addHTML && !removeHTML && !addStatusMd && !removeStatusMd && !addExampleYAML {
	...
	if len(missing) > 0 || addHTML || addStatusMd || addExampleYAML {
		...
		if len(missing) > 0 || addHTML {
			b.WriteString("# aiwf: marker-managed framework artifacts (regenerated by aiwf update)\n")
		...
		if addExampleYAML {
			// M-0232/AC-4: aiwf.example.yaml is a derived artifact,
			// always gitignored, no opt-out — own header block since
			// it is not a skill and carries no status_md-style toggle.
			if len(missing) > 0 || addHTML || addStatusMd {
				b.WriteString("\n")
			}
			b.WriteString("# aiwf: gitignored schema reference, regenerated by aiwf init/update\n")
			b.WriteString(config.ExampleFileName + "\n")
		}
		out = []byte(b.String())
	}
	...
	detail := buildGitignoreDetail(len(missing), addHTML, removeHTML, addStatusMd, removeStatusMd, addExampleYAML, htmlReason)
	return StepResult{
		What:   ".gitignore",
		Action: action,
		Detail: detail,
...
func buildGitignoreDetail(missingSkills int, addHTML, removeHTML, addStatusMd, removeStatusMd, addExampleYAML bool, htmlReason string) string {
	var parts []string
	if missingSkills > 0 {
	...
	if addExampleYAML {
		parts = append(parts, "added "+config.ExampleFileName)
	}
	if len(parts) == 0 {
		return ""
	}
