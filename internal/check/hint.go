...
var hintTable = map[string]string{
	...
	// ADR-0041: the target resolves from local branch refs only, so it
	// exists on this working copy and nowhere else — blocking, unlike
	// the published case above. The remedy is neither of the two
	...
	"refs-resolve/unresolved-unverified": "run `aiwf check` to settle it — this surface skipped the cross-branch scan, so it cannot tell an id that exists nowhere from one on an unmerged branch; the full check builds every tier",
	...
	"skill-body-claude-md-section": "a shipped surface cites a section of CLAUDE.md, which is repo-development guidance and never ships — `aiwf init` / `aiwf update` write only the guidance import line into a consumer's CLAUDE.md, so the citation dangles there or resolves against an unrelated section a consumer named the same way. State the rule inline in the surface itself, drop the citation, then re-run `aiwf check`. Naming the consumer's own CLAUDE.md without a section is fine and keeps passing",
	...
	"milestone-done-empty-release-note":    "write the milestone's user-visible delta under the release-note heading — what a consumer can now do, or what changed under them, or the words \"no user-visible change\" when there is none — then commit it with `aiwf edit-body <milestone-id>`",
	...
	// empty-body subcode: the AC already exists but its body is blank, so
	// the fix is edit-body (fill the contract), not add ac. Its own line
	// keeps the aligned block above from re-flowing on this longer key.
	"milestone-draft-incomplete-acs/empty-body": "fill the AC's body under its `### AC-N` heading at plan time via `aiwf edit-body <milestone-id>`; a warning only — a draft milestone is legitimately mid-planning, so no action is needed if you are still scoping it",
	...
	"entity-body-section-dropped": "restore the named `## <Section>` heading, with its content, via `aiwf edit-body <id>` — on an entity that is not terminal, a required section left empty is reported by `entity-body-empty` instead; to keep a removal you meant, run `aiwf acknowledge illegal <sha> --reason \"...\"` on the commit the finding names, adding `--for-entity <id>` when that commit is also reported by `provenance-untrailered-entity-commit`, so both clear",
	...
	"provenance-authorization-ended":                    "the scope was already ended (by a closing promote or cancel, or by `aiwf authorize <id> --end`); open a fresh scope with `aiwf authorize <id> --to <agent>`",
