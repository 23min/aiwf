...
var hintTable = map[string]string{
	...
	"skill-body-id":              "a shipped surface cites a real entity id, which is meaningless in a consumer repo and rots as the entity changes; write a canonical `<prefix>-NNNN` placeholder or a shape-description instead (or cite a design/ADR doc as a markdown link, so the id rides in the destination while the visible text stays descriptive), then re-run `aiwf check`. A command example or fenced transcript is in scope too — it ships to consumers exactly as prose does",
