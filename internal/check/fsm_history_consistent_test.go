...
func TestFSMHistoryConsistent_NilGuards(t *testing.T) {
	...
	for _, c := range cases {
		t.Run(c.name, func(t *testing.T) {
			t.Parallel()
			got := FSMHistoryConsistent(context.Background(), c.root, c.tr, nil, WalkHeadCommits(context.Background(), c.root))
			if got != nil {
...
func TestFSMHistoryConsistent_FiresOnIllegalTransition(t *testing.T) {
	...
	tr := r.tree()
	got := FSMHistoryConsistent(context.Background(), r.root, tr, nil, WalkHeadCommits(context.Background(), r.root))
	if len(got) != 1 {
