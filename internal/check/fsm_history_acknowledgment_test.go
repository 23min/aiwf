...
func TestFSMHistoryConsistent_AC2_AcknowledgmentExemptsIllegalTransition(t *testing.T) {
	...
	ackedSHAs := WalkAcknowledgedSHAs(context.Background(), r.root, WalkHeadCommits(context.Background(), r.root))
	got := FSMHistoryConsistent(context.Background(), r.root, r.tree(), ackedSHAs, WalkHeadCommits(context.Background(), r.root))
	for _, f := range got {
