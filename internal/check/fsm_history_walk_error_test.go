...
func TestFSMHistoryConsistent_AC4_CancelledContext_EmitsWalkError(t *testing.T) {
	...
	got := FSMHistoryConsistent(ctx, r.root, r.tree(), nil, WalkHeadCommits(ctx, r.root))
...
func TestFSMHistoryConsistent_AC5_PartialFailure_PreservesGoodFindings(t *testing.T) {
	...
	got := fsmHistoryConsistentWithDeps(context.Background(), r.root, tr, nil, WalkHeadCommits(context.Background(), r.root), fake)
